...
export function useTechnicianField(jobId: string) {
  ...
  return {
    ...
    note: useMutation({ mutationFn: (content: string) => api.addWorkNote(jobId, content), onSuccess: update }),
    approval: useMutation({
      mutationFn: (input: { disposition: CustomerDisposition; customerName: string; reason: string }) =>
        api.recordCustomerDisposition(jobId, input.disposition, input.customerName, input.reason),
      onSuccess: update,
    }),
    handoff: useMutation({ mutationFn: () => api.refreshInvoiceHandoff(jobId), onSuccess: update }),
