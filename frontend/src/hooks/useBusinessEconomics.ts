...
import { getEconomicsPolicyAdministration, getEconomicsResult, getEconomicsResultLineage, getEconomicsWorkspace, getOwnerIntelligence, type OwnerQuestion } from "../api/businessEconomics";
...
export function useEconomicsPolicyAdministration(start: string, end: string, enabled = true) { return useQuery({ queryKey: ["business-economics", "policy-administration", start, end], queryFn: () => getEconomicsPolicyAdministration(start, end), enabled }); }
export function useEconomicsResultLineage(resultId: string | null, enabled = true) { return useQuery({ queryKey: ["business-economics", "result-lineage", resultId], queryFn: () => getEconomicsResultLineage(resultId!), enabled: enabled && Boolean(resultId) }); }
