...
import type { EligibleCapacityWorker } from "./types";
...
export function EngineeringCapacityPanel() {
  ...
  return (
    <section aria-labelledby="engineering-capacity-heading" className="space-y-ui-4">
      ...
      {data.active_allocations.filter((item) => item.status === "active").length > 0 && <Card><h3 className="font-semibold">Active allocations</h3>{data.active_allocations.filter((item) => item.status === "active").map((item) => <div key={item.id} className="mt-3 flex flex-col gap-2 rounded-lg border border-stroke p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{item.machine_label}</p><p className="text-xs text-content-muted">Running · capacity allocated</p></div><Button className="min-h-11" variant="outline" onClick={() => releaseAllocationMutation.mutate(item)}>Release capacity</Button></div>)}</Card>}

      {data.active_reservations.length > 0 && <Card><h3 className="font-semibold">Reservations</h3>{data.active_reservations.map((item) => <div key={item.id} className="mt-3 flex flex-col gap-2 rounded-lg border border-stroke p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{item.machine_label}</p><p className="text-xs text-content-muted">{mobileEngineeringLabel(item.status)}</p></div>{item.status === "active" && <Button className="min-h-11" variant="outline" onClick={() => releaseReservationMutation.mutate(item)}>Release reservation</Button>}</div>)}</Card>}

      {data.active_allocations.filter((item) => item.status === "reconciliation_required").map((allocation) => <Alert key={allocation.id} variant="warning" title="Reconciliation required"><p>{allocation.machine_label} has an ambiguous assignment. Capacity remains held.</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><Button className="min-h-11" onClick={() => reconcileMutation.mutate({ allocation, resolution: "confirmed_active" })}>Still active</Button><Button className="min-h-11" variant="outline" onClick={() => reconcileMutation.mutate({ allocation, resolution: "confirmed_released" })}>Confirmed released</Button></div></Alert>)}

      {data.waiting_workstreams.length > 0 && <Card><div className="flex items-center justify-between gap-2"><h3 className="font-semibold">Waiting for capacity</h3><Badge>{data.waiting_workstreams.length}</Badge></div><div className="mt-3 space-y-3">{data.waiting_workstreams.slice(0, showFullQueue ? undefined : 5).map((item) => <div key={item.command_id} className="rounded-lg border border-stroke p-3"><p className="font-semibold">{item.ecid}</p><p className="text-sm text-content-muted">{item.reason}</p><Button className="mt-2 min-h-11 w-full sm:w-auto" variant="outline" disabled={item.decision !== "capacity_available" || reserveMutation.isPending} onClick={() => reserveMutation.mutate(item.command_id)}>Reserve capacity</Button></div>)}</div>{data.waiting_workstreams.length > 5 && <Button className="mt-3 min-h-11 w-full" variant="ghost" onClick={() => setShowFullQueue((value) => !value)}>{showFullQueue ? "Show fewer waiting workstreams" : `Show all ${data.waiting_workstreams.length} waiting workstreams`}</Button>}</Card>}
    </section>
  );
}
