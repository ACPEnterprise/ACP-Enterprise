...
describe("AppointmentDetailRoute", () => {
  beforeEach(() => {
    ...
    vi.mocked(useDispatchAssignment).mockReturnValue({ isLoading: false, data: undefined } as never);
  ...
  it("shows the Dispatch-authoritative technician on Appointment detail", () => {
    permissions.add("COMPANY_DISPATCH_READ");
    vi.mocked(useJobForAppointment).mockReturnValue({ isLoading: false, isError: false, data: { items: [{ id: "job-1", job_number: "JOB-000001", status: "ready" }] } } as never);
    vi.mocked(useDispatchAssignment).mockReturnValue({ isLoading: false, data: { primary_employee_name: "Melvin Santiago", status: "assigned" } } as never);
    renderRoute();
    expect(screen.getByText("Melvin Santiago")).toBeVisible();
    expect(screen.getByText("assigned")).toBeVisible();
    expect(useDispatchAssignment).toHaveBeenCalledWith("appointment-1", true);
  });
