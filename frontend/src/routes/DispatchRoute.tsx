...
export function DispatchRoute() {
  ...
  return <div className="min-w-0 space-y-6"><header><p className="text-sm font-medium text-action-primary">Operations</p><h2 className="mt-1 text-2xl font-bold sm:text-3xl">Dispatch</h2><p className="mt-2 text-content-muted">Review the selected day’s scheduled work and Jobs requiring operational attention.</p></header><Alert variant="information" title="Technician assignment unavailable">Scheduling currently records Appointment timing and capacity, but it does not yet expose an authoritative employee or technician assignment. Scheduled and unscheduled work remain truthful below.</Alert><DispatchScopeControls date={date} branchId={branchId} branches={activeCompany.branches} onDateChange={changeDate} onBranchChange={changeBranch} /><DispatchSummary appointmentTotal={appointments.data?.total_count ?? 0} jobTotal={jobs.data?.total_count ?? 0} visibleJobs={jobs.data?.items ?? []} /><DispatchWorkspaceLayout appointments={<DispatchAppointmentsQueue appointments={appointments.data?.items} loading={appointments.isLoading} error={appointments.error} onRetry={() => void appointments.refetch()} page={appointmentPage} totalPages={appointmentTotalPages} onPageChange={setAppointmentPage} />} jobs={<DispatchJobsQueue jobs={jobs.data?.items} loading={jobs.isLoading} error={jobs.error} onRetry={() => void jobs.refetch()} page={jobPage} totalPages={jobs.data?.total_pages ?? 0} onPageChange={setJobPage} />} /></div>;
}
