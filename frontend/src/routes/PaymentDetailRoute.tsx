...
export function PaymentDetailRoute() {
  const { receiptId = "" } = useParams(); const canRead = useHasPermission("COMPANY_PAYMENT_READ"); const query = usePayment(receiptId, canRead); const mutations = usePaymentMutations();
  ...
  if (!canRead) return <Alert variant="danger">You are not authorized to view this Payment.</Alert>; if (query.isPending) return <Spinner label="Loading payment" />; if (!query.data || query.isError) return <Alert variant="danger">Payment could not be loaded.</Alert>; const receipt = query.data;
