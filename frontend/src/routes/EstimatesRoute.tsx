...
import { useCommercialHistory, useCommercialPolicies, useCommercialReport, useEstimate, useEstimateFollowUps, useEstimateMutations, useEstimates } from "../hooks/useEstimates";
...
import { CommercialReadiness } from "../components/estimates/CommercialReadiness";
import { CommercialEngagementControls, CommercialSummary, CommercialTimeline, FollowUpQueue } from "../components/estimates/CommercialOperations";
...
export function EstimatesRoute() {
  ...
  const policies = useCommercialPolicies(canRead);
  const followUps = useEstimateFollowUps(canRead);
  const report = useCommercialReport(canRead);
  const history = useCommercialHistory(id, canRead);
  const [lookup, setLookup] = useState(id);
  const [form, setForm] = useState({ branch: "", customer: "", location: "", item: "", quantity: "1", title: "", description: "", discountType: "", discountValue: "" });
  const customers = useCustomerList("", 100, 0, canManage);
  const customer = useCustomerDetail(form.customer || null);
  const priceBook = usePriceBook(form.branch || undefined, canManage && Boolean(form.branch));
  ...
  const submit = async (event: FormEvent) => {
    ...
    const item = priceBook.data?.service_items.find((candidate) => candidate.id === form.item);
    if (!item) return;
    const snapshot = await priceBookMutations.snapshot.mutateAsync({ itemId: item.id, data: { branch_id: form.branch, quantity: form.quantity, currency: "USD", effective_at: new Date().toISOString(), idempotency_key: `estimate-snapshot:${crypto.randomUUID()}` } });
    const created = await mutations.create.mutateAsync({
      branch_id: form.branch,
      customer_id: form.customer,
      service_location_id: form.location || undefined,
      proposal_title: form.title,
      lines: [{ snapshot_id: snapshot.id, title: item.name, description: form.description || item.customer_description }],
  ...
  return <div className="mx-auto max-w-5xl space-y-6 pb-12">
    ...
    <CommercialReadiness branches={activeCompany?.branches ?? []} policies={policies.data ?? []} canManage={canManage} pending={mutations.configurePolicy.isPending} onSave={(input) => mutations.configurePolicy.mutateAsync(input)} />
    <CommercialSummary report={report.data} />
    <FollowUpQueue items={followUps.data ?? []} />
    ...
    {id && (estimate.isPending ? <Spinner label="Loading Estimate" /> : estimate.isError ? <Alert variant="danger">Estimate could not be loaded.</Alert> : estimate.data && <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><CardTitle>{estimate.data.current_revision.proposal_title}</CardTitle><div className="flex gap-2"><Badge variant="neutral">{estimate.data.status}</Badge><Button variant="secondary" onClick={() => void openArtifact(estimate.data.id)}>{estimate.data.status === "draft" ? "Preview document" : "Print estimate"}</Button></div></div><CardDescription>{estimate.data.estimate_number} · Revision {estimate.data.current_revision.revision_number}</CardDescription></CardHeader><CardContent className="space-y-5"><ul className="space-y-3">{estimate.data.current_revision.lines.map((line) => <li key={line.id} className="rounded-lg border border-stroke p-4"><div className="flex justify-between gap-4"><div><strong>{line.title}</strong>{line.description && <p className="text-sm text-content-muted">{line.description}</p>}{line.option_id && <p className="text-xs text-content-muted">Selected customer option</p>}</div><span>{money(line.line_total, line.currency)}</span></div></li>)}</ul><dl className="ml-auto grid max-w-sm grid-cols-2 gap-2 text-right"><dt>Subtotal</dt><dd>{money(estimate.data.current_revision.subtotal_amount)}</dd><dt>Discount</dt><dd>−{money(estimate.data.current_revision.discount_amount)}</dd><dt>Tax</dt><dd>{money(estimate.data.current_revision.tax_amount)}</dd><dt className="font-bold">Total</dt><dd className="font-bold">{money(estimate.data.current_revision.total_amount)}</dd></dl>{canManage && <><EstimateDecisionControls estimate={estimate.data} mutations={mutations} />{user && <CommercialEngagementControls estimate={estimate.data} userId={user.id} mutations={mutations} />}</>}<CommercialTimeline items={history.data ?? []} /></CardContent></Card>)}
    {canManage && <Card><CardHeader><CardTitle>Create proposal</CardTitle><CardDescription>Choose Customer, service location, and active Price Book evidence; ACP creates the immutable commercial snapshot.</CardDescription></CardHeader><CardContent><form className="grid gap-3 sm:grid-cols-2" onSubmit={(event) => void submit(event)}><Select aria-label="Branch" value={form.branch} onChange={(event) => setForm({ ...form, branch: event.target.value, item: "" })} required><option value="">Select Branch</option>{activeCompany?.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</Select><Select aria-label="Customer" value={form.customer} onChange={(event) => setForm({ ...form, customer: event.target.value, location: "" })} required><option value="">Select Customer</option>{customers.data?.items.map((item) => <option key={item.id} value={item.id}>{item.business_name || [item.first_name, item.last_name].filter(Boolean).join(" ")}</option>)}</Select><Select aria-label="Service location" value={form.location} onChange={(event) => setForm({ ...form, location: event.target.value })}><option value="">No service location</option>{customer.data?.properties.map((property) => <option key={property.id} value={property.id}>{property.address_line_1}, {property.city}</option>)}</Select><Select aria-label="Price Book item" value={form.item} onChange={(event) => { const item = priceBook.data?.service_items.find((candidate) => candidate.id === event.target.value); setForm({ ...form, item: event.target.value, title: form.title || item?.name || "" }); }} required><option value="">Select active service</option>{priceBook.data?.service_items.filter((item) => item.status === "active" && item.current_version_id).map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</Select><Input aria-label="Quantity" type="number" min="0.01" step="0.01" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value })} required /><Input aria-label="Proposal title" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} required /><Input aria-label="Scope description" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /><Select aria-label="Discount type" value={form.discountType} onChange={(event) => setForm({ ...form, discountType: event.target.value })}><option value="">No discount</option><option value="fixed">Fixed amount</option><option value="percentage">Percentage</option></Select><Input aria-label="Discount value" type="number" min="0" step="0.01" disabled={!form.discountType} value={form.discountValue} onChange={(event) => setForm({ ...form, discountValue: event.target.value })} /><Button fullWidth type="submit" loading={mutations.create.isPending || priceBookMutations.snapshot.isPending}>Create immutable revision</Button></form></CardContent></Card>}
  </div>;
}
