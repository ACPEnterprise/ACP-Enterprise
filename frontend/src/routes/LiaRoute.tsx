...
export function LiaRoute() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 pb-28 sm:pb-12">
    ...
    {readiness.isPending ? <Spinner label="Checking LIA readiness" /> : readiness.isError || !readiness.data ? <Alert variant="danger" title="LIA readiness unavailable"><div className="space-y-3"><p>No readiness state was inferred.</p><Button variant="secondary" onClick={() => void readiness.refetch()}>Retry readiness</Button></div></Alert> : <Alert variant={readiness.data.provider_state === "AI_PROVIDER_NOT_CONFIGURED" ? "warning" : "success"} title={readiness.data.state.replaceAll("_", " ")}><span className="inline-flex items-center gap-2"><ShieldCheck className="size-4" />Deterministic authorized retrieval is ready. {readiness.data.provider_state === "AI_PROVIDER_NOT_CONFIGURED" ? "Generative explanations await a separately configured provider." : "The configured provider is available."}</span></Alert>}
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="size-5" />Owner briefing</CardTitle><CardDescription>Current evidence across the domains you are permitted to read.</CardDescription></CardHeader><CardContent>{briefing.isPending ? <Spinner label="Preparing briefing" /> : briefing.isError || !briefing.data ? <Alert variant="danger"><div className="space-y-3"><p>The briefing could not be verified. No summary was fabricated.</p><Button variant="secondary" onClick={() => void briefing.refetch()}>Retry briefing</Button></div></Alert> : <Answer result={briefing.data} />}</CardContent></Card>
