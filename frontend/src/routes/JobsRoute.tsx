...
export function JobsRoute() {
  ...
  return <div className="space-y-6"><header><p className="text-sm font-medium text-blue-400">Operations</p><h2 className="mt-1 text-3xl font-bold">Jobs</h2><p className="mt-2 text-slate-400">Manage work from creation through completion.</p></header>
    <form onSubmit={submit} className="grid gap-3 rounded-xl border border-stroke bg-surface p-ui-4 md:grid-cols-4">
      ...
      <Input aria-label="Branch ID" value={branchId} onChange={(event) => { setBranchId(event.target.value); setPage(1); }} placeholder="Branch ID" />
