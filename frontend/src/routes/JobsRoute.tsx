...
export function JobsRoute() {
  ...
  const hasCustomerContext = Boolean(initialCustomerId);
  ...
  const query = useJobs({ searchText, status: status ? [status] : undefined, priority: priority ? [priority] : undefined, jobType: jobType ? [jobType] : undefined, branchId: branchId || undefined, customerId: initialCustomerId || undefined, serviceLocationId: initialLocationId || undefined, page, pageSize: 20, sortField, sortDirection }, canRead);
  ...
  return <div className="space-y-6"><header className="flex flex-col items-stretch gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-action-primary">Operations</p><h2 className="mt-1 text-2xl font-bold sm:text-3xl">Jobs</h2><p className="mt-2 text-content-muted">Manage work from creation through completion.</p></div>{canManage && <Button leadingIcon={<Plus size={18} />} onClick={() => setCreating(true)}>Create Job</Button>}</header>
    ...
    {hasCustomerContext && <Alert title="Customer work context"><span>Showing only Jobs for the Customer{initialLocationId ? " and Service Location" : ""} selected from Customer detail.</span> <Link className="font-semibold text-action-primary underline" to={`/customers/${encodeURIComponent(initialCustomerId)}`}>Return to Customer</Link> <Link className="font-semibold text-action-primary underline" to="/jobs">Show all Jobs</Link></Alert>}
    <form onSubmit={submit} className="grid gap-3 rounded-xl border border-stroke bg-surface p-ui-4 md:grid-cols-4">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto]"><Select aria-label="Sort direction" value={sortDirection} onChange={(event) => setSortDirection(event.target.value as SortDirection)}><option value="desc">Descending</option><option value="asc">Ascending</option></Select><Button type="submit">Search</Button></div>
    </form>
    <section className="rounded-xl border border-stroke bg-surface">{query.isLoading && <JobsLoadingState />}{query.isError && <div className="p-ui-5"><JobsErrorState error={query.error} onRetry={() => void query.refetch()} /></div>}{query.data?.items.length === 0 && <div className="p-ui-5"><JobsEmptyState /></div>}{query.data && query.data.items.length > 0 && <JobsTable jobs={query.data.items} />}{query.data && query.data.total_pages > 0 && <footer className="flex flex-col gap-3 border-t border-stroke p-ui-4 text-sm text-content-muted sm:flex-row sm:items-center sm:justify-between"><span>Page {query.data.page} of {query.data.total_pages} · {query.data.total_count} Jobs</span><div className="grid grid-cols-2 gap-2 sm:flex"><Button variant="outline" aria-label="Previous page" disabled={page === 1} onClick={() => setPage((value) => value - 1)}><ChevronLeft size={17} /></Button><Button variant="outline" aria-label="Next page" disabled={page >= query.data.total_pages} onClick={() => setPage((value) => value + 1)}><ChevronRight size={17} /></Button></div></footer>}</section>
  </div>;
}
