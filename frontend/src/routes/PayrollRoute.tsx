...
export function PayrollRoute() {
  ...
  if (operations.isError || reports.isError || schemas.isError || registers.isError || !operations.data) return <Alert variant="danger" title="Payroll Administration unavailable">Authoritative Payroll readiness could not be loaded. No Payroll action was taken.</Alert>;
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-12">
    ...
    <Card><CardHeader><CardTitle>Operating payroll register</CardTitle><CardDescription>Accepted evidence and exact blockers. Approval does not move money, file taxes, or post Accounting.</CardDescription></CardHeader><CardContent>{registers.data?.length ? <div className="space-y-5">{registers.data.map((register) => <section key={register.run_id} className="space-y-3"><h3 className="font-semibold">{register.period_start} – {register.period_end} · {label(register.lifecycle)} / {label(register.review_state)}</h3><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left"><th>Employee</th><th>Accepted</th><th>Regular / OT</th><th>Gross</th><th>Withholding</th><th>Deductions</th><th>Net</th><th>Employer liability</th><th>Status / exceptions</th></tr></thead><tbody>{register.members.map((member) => <tr className="border-b border-stroke" key={member.employee_id}><td className="py-3">{member.employee_name}<br/><span className="text-content-muted">{member.employee_number}</span></td><td>{member.accepted_minutes === null ? "Unavailable" : `${member.accepted_minutes / 60} h`}</td><td>{member.regular_minutes === null ? "Unavailable" : `${member.regular_minutes / 60} / ${(member.overtime_minutes ?? 0) / 60} h`}</td><td>{member.gross ?? "—"}</td><td>{member.employee_taxes ?? "—"}</td><td>{member.deductions ?? "—"}</td><td>{member.net_pay ?? "—"}</td><td>{member.employer_liabilities ?? "—"}</td><td>{label(member.status)}{member.blockers.length ? <ul>{member.blockers.map((blocker) => <li key={blocker}>{label(blocker)}</li>)}</ul> : null}</td></tr>)}</tbody></table></div><p className="text-sm text-content-muted">Period liabilities: employee withholding {register.liability_totals.employee_taxes} · deductions {register.liability_totals.employee_deductions} · employer liabilities {register.liability_totals.employer_liabilities}. Manual filing/payment required.</p></section>)}</div> : <p className="text-content-muted">No operating Payroll register has been assembled. Missing real Employee inputs remain unaccepted—not zero.</p>}</CardContent></Card>
