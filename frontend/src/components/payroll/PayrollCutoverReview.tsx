...
export function PayrollCutoverReview() {
  ...
  const bridgeCertification = useMutation({ mutationFn: ({ periodId, role, version }: { periodId: string; role: "owner" | "accountant"; version: number }) => certifyBridgePeriod(periodId, { certifier_role: role, expected_version: version, idempotency_key: crypto.randomUUID() }), onSuccess: refresh });
  ...
  if (!review.data.review) return <Card><CardHeader><CardTitle>Cutover Review</CardTitle><CardDescription>Candidate evidence remains non-authoritative until certified.</CardDescription></CardHeader><CardContent><Button onClick={() => start.mutate({ idempotency_key: crypto.randomUUID() })}>Start protected review</Button></CardContent></Card>;
  ...
  return <section className="space-y-4" aria-labelledby="cutover-title">
    ...
    <Card id="manual-bridge-payroll"><CardHeader><CardTitle>Record reviewed manual payroll</CardTitle><CardDescription>Externally calculated / manually paid evidence. This does not create a native ACP Payroll run.</CardDescription></CardHeader><CardContent className="space-y-4"><form className="grid gap-3 md:grid-cols-2" onSubmit={submitBridge}><label>Period start<input className="block w-full" name="period_start" type="date" required /></label><label>Period end<input className="block w-full" name="period_end" type="date" required /></label><label>Pay/check date<input className="block w-full" name="pay_date" type="date" required /></label><label>Source/document reference<input className="block w-full" name="source_reference" required /></label><Button disabled={bridge.isPending} type="submit">Save reviewed payroll draft</Button></form>
