...
export type QueueAssignmentFilter =
  "needs_attention" | "scheduled_unassigned" | "assigned" | "all";
...
const priorities: readonly JobPriority[] = [
  "low",
  "normal",
  "high",
  "urgent",
...
const activeAssignment = (item?: DispatchBoardItem) =>
  item?.assignment &&
...
const classification = (
  ...
  if (appointment.status === "draft" || !appointment.arrival_window_start_at)
...
const attentionReason = (state: string, partial: boolean) =>
  partial
    ? "Customer, Job, or Dispatch context is incomplete"
    : state === "NEEDS_SCHEDULING"
      ? "No authoritative appointment window"
      : state === "SCHEDULED_UNASSIGNED"
        ? "Appointment has no active technician assignment"
        : "Assigned; included for comparison";
const readiness = (state: string, partial: boolean) =>
  partial
    ? "CONTEXT_REQUIRED"
    : state === "NEEDS_SCHEDULING"
      ? "READY_TO_SCHEDULE"
...
export function NeedsSchedulingQueue(props: NeedsSchedulingQueueProps) {
  const branchName = (id: string) =>
    props.branches.find((branch) => branch.id === id)?.name ??
    "Accessible Branch";
  ...
  const matchesJob = (job?: JobListItem) =>
    Boolean(
      ...
      (!props.serviceCategory || job.job_type_code === props.serviceCategory) &&
      (!search ||
  ...
  const unscheduled = (
    props.assignmentFilter === "needs_attention" ||
  ...
  const compare = (
    left: { created: string; priority?: JobPriority },
    right: { created: string; priority?: JobPriority },
  ) => {
  ...
  unscheduled.sort((a, b) =>
    compare(
      { created: a.timestamp, priority: a.job.priority },
      { created: b.timestamp, priority: b.job.priority },
    ),
  );
  projected.sort((a, b) =>
    compare(
      { created: a.created, priority: a.job?.priority },
      { created: b.created, priority: b.job?.priority },
    ),
  );
  const count = unscheduled.length + projected.length;

  return (
    <Card className="min-w-0 p-4" aria-label="Needs scheduling work queue">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold">Needs Scheduling work queue</h2>
          <p className="text-sm text-content-muted">
            Native Jobs and Appointments in the current authorized Branch, date,
            and filter scope.
          </p>
        </div>
        <Badge>{count}</Badge>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <label className="text-sm font-medium">
          ...
          <Select
            ...
            onChange={(event) =>
              props.onAssignmentFilterChange(
                event.target.value as QueueAssignmentFilter,
              )
            }
          >
            <option value="needs_attention">Needs attention</option>
            <option value="scheduled_unassigned">Scheduled, unassigned</option>
            <option value="assigned">Assigned</option>
            <option value="all">All projected work</option>
          </Select>
        </label>
        <label className="text-sm font-medium">
          Job status
          <Select
            className="mt-1"
            aria-label="Queue Job status"
            value={props.jobStatus}
            onChange={(event) =>
              props.onJobStatusChange(event.target.value as JobStatus | "")
            }
          >
            <option value="">All open Job statuses</option>
        ...
        <label className="text-sm font-medium">
          ...
          <Select
            ...
            onChange={(event) =>
              props.onPriorityChange(event.target.value as JobPriority | "")
            }
          >
        ...
        <label className="text-sm font-medium">
          ...
          <Select
            ...
            value={props.sort}
            onChange={(event) =>
              props.onSortChange(event.target.value as QueueSort)
            }
          >
            <option value="oldest">Oldest created first</option>
            <option value="newest">Newest created first</option>
            <option value="priority">Explicit priority, then oldest</option>
          </Select>
        </label>
      </div>
      ...
      <div className="mt-5 space-y-3">
        {projected.map(({ appointment, dispatch, job, state, created }) => {
          ...
          return (
            <article
              className="rounded-xl border border-stroke p-4"
              key={appointment.id}
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {job?.priority === "emergency" && <Badge>EMERGENCY</Badge>}
                  </div>
                  <h3 className="mt-2 font-semibold">
                    {job?.job_number ?? appointment.appointment_number}
                  </h3>
                  <p className="text-sm">
                    {job?.customer_display_name ??
                      "Customer/Job context unavailable"}
                  </p>
                  <p className="text-sm text-content-muted">
                ...
                <div className="text-right text-sm">
                  <p>{branchName(appointment.branch_id)}</p>
              ...
              <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  <dt className="text-content-muted">Job status</dt>
                  <dd>{job?.status?.replaceAll("_", " ") ?? "Unavailable"}</dd>
                </div>
                <div>
                  <dt className="text-content-muted">Arrival window</dt>
                  <dd>
                    {dateTime(appointment.arrival_window_start_at)} –{" "}
                ...
                <div>
                  <dt className="text-content-muted">Attention reason</dt>
                  <dd>{attentionReason(state, partial)}</dd>
                </div>
                <div>
                  <dt className="text-content-muted">Readiness</dt>
                  <dd>{readiness(state, partial)}</dd>
                </div>
                <div>
                  <dt className="text-content-muted">Service category</dt>
                  <dd>
                    {job?.job_type_code?.replaceAll("_", " ") ??
                      "Not established"}
                  </dd>
                </div>
                <div>
              ...
              {partial && (
                <p className="mt-3 text-sm text-status-warning">
                  Some Customer, Job, or Dispatch context is unavailable.
                  Appointment timing remains authoritative.
                </p>
              )}
              <div className="mt-4 flex flex-wrap gap-2">
                <Button
                  variant="outline"
                  onClick={() => props.onSelect(appointment)}
                >
                  {state === "ASSIGNED"
                    ? "Inspect assignment"
                ...
                <Link
                  className="inline-flex min-h-11 items-center rounded-lg border border-stroke px-3 font-semibold"
                  to={withSchedulingReturn(
                    appointmentDetailPath(appointment.id),
                    props.returnTo,
                  )}
                >
                  Open Appointment
                ...
                {job && (
                  <>
                    ...
                    <Link
                      className="inline-flex min-h-11 items-center rounded-lg border border-stroke px-3 font-semibold"
                      to={withSchedulingReturn(
                        customerDetailPath(job.customer_id),
                        props.returnTo,
                      )}
                    >
                      Open Customer
        ...
        {unscheduled.map(({ job }) => (
          <article className="rounded-xl border border-stroke p-4" key={job.id}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="flex flex-wrap gap-2">
                  <Badge>NEEDS_SCHEDULING</Badge>
                  {job.priority === "emergency" && <Badge>EMERGENCY</Badge>}
                </div>
                <h3 className="mt-2 font-semibold">{job.job_number}</h3>
                <p className="text-sm">{job.customer_display_name}</p>
                <p className="text-sm text-content-muted">
                  {job.service_location_label}
                </p>
              </div>
              <div className="text-right text-sm">
                <p>{branchName(job.branch_id)}</p>
                <p className="text-content-muted">
                  {job.created_at
            ...
            <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                ...
                <dd>{job.priority ?? "Unavailable"}</dd>
              </div>
              <div>
                <dt className="text-content-muted">Requested date/window</dt>
                <dd>Not established</dd>
              </div>
              <div>
                <dt className="text-content-muted">Age</dt>
                <dd>{age(job.created_at)}</dd>
              </div>
              <div>
                <dt className="text-content-muted">Attention reason</dt>
                <dd>No authoritative Appointment exists</dd>
              </div>
              <div>
                <dt className="text-content-muted">Readiness</dt>
                <dd>
                  {job.customer_id && job.service_location_id
                    ? "READY_TO_SCHEDULE"
                    : "CONTEXT_REQUIRED"}
                </dd>
              </div>
              <div>
                <dt className="text-content-muted">Service category</dt>
                <dd>
                  {job.job_type_code?.replaceAll("_", " ") ?? "Not established"}
                </dd>
              </div>
            </dl>
            <div className="mt-4 flex flex-wrap gap-2">
              <Link
                className="inline-flex min-h-11 items-center rounded-lg bg-action-primary px-3 font-semibold text-white"
                to={withSchedulingReturn(jobDetailPath(job.id), props.returnTo)}
              >
                Open Job to schedule
              </Link>
              {job.customer_id && (
        ...
        {!count && (
          <p className="rounded-xl border border-dashed border-stroke p-5 text-sm text-content-muted">
            No work matches this authorized queue scope. Change a filter or
            date; no missing value was treated as completed or assigned.
          </p>
        )}
      </div>
    </Card>
