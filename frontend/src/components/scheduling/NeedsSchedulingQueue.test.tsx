...
const job = (
  ...
  service_location_label: `${id} Main Street`,
  status: "ready",
  priority: "normal",
  job_type_code: "service",
  customer_reported_problem_summary: null,
  appointment_count: 0,
...
const appointment = (
  ...
  service_location_id: `location-${id}`,
  status: "scheduled",
  arrival_window_start_at: "2026-09-15T13:00:00Z",
...
const dispatch = (
  ...
  branch_id: "branch-1",
  status: "scheduled",
  window_start_at: "2026-09-15T13:00:00Z",
  window_end_at: "2026-09-15T16:00:00Z",
  assignment: assigned
    ? {
        ...
        primary_employee_name: "Casey Technician",
        status: "assigned",
        arrival_state: "pending",
        active_exception_code: null,
        assignment_reason: "human_dispatch",
        window_start_at: "2026-09-15T13:00:00Z",
        window_end_at: "2026-09-15T16:00:00Z",
...
function renderQueue(
  ...
  const jobs = [
    ...
    job("unassigned", {
      earliest_appointment_start_at: "2026-09-15T13:00:00Z",
      appointment_count: 1,
    }),
    job("assigned", {
      earliest_appointment_start_at: "2026-09-15T14:00:00Z",
      appointment_count: 1,
    }),
  ];
  const appointments = [
  ...
  const dispatches = [
    ...
    dispatch("assigned", "assigned", true),
  ];
    ...
    <MemoryRouter>
      <NeedsSchedulingQueue
        ...
        dispatchByAppointment={
          new Map(dispatches.map((item) => [item.appointment_id, item]))
        }
        ...
        assignmentFilter={options.assignment ?? "needs_attention"}
...
describe("NeedsSchedulingQueue", () => {
  it("shows unscheduled, scheduled-unassigned, emergency, and partial truth without assigned noise", () => {
    renderQueue();
    const queue = screen.getByRole("article", {
      name: "Needs scheduling work queue",
    });
    ...
    expect(
      within(queue).queryByText("Casey Technician"),
    ).not.toBeInTheDocument();
    expect(within(queue).getAllByText("Not established").length).toBeGreaterThan(0);
    expect(within(queue).getByText("No authoritative Appointment exists")).toBeVisible();
    expect(within(queue).getByText("READY_TO_SCHEDULE")).toBeVisible();
  });
  ...
  it("exposes assigned comparison and selects only through explicit human action", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Inspect assignment" }),
    );
  ...
  it("preserves queue context in Customer, Location, Job, and Appointment links", () => {
    renderQueue();
    expect(
      screen.getAllByRole("link", { name: "Open Job to schedule" })[0],
    ).toHaveAttribute(
      "href",
      expect.stringContaining(
        "returnTo=%2Fscheduling%3Fview%3Dunassigned%26branch%3Dbranch-1",
      ),
    );
    expect(
      screen.getAllByRole("link", { name: "Open Location" })[0],
    ).toHaveAttribute(
      "href",
      expect.stringMatching(/returnTo=.*#service-location-/),
    );
    expect(
      screen.getAllByRole("link", { name: "Open Customer" })[0],
    ).toHaveAttribute("href", expect.stringContaining("returnTo="));
    expect(
      screen.getAllByRole("link", { name: "Open Appointment" })[0],
    ).toHaveAttribute("href", expect.stringContaining("returnTo="));
  });
  ...
  it("applies authoritative Job priority and search filters and shows a truthful empty state", () => {
    ...
    expect(
      screen.getByText(/No work matches this authorized queue scope/),
    ).toBeVisible();
    expect(screen.queryByText("JOB-emergency")).not.toBeInTheDocument();
  });
});
