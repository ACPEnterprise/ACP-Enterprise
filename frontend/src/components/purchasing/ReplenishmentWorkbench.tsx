...
export function ReplenishmentWorkbench({ canApprove, pending, decisionPending, result, error, decisionError, decisionSucceeded, onRun, onDecision }: Props) {
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        {decisionSucceeded && <Alert variant="success">The disposition was recorded and reconciled to server truth.</Alert>}
            ...
            {canApprove ? (
              <>
                <div className="mt-2 grid gap-2 sm:grid-cols-5">
                  <Input disabled={decisionPending} aria-label="Replenishment Vendor ID" placeholder="Vendor ID" value={vendorId} onChange={(e) => setVendorId(e.target.value)} />
                  <Input disabled={decisionPending} aria-label="Replenishment PO number" placeholder="PO number" value={poNumber} onChange={(e) => setPoNumber(e.target.value)} />
                  <Input disabled={decisionPending} aria-label="Approved quantity" type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} />
                  <Input disabled={decisionPending} aria-label="Approved unit cost" type="number" value={unitCost} onChange={(e) => setUnitCost(e.target.value)} />
                  <Input disabled={decisionPending} aria-label="Decision reason" value={reason} onChange={(e) => setReason(e.target.value)} />
                </div>
                <div className="mt-2 flex gap-2"><Button loading={decisionPending} disabled={decisionPending || !vendorId || !poNumber || !quantity || !reason} onClick={() => void onDecision("approved", reason, vendorId, poNumber, quantity, unitCost).catch(() => undefined)}>Approve and create draft PO</Button><Button disabled={decisionPending || !reason} onClick={() => void onDecision("rejected", reason, "", "", "", "0").catch(() => undefined)}>Reject</Button></div>
              </>
            ) : (
              <p className="mt-2 text-content-muted">Recommendation evidence is read-only. Replenishment approval authority is required to record a disposition.</p>
            )}
          </div>
        ))}
      </CardContent>
