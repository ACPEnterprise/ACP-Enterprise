  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-ui-4 sm:p-ui-6">
        ...
        {timeline.data && timeline.data.total_pages > 1 && <div className="mt-4 flex flex-col gap-3 border-t border-stroke pt-4 sm:flex-row sm:items-center sm:justify-between"><span className="text-sm text-content-muted">History page {timeline.data.page} of {timeline.data.total_pages}</span><div className="grid grid-cols-1 gap-2 sm:flex"><Button type="button" variant="outline" disabled={timelinePage === 1} onClick={() => setTimelineCursor({ customerId, page: Math.max(1, timelinePage - 1) })} leadingIcon={<ChevronLeft size={16} />}>Previous history</Button><Button type="button" variant="outline" disabled={timelinePage >= timeline.data.total_pages} onClick={() => setTimelineCursor({ customerId, page: timelinePage + 1 })} trailingIcon={<ChevronRight size={16} />}>Next history</Button></div></div>}
      </Card>
