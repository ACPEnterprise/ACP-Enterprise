...
export function CustomerManagement() {
  ...
  return <div className="space-y-6">
    ...
    {canManage && <Card className="p-ui-4 sm:p-ui-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-action-primary">Customer source reconciliation</p><h3 className="mt-1 text-lg font-semibold">Refresh accepted HCP population</h3><p className="mt-2 max-w-3xl text-sm text-content-muted">Classify accepted source identities as bound, held, ambiguous, or unexplained. This action does not create Customers, bind identities, or admit source records.</p></div><Button type="button" variant="outline" loading={populationRefresh.isPending} onClick={() => populationRefresh.mutate()}>Refresh source classification</Button></div>{populationRefresh.isError && (() => { const error = getOperatorApiError(populationRefresh.error, "Customer source reconciliation"); return <Alert className="mt-4" variant="danger" title={error.title}>{error.message}</Alert>; })()}{populationRefresh.data && <Alert className="mt-4" variant={populationRefresh.data.counts.ambiguous || populationRefresh.data.counts.unexplained ? "warning" : "success"} title="Source classification refreshed">{populationRefresh.data.counts.total.toLocaleString()} accepted source Customers · {populationRefresh.data.counts.bound.toLocaleString()} bound · {populationRefresh.data.counts.held.toLocaleString()} held · {populationRefresh.data.counts.ambiguous.toLocaleString()} ambiguous · {populationRefresh.data.counts.unexplained.toLocaleString()} unexplained. No Customer admission was performed.</Alert>}</Card>}
