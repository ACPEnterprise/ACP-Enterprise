...
export function CustomerManagement() {
  ...
  const [recordState, setRecordState] = useState<"current" | "archived" | "all">("current");
  ...
  const populationRefresh = useCustomerPopulationRefresh();
  const cleanMajorityAdmission = useCustomerCleanMajorityAdmission();
  ...
  const sourceProjectionPartial = sourceCounts.some((item) =>
    item.migrated !== item.source
    || item.held > 0
    || item.exception > 0
    || item.deferred > 0
    || item.unresolved > 0
  ...
  const clearFilters = () => {
    setSearchInput(""); setSearch(""); setStatus(""); setCustomerType(""); setRecordState("current"); setPage(1);
  };
  ...
  return <div className="space-y-6">
    ...
    {canReviewSource && <Card className="p-ui-4 sm:p-ui-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-action-primary">Migration source accounting</p><h3 className="mt-1 text-lg font-semibold">Population availability</h3></div>{migration.data && <Badge variant={migration.data.stale ? "warning" : "neutral"}>{migration.data.stale ? "Stale source projection" : migration.data.overall_status.replaceAll("_", " ")}</Badge>}</div>{migration.isLoading && <div className="mt-4"><Spinner label="Loading source accounting" /></div>}{migration.isError && <div className="mt-4"><Alert variant="warning" title="Source accounting unavailable" action={<Button type="button" variant="outline" onClick={() => void migration.refetch()}>Retry source accounting</Button>}>The native roster remains usable, but population completeness and source freshness are unverified.</Alert></div>}{migration.data && <><p className="mt-2 text-sm text-content-muted">Source evidence through {migration.data.historical_window.ends_on || "an unavailable date"}. Source totals reconcile only when each row’s delta is zero.</p>{migration.data.stale && <Alert className="mt-4" variant="warning" title="Source projection is stale">Use the admitted native roster for current ACP work, but do not treat this source population or as-of date as current completeness evidence.</Alert>}{sourceProjectionPartial && <Alert className="mt-4" variant="information" title="Source population is partial">Only admitted native records appear in the roster. Held, exception, deferred, unresolved, and not-yet-admitted records remain outside Customer authority.</Alert>}<div className="mt-4 grid gap-3 sm:grid-cols-3">{sourceCounts.map((item) => <div key={item.domain} className="rounded-lg border border-stroke p-3"><p className="font-semibold">{item.domain}</p><p className="mt-1 text-sm">{item.migrated.toLocaleString()} admitted / {item.source.toLocaleString()} source</p><p className="mt-1 text-xs text-content-muted">{item.held.toLocaleString()} held · {item.exception.toLocaleString()} exception · {item.deferred.toLocaleString()} deferred · {item.unresolved.toLocaleString()} unresolved · delta {item.delta.toLocaleString()}</p></div>)}</div><p className="mt-3 text-xs text-content-muted">Held, exception, deferred, and unresolved source records remain Migration evidence and are never presented as native Customers.</p></> }</Card>}

    {canManage && <Card className="p-ui-4 sm:p-ui-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-action-primary">Customer source reconciliation</p><h3 className="mt-1 text-lg font-semibold">Admit deterministic HCP Customers</h3><p className="mt-2 max-w-3xl text-sm text-content-muted">Create or bind only Customers with exact provider identity and validated source evidence. Any record that cannot be admitted safely is held for review while unrelated Customers continue. This never performs a fuzzy merge.</p></div><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" loading={populationRefresh.isPending} onClick={() => populationRefresh.mutate()}>Refresh source classification</Button><Button type="button" loading={cleanMajorityAdmission.isPending} onClick={() => cleanMajorityAdmission.mutate()}>Admit deterministic Customers</Button></div></div>{populationRefresh.isError && (() => { const error = getOperatorApiError(populationRefresh.error, "Customer source reconciliation"); return <Alert className="mt-4" variant="danger" title={error.title}>{error.message}</Alert>; })()}{populationRefresh.data && <Alert className="mt-4" variant={populationRefresh.data.counts.ambiguous || populationRefresh.data.counts.unexplained ? "warning" : "success"} title="Source classification refreshed">{populationRefresh.data.counts.total.toLocaleString()} accepted source Customers · {populationRefresh.data.counts.bound.toLocaleString()} bound · {populationRefresh.data.counts.held.toLocaleString()} held · {populationRefresh.data.counts.ambiguous.toLocaleString()} ambiguous · {populationRefresh.data.counts.unexplained.toLocaleString()} unexplained. No Customer admission was performed.</Alert>}{cleanMajorityAdmission.isError && (() => { const error = getOperatorApiError(cleanMajorityAdmission.error, "Customer admission"); return <Alert className="mt-4" variant="danger" title={error.title}>{error.message}</Alert>; })()}{cleanMajorityAdmission.data && <Alert className="mt-4" variant={cleanMajorityAdmission.data.quarantined || cleanMajorityAdmission.data.remaining_unexplained ? "warning" : "success"} title="Deterministic admission complete">{cleanMajorityAdmission.data.selected.toLocaleString()} source Customers processed · {cleanMajorityAdmission.data.admitted.toLocaleString()} admitted · {cleanMajorityAdmission.data.replayed.toLocaleString()} already admitted · {cleanMajorityAdmission.data.quarantined.toLocaleString()} held for review · {cleanMajorityAdmission.data.remaining_unexplained.toLocaleString()} remaining unexplained.</Alert>}</Card>}
    ...
    <Card>
      <div className="border-b border-stroke p-ui-4 sm:p-ui-5">
        <form onSubmit={submitSearch} className="grid gap-3 lg:grid-cols-[minmax(16rem,1fr)_auto_auto_auto_auto]">
          ...
          <select aria-label="Customer record state" className="min-h-11 rounded-lg border border-stroke-strong bg-surface px-3" value={recordState} onChange={(event) => { setRecordState(event.target.value as "current" | "archived" | "all"); setPage(1); }}><option value="current">Current records</option><option value="archived">Archived records</option><option value="all">Current and archived</option></select>
          <Button type="submit" variant="outline">Search</Button>
        </form>
        {(search || status || customerType || recordState !== "current") && <Button type="button" variant="ghost" onClick={clearFilters} className="mt-2">Clear roster filters</Button>}
      ...
      {customers.data && customers.data.items.length === 0 && customers.data.total_count === 0 && <EmptyState title={search || status || customerType || recordState !== "current" ? "No admitted Customers match these filters." : "No Customer records are currently admitted."} description={search || status || customerType || recordState !== "current" ? "Change the search or filters. Records held by Migration are outside this result." : "This is an empty native-authority result, not proof that upstream source data is empty or complete."} />}
      {customers.data && customers.data.items.length > 0 && <div className="divide-y divide-stroke">{customers.data.items.map((customer) => <Link key={customer.id} to={customerDetailPath(customer.id)} className="grid min-h-11 w-full min-w-0 gap-3 p-ui-4 text-left transition hover:bg-surface-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-focus sm:grid-cols-[1.5fr_1fr_1fr_auto] sm:items-center sm:p-ui-5">
        ...
        <p className="break-words text-sm text-content-secondary">{customer.primary_phone || "No phone"}</p><p className="break-all text-sm text-content-muted">{customer.email ?? "No email"}</p><span className={`w-fit rounded-full px-2.5 py-1 text-xs ${customer.archived_at || customer.status === "inactive" ? "bg-status-warning/15 text-status-warning" : "bg-status-success/15 text-status-success"}`}>{customer.archived_at ? "archived" : customer.status.replaceAll("_", " ")}</span>
      </Link>)}</div>}
