  ...
  return (
    <Card className="p-ui-4 sm:p-ui-6">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        ...
        {canReadPayments && <section className="rounded-xl border border-stroke p-4 lg:col-span-2"><h4 className="font-semibold">Payment status</h4><div className="mt-3 grid gap-2 md:grid-cols-2">{customerPayments.map((item) => <Link key={item.id} to={`/payments/${item.id}`} className="flex justify-between gap-3 rounded-lg bg-surface-subtle p-3 text-sm"><span>{item.currency} {item.captured_amount} received · {item.applied_amount} applied</span><Badge variant="neutral">{item.status}</Badge></Link>)}{payments.isSuccess && customerPayments.length === 0 && <p className="text-sm text-content-muted">No native Payment receipts.</p>}</div></section>}
      </div>
    </Card>
  );
}
