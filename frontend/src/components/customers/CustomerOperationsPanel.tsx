...
export function CustomerOperationsPanel({ customerId }: { customerId: string }) {
  ...
  return (
    <Card className="p-ui-4 sm:p-ui-6">
      ...
      {[jobs, appointments, estimates, invoices, payments].some((query) => query.isLoading) && <div className="mt-4"><Spinner label="Loading related work" /></div>}
      {[jobs, appointments, estimates, invoices, payments].some((query) => query.isError) && <div className="mt-4"><Alert variant="warning">Some related work is unavailable. Customer identity remains available; refresh before relying on this workspace as a complete related-work view.</Alert></div>}
