    ...
    <div className="mt-4 space-y-3">{items.map((person) => {
      ...
      return <article key={person.roster_key} className="rounded-xl border border-stroke p-4">
        ...
        {person.employee_id ? <>
          ...
          <div className="mt-3 flex flex-wrap gap-2"><Link className="text-sm font-semibold text-action-primary" to={`/employees?employee=${person.employee_id}`}>Open access, capabilities and history</Link><Link className="text-sm font-semibold text-action-primary" to="/payroll">Review Payroll blockers</Link></div>
