      ...
      return <article key={person.roster_key} className="rounded-xl border border-stroke p-4">
        ...
        {person.employee_id ? <>
          <dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
            <div><dt className="text-content-muted">ACP Employee</dt><dd>{person.employee_display_name}</dd></div><div><dt className="text-content-muted">Employment</dt><dd>{person.employment_status ? label(person.employment_status) : "Not established"}</dd></div><div><dt className="text-content-muted">Account</dt><dd>{label(person.user_state)} · {label(person.credential_state)}</dd></div><div><dt className="text-content-muted">Membership / MAIN</dt><dd>{label(person.membership_state)} · {label(person.branch_state)}</dd></div><div><dt className="text-content-muted">Role</dt><dd>{label(person.role_state)}</dd></div>
            <div><dt className="text-content-muted">Workforce</dt><dd>{label(person.workforce_profile_state)}</dd></div><div><dt className="text-content-muted">Mobile</dt><dd>{label(person.mobile_state)}</dd></div><div><dt className="text-content-muted">Dispatch handoff</dt><dd>{label(person.dispatch_state)}</dd></div><div><dt className="text-content-muted">Timekeeping handoff</dt><dd>{label(person.timekeeping_state)}</dd></div><div><dt className="text-content-muted">Payroll handoff</dt><dd>{label(person.payroll_linkage_state)}</dd></div>
          </dl>
