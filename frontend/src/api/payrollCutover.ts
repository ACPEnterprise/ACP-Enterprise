...
export interface CutoverReview {
  ...
  bridge_periods: Array<{ id: string; period_start: string; period_end: string; pay_date: string; source_type: string; certification_state: string; source_reference: string; coverage_complete: boolean }>;
}
...
export const certifyBridgePeriod = async (periodId: string, certifier_role: "owner" | "accountant") => (await apiClient.post(`/api/v1/payroll/cutover-review/bridge-periods/${periodId}/certify`, { certifier_role })).data;
