...
): Promise<InvoiceWorkspaceItem[]> {
  return (
    await apiClient.get<InvoiceWorkspaceItem[]>(`${root}/workspace`, {
      params: {
        as_of: filters.asOf,
        state: filters.state,
        ...
        aging_bucket: filters.agingBucket || undefined,
      },
    })
  ).data;
}
...
export async function getReceivablesSummary(
  ...
  return (
    await apiClient.get<ReceivablesSummary>(`${root}/receivables-summary`, {
      params: { as_of: asOf, branch_id: branchId || undefined },
    })
  ).data;
}
...
export async function getInvoiceCandidates(): Promise<InvoiceCandidate[]> {
  return (
    await apiClient.get<InvoiceCandidate[]>(`${root}/candidates`, {
      params: { limit: 100 },
    })
  ).data;
}
...
export async function getCustomerBalance(
  ...
  return (
    await apiClient.get<CustomerBalance>(
      `${root}/customers/${customerId}/balance`,
      { params: { as_of: asOf } },
    )
  ).data;
}

export async function getInvoiceOfficeDetail(
  id: string,
  asOf: string,
): Promise<InvoiceWorkspaceItem> {
  return (
    await apiClient.get<InvoiceWorkspaceItem>(`${root}/${id}/office-detail`, {
      params: { as_of: asOf },
    })
  ).data;
}
...
export async function creditInvoice(
  id: string,
  input: InvoiceAmountMutationInput,
): Promise<Invoice> {
...
export async function writeOffInvoice(
  ...
  return (await apiClient.post<Invoice>(`${root}/${id}/write-offs`, input))
    .data;
}
...
export async function recordManualPayment(
  ...
  return (
    await apiClient.post<ManualPaymentResult>(
      `${root}/${id}/manual-payments`,
      input,
    )
  ).data;
}
...
export async function getManualPaymentHistory(
  ...
  return (await apiClient.get<ManualPayment[]>(`${root}/${id}/manual-payments`))
    .data;
}
