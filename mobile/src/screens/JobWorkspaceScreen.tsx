  ...
  return <ScrollView testID="job-workspace-scroll" style={styles.safe} contentContainerStyle={styles.body} refreshControl={<RefreshControl refreshing={detail.status === "loading" || detail.refreshing} onRefresh={() => void detail.refresh()} accessibilityLabel="Refresh authoritative Job workspace" />}>
    ...
    {canReadField && detail.assignment?.job_id && <View style={styles.detail} accessible accessibilityLabel="Authoritative field workflow">
      ...
      {canExecuteField && field.item && field.item.job_id && field.item.job_version && <View style={styles.section}>
        ...
        <PrimaryButton label="Save Work Summary" disabled={!summary.trim() || field.status !== "ready"} onPress={() => void field.mutate(async () => { await fieldService.note(field.item!.job_id!, summary.trim(), field.item!.job_version!, field.item!.assignment_version); setSummary(""); })} />
        <Text style={styles.sectionTitle}>Customer work approval</Text><PrimaryButton label="Customer Approved Work" disabled={field.status !== "ready"} onPress={() => void field.mutate(() => fieldService.approval(field.item!.job_id!, "approved", field.item!.job_version!, field.item!.assignment_version))} /><PrimaryButton label="Customer Unavailable" disabled={field.status !== "ready"} onPress={() => void field.mutate(() => fieldService.approval(field.item!.job_id!, "unavailable", field.item!.job_version!, field.item!.assignment_version))} /><PrimaryButton label="Customer Refused" disabled={field.status !== "ready"} onPress={() => void field.mutate(() => fieldService.approval(field.item!.job_id!, "refused", field.item!.job_version!, field.item!.assignment_version))} />
