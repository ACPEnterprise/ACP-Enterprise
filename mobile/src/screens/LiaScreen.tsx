...
function ResponseCard({ item }: { item: Message }) {
  const [expanded, setExpanded] = useState(false);
  const { response } = item;
  return <View style={styles.card}><Text style={styles.question}>{item.question}</Text><Text style={styles.answer}>{response.answer}</Text><Text style={styles.meta}>{response.classification} · {response.freshness.replaceAll("_", " ")}</Text>{response.as_of && <Text style={styles.meta}>As of {response.as_of}</Text>}<Pressable accessibilityRole="button" accessibilityLabel={expanded ? "Hide LIA evidence" : "Show LIA evidence"} accessibilityState={{ expanded }} onPress={() => setExpanded((value) => !value)} style={styles.evidenceButton}><Text style={styles.evidenceLabel}>{expanded ? "Hide supporting evidence" : "Show supporting evidence"}</Text></Pressable>{expanded && <View style={styles.evidence}>{response.evidence.map((evidence) => <View key={`${evidence.domain}-${evidence.evidence_digest}`}><Text style={styles.evidenceTitle}>{evidence.label}</Text><Text style={styles.meta}>Source: {evidence.authority} · {evidence.freshness.replaceAll("_", " ")}</Text>{evidence.limitations.map((limitation) => <Text key={limitation} style={styles.meta}>{limitation}</Text>)}</View>)}{response.limitations.map((limitation) => <Text key={limitation} style={styles.meta}>{limitation}</Text>)}</View>}</View>;
}

export function LiaScreen({ service }: { service: LiaService }) {
  const [question, setQuestion] = useState(""); const [messages, setMessages] = useState<Message[]>([]); const [conversationId, setConversationId] = useState<string>(); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null);
  const submit = async () => { const value = question.trim(); if (!value || busy) return; setBusy(true); setError(null); try { const response = await service.ask(value, conversationId); setMessages((current) => [...current, { question: value, response }]); setConversationId(response.conversation_id); setQuestion(""); } catch (reason) { setError(failureMessage(reason)); } finally { setBusy(false); } };
  return <KeyboardAvoidingView style={styles.safe} behavior={Platform.OS === "ios" ? "padding" : undefined}><ScrollView style={styles.safe} contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled"><Text accessibilityRole="header" style={styles.title}>Ask LIA</Text><Text style={styles.intro}>Ask about your own assigned day and work. ACP Enterprise checks Employee permissions for every answer.</Text>{messages.length === 0 && <Text style={styles.empty}>Try “What is my next job?”</Text>}{messages.map((item, index) => <ResponseCard key={`${item.response.request_id}-${index}`} item={item} />)}{error && <View><Text accessibilityRole="alert" style={styles.error}>{error}</Text><PrimaryButton label="Retry" disabled={busy || !question.trim()} onPress={() => void submit()} /></View>}</ScrollView><View style={styles.composer}><TextInput accessibilityLabel="Ask LIA a question" placeholder="Ask about your assigned work" value={question} onChangeText={setQuestion} editable={!busy} multiline maxLength={1000} onSubmitEditing={() => void submit()} style={styles.input} /><PrimaryButton label={busy ? "Asking…" : "Send"} accessibilityLabel="Send question to Employee-safe LIA" disabled={busy || !question.trim()} onPress={() => void submit()} /></View></KeyboardAvoidingView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.canvas }, body: { padding: spacing.lg, gap: spacing.md, paddingBottom: spacing.lg }, title: { fontSize: 30, fontWeight: "800", color: colors.text }, intro: { fontSize: 16, lineHeight: 23, color: colors.muted }, empty: { color: colors.muted, fontStyle: "italic" }, card: { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 14, padding: spacing.md, gap: spacing.sm }, question: { fontWeight: "700", color: colors.muted }, answer: { fontSize: 18, lineHeight: 26, color: colors.text }, meta: { fontSize: 13, lineHeight: 19, color: colors.muted }, evidenceButton: { minHeight: touchTarget, justifyContent: "center" }, evidenceLabel: { color: colors.brand, fontWeight: "700" }, evidence: { borderTopColor: colors.border, borderTopWidth: 1, paddingTop: spacing.sm, gap: spacing.xs }, evidenceTitle: { fontWeight: "700", color: colors.text }, error: { color: colors.danger, fontSize: 16, lineHeight: 22 }, composer: { borderTopColor: colors.border, borderTopWidth: 1, backgroundColor: colors.surface, padding: spacing.md, gap: spacing.sm }, input: { minHeight: touchTarget, maxHeight: 100, borderColor: colors.border, borderWidth: 1, borderRadius: 10, padding: spacing.sm, fontSize: 16, color: colors.text } });
