...
requireConfig(app.version === "0.2.0" && app.ios.buildNumber === "3" && app.android.versionCode === 3, "Expo version metadata must be explicit");
requireConfig(info.includes("<string>0.2.0</string>") && info.includes("<string>3</string>"), "iOS plist version metadata must match Expo");
requireConfig((project.match(/MARKETING_VERSION = 0\.2\.0;/g) ?? []).length === 2 && (project.match(/CURRENT_PROJECT_VERSION = 3;/g) ?? []).length === 2, "Xcode version metadata must match Expo");
requireConfig(gradle.includes("versionCode 3") && gradle.includes('versionName "0.2.0"'), "Android version metadata must match Expo");
