...
requireConfig(app.version === "0.2.0" && app.ios.buildNumber === "2" && app.android.versionCode === 2, "Expo version metadata must be explicit");
requireConfig(info.includes("<string>0.2.0</string>") && info.includes("<string>2</string>"), "iOS plist version metadata must match Expo");
requireConfig((project.match(/MARKETING_VERSION = 0\.2\.0;/g) ?? []).length === 2 && (project.match(/CURRENT_PROJECT_VERSION = 2;/g) ?? []).length === 2, "Xcode version metadata must match Expo");
requireConfig(gradle.includes("versionCode 2") && gradle.includes('versionName "0.2.0"'), "Android version metadata must match Expo");
