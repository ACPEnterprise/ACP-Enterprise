...
test "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$app/Info.plist")" = "3"
embedded_config="$app/EXConstants.bundle/app.config"
test "$(plutil -extract extra.environment raw "$embedded_config")" = "preview"
test "$(plutil -extract extra.apiBaseUrl raw "$embedded_config")" = "https://preview.allcountyhomeservices.com"
test "$(plutil -extract extra.productionActivated raw "$embedded_config")" = "false"
