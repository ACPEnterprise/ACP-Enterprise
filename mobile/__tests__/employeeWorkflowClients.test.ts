...
describe("employee workflow clients", () => {
  ...
  it("uses authoritative field contracts without employee identity or client time", async () => {
    ...
    for (const body of bodies) { expect(body).not.toHaveProperty("employee_id"); expect(body).not.toHaveProperty("timestamp"); expect(body.idempotency_key).toMatch(/^mobile-field:/); }
