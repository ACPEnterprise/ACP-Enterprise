...
const service: TimekeepingService = { state: async () => state, timecard: async () => ({ employee_id: "synthetic", punch_state: state, pay_period: null, entries: [], job_intervals: [] }), punch: jest.fn(), jobClockState: jest.fn(), jobClock: jest.fn() };
const network = { isConnected: async () => true, subscribe: () => () => undefined };
describe("application shell", () => {
  it("boots an unauthenticated sign-in foundation", () => { render(<SignInScreen onSignIn={jest.fn()} onRequestPasswordReset={jest.fn()} />); expect(screen.getByText("ACP Employee")).toBeOnTheScreen(); });
