...
describe("employee-safe Mobile LIA", () => {
  ...
  it("speaks the server-composed answer and stops/replays safely", async () => { const lia = service(); const speech = { speak: jest.fn(), stop: jest.fn() }; render(<LiaScreen service={lia} speech={speech} />); fireEvent.changeText(screen.getByLabelText("Ask LIA a question"), "What is my next job?"); fireEvent.press(screen.getByLabelText("Send question to Employee-safe LIA")); await screen.findByText(response.answer); fireEvent.press(screen.getByLabelText("Speak this LIA answer")); expect(speech.speak).toHaveBeenCalledWith(response.answer, { language: "en-US", rate: 0.94, pitch: 1 }); fireEvent.press(screen.getByLabelText("Stop LIA playback")); expect(speech.stop).toHaveBeenCalled(); fireEvent.press(screen.getByLabelText("Speak this LIA answer")); expect(speech.speak).toHaveBeenCalledTimes(2); });
  it("cancels playback when a new answer arrives or the screen unmounts", async () => { const lia = service(); const speech = { speak: jest.fn(), stop: jest.fn() }; const view = render(<LiaScreen service={lia} speech={speech} />); fireEvent.changeText(screen.getByLabelText("Ask LIA a question"), "What is my next job?"); fireEvent.press(screen.getByLabelText("Send question to Employee-safe LIA")); await screen.findByText(response.answer); fireEvent.press(screen.getByLabelText("Speak this LIA answer")); fireEvent.changeText(screen.getByLabelText("Ask LIA a question"), "What is my schedule?"); fireEvent.press(screen.getByLabelText("Send question to Employee-safe LIA")); await waitFor(() => expect(speech.stop).toHaveBeenCalled()); view.unmount(); expect(speech.stop.mock.calls.length).toBeGreaterThanOrEqual(2); });
