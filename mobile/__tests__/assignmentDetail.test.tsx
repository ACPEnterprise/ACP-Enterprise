...
const fieldService: FieldService = {
  itinerary: jest.fn(), state: jest.fn(), arrival: jest.fn(), transition: jest.fn(), workSummary: jest.fn(), customerDisposition: jest.fn(), note: jest.fn(), approval: jest.fn(), refreshHandoff: jest.fn(),
};
...
function detail(h: ReturnType<typeof harness>, appointmentId = assignment.appointment_id, initial: DayAssignment | null = assignment) {
  return <JobWorkspaceScreen appointmentId={appointmentId} businessDate="2026-08-28" initialAssignment={initial} initialTimezone="America/New_York" service={h.service} fieldService={fieldService} network={h.network} />;
}
