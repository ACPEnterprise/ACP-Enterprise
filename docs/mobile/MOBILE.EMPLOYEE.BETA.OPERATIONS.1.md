...
`npm run beta:fixture` validates and prints the deterministic Preview-only plan without mutation. The fixture key is stable and every record must carry the synthetic marker/external key. The provider-neutral `betaFixtureProvisioning` boundary defines the accepted create order and domain-safe reset orchestration. It requires explicit Preview authorization, exact fixture-key binding, and an owner-supplied non-routable `.invalid` login. It rejects Production, routable or All County identities, contradictory records, and removal of records whose complete expected value does not match.

No live domain adapter is registered with the command, so `--apply` still fails closed and no Preview mutation occurs. A successor may bind the boundary only to reviewed domain APIs; it must not use direct SQL. Reset revokes the invitation, revokes sessions, clears assignment/time/field evidence through owning domains, and only then removes matching fixture records in reverse dependency order. The completed contract requires Job read/execute and own-pay-statement read plus synthetic statement/status scenarios; it still prohibits Payroll administration, wages, compensation, bank, and tax data. Never use Production or realistic identities.
