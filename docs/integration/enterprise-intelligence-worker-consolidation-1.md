...
## Cosmic consolidation cycle

- Fresh protected authority: `e69241c399323090ebbd003b4bd3bef73501fc78`.
  Prior Intelligence head `7189a28239b2a7dd45cfe1c9c4275f50dd7edf8f` was
  rebased onto it; protected ancestry is restored.
- Laptop-A `work/lia-connected-intelligence-maximum-1` at
  `cccfc26991f5c94f2f8f4edcd35d081168118c78` was integrated. Its cumulative
  connected-context, topic-switching, correction-routing, natural source
  routing, navigation, and corpus improvements are new relative to the
  current Intelligence head. No migration changed.
- Laptop-B `work/cosmic-intelligence-realdata-maximum-1` at
  `ee08fd5340427c2bee2f637b974c30d23a426612` was reconciled cumulatively.
  Analytics incomplete-monetary evidence, Beacon history, reporting repair,
  and compatible Luminary currency/period safeguards were integrated. Its
  older overlapping Luminary UI patch was intentionally not replayed because
  it removed the already-qualified delta explanation; this is recorded as
  stale/superseded overlap, not silently omitted.
- Phone/C `work/cosmic-mobile-employee-beta-maximum-1` at
  `2516f1128d4d24c90cba907dc227ba470b156219` was integrated. It adds
  authoritative Mobile date handling and a fail-closed Twelve Hats beta
  endpoint migration contract. The beta endpoint remains inactive and no
  employee-safe LIA client was present in the candidate.
- Employee-safe LIA is currently available at
  `POST /api/v1/lia/employee/ask`, guarded by
  `COMPANY_EMPLOYEE_OPERATIONS_OWN_LIA_READ` and the
  `ACP_EMPLOYEE_MOBILE` role. The implementation enforces tenant and
  assignment-safe scope, default denial, and excludes owner-only economic
  answers. Mobile may implement a gated client in a subsequent worker cycle;
  it must not fall back to owner `/api/v1/lia/ask`.
- Source-domain gaps remain handoffs to Scheduling/Dispatch, Price Book,
  Revenue Cycle, Timekeeping, and Customer/Job. No source-domain joins or
  calculations were implemented here.
