...
- Acceptance state: pending. Preview initially remained at `b296cc6b...` after
  ...
  transient incidents in acceptance evidence. Required #261-#283 evidence and
...
not later #261-#283. That bounded run passed PostgreSQL zero-to-head, 20 affected
...
|---|---|---|---|---:|---|---|
| OM2-C persona contract and acceptance harness | `work/om2c-launch-20260911-e2e-acceptance-1` | `1fb0481a043caaca749ae5dd49dc6cdf6d994061` | None; prior #212 is merged | 21/46 | `7e59bd741c60034294310a0086b8941f5c4a0e56` | Stale but reconcilable; merge-clean; protected/schema bindings must advance; acceptance execution blocked on missing platform service principal |
| SOURCE.4 artifact recovery | `work/migration-source4-accepted-artifact-recovery-1` | `a3cad3d389b9ed69300939d15c19e2d7b08da063` | None | 26/1 | `a7438bff69d42b9c26c7d44141ad455cd01ad0d4` | Stale but reconcilable; merge-clean; documentation-only |
| HCP historical safe-tranche builder | `work/hcp-historical-safe-tranche-1` | `b32f99ff80f447bf8140b73380d19191ccb8db59` | None | 36/1 | `c9afd4f9465d97f8e61230ea61afaebd93b5c041` | Stale but reconcilable; merge-clean; metadata edit required |
| SOURCE.4 UPDATE cohort authority | `work/hcp-update-cohort-authority-1` | `d53e5d36422218bd715f07d8099e09865167e0f1` | None | 20/1 | `a37c2bcd277603f0c36f26b6356e97864c3209d7` | Stale but reconcilable; merge-clean, but blocked: unsafe overwrite/symlink handling, incomplete authority verification, and no command/generator integration tests |
| ECO reconciliation | `work/eco-migration-reconciliation-integration-watch-1` | `1c0e7b20db62b6a342548f2842ea1a3a45965386` | None | 28/13 | `eb6d43a164dadcbe955de0e3ec618cf1447edad4` | Blocked on schema reconciliation; now produces a sibling head beside protected `i9k1m3o5q7s9`; rebase and assign a new revision downstream of that head |
| Payroll/QBO read UI | `work/om2b-payroll-accounting-continuation-1` | `724398348b566f655d2bc7127c20beeb6be52d6c` | #215 open/CLEAN | 47/1 | `08034745a5f9492802641159280dbf120d84cfd4` | Stale but reconcilable; merge-clean after #283; blocked behind authorization repairs, PR refresh, and combined rerun |
| Accounting navigation grouping | `work/ux-accounting-navigation-1` | `1e0f53a7e01e6ef342b1551e3489fb332978a4c2` | None | 4/1 | `90aeb969d325b114fb32e6a5661b693f609caeec` | Four protected commits stale but reconcilable; merge-clean; no schema/config; admissible after rebase, permission/deep-link/responsive/accessibility evidence, and a fresh PR |
| Mobile Apple release packet | `work/mobile-apple-owner-release-packet-1` | `0183eaec3e2825a79b683e9e684a761243c86ea7` | None | 33/15 | `af7edddda4d2946a66c08a5af30cab25d067b4b4` | Stale but reconcilable; merge-clean; blocked on visible-error/concurrency repair successor and two manifest edits |
...
The eight remaining effective deltas are merge-clean. ECO and cohort authority
are not admissible, and Mobile is blocked on a complete repair to protected #273-#276: ECO's
...
`3cb4e3ddb6ca1981f7ceb81ae3cffa06839a77d7`; they change 31 files and pass
`git diff --check`. Including blocked Mobile remains mechanically clean at tree
`5572044ba2615fdf00577655d82e43d20f7d9a11` across 53 files. Recompute all
...
7. Reconcile ECO to a new unique revision downstream of the sole protected head
   `i9k1m3o5q7s9`. Integrate ECO as its own later database checkpoint; never admit
   its current sibling head.
8. Treat #277-#282 as protected but operationally blocked; preserve #279 readback,
...
9. Treat protected #282 as rejected for operational use. Repair and requalify its
...
10. Keep protected #283 operationally disabled. Repair its cutover separation,
   completeness, replay, metadata, and concurrency contracts in a successor;
   requalify PostgreSQL zero-to-head at protected revision `i9k1m3o5q7s9`.
11. Integrate the Accounting navigation grouping as an independent frontend
...
12. Preserve #274's selector fix, audit #275's role expansion, and preserve #276's
...
13. Reconcile Mobile manifests after the complete #273-#276 successor is protected, then
...
```mermaid
flowchart LR
    A[Protected 4be32635 through Payroll cutover review] -->|reconcile| R[SOURCE.4 recovery]
    ...
    A --> T[PR 283 Payroll cutover repair]
...
|---|---|---|
| Acceptance tooling | Reconcile `1fb0481a...` to `4be32635...`, update schema/release bindings, then open a fresh PR. Route the declared service-principal/activation/session/token-writer gaps to Enterprise/platform before persona issuance | Contract tests fail, protected SHA moves, platform primitive remains missing at execution time, persona permissions/digests differ, or secret material appears in arguments/evidence |
| Current deployed acceptance | Wait for consecutive exact-`4be32635...` healthy responses; preserve the #281 rollout HTTP 502 at 19:42, exact `60035693...` through the #282/#283 merges, preceding `858b59cd...` at 19:21, the #277 rollout HTTP 502 at 18:48, and all earlier evidence; attach or rerun missing #261-#283 qualification; keep protected Migration commands/verifier, #273-#276 operator action, and #277-#283 office/Payroll-QBO/cutover operations disabled | Deployed SHA differs, focused test, zero-to-head migration, exactly-one-head/drift check fails, required evidence cannot be produced, defective tooling is exercised, or health/dependency state regresses |
...
| ECO checkpoint | Assign ECO a unique revision downstream of protected `i9k1m3o5q7s9`, requalify, then integrate/deploy independently | Sibling/duplicate/multiple Alembic head, drift, migration failure, or governed-policy acceptance failure |
...
| Protected Payroll cutover checkpoint | Keep #283 mutations disabled. Repair authority and completeness defects in a successor, require exactly one protected head `i9k1m3o5q7s9`, zero drift, full PostgreSQL/API/concurrency/replay/rollback evidence, then deploy and accept synthetic certification only—never Payroll execution | Same actor can certify and approve, a role can self-select stronger authority, any active Employee/fact/bridge coverage is omitted, changed replay succeeds, protected value leaks through metadata, partial commit/race occurs, multiple schema heads/drift appears, or any execution/ACH/provider action becomes reachable |
...
test "$(git rev-parse origin/customer-management-v1)" = \
  4be32635f4c9d8b684c8d97521f6e0092f666661
...
`4be32635...` and its deployed binding only after Preview reports that exact SHA;
advance schema binding to `i9k1m3o5q7s9` and recompute any
...
`a37c2bcd277603f0c36f26b6356e97864c3209d7`. It has no schema change and is the
...
`a7438bff69d42b9c26c7d44141ad455cd01ad0d4`, run `git diff --check`, push the
...
Update the packet authority from `9096a777...` to `4be32635...` and state that
...
`c9afd4f9465d97f8e61230ea61afaebd93b5c041`; lane qualification on 2026-09-12
...
`down_revision` to the sole protected head `i9k1m3o5q7s9`, and update every ECO
...
Composition was revalidated on 2026-09-14 at tree
`08034745a5f9492802641159280dbf120d84cfd4`; lane qualification on 2026-09-12
...
`4be32635f4c9d8b684c8d97521f6e0092f666661` from exact head
...
`08034745a5f9492802641159280dbf120d84cfd4`. There is no migration, QBO write,
...
cd backend
ENVIRONMENT=test DATABASE_URL="$QUALIFICATION_DATABASE_URL" alembic upgrade head
test "$(ENVIRONMENT=test DATABASE_URL="$QUALIFICATION_DATABASE_URL" alembic current | awk 'NR == 1 {print $1}')" = i9k1m3o5q7s9
test "$(ENVIRONMENT=test DATABASE_URL="$QUALIFICATION_DATABASE_URL" alembic heads | awk 'NR == 1 {print $1}')" = i9k1m3o5q7s9
test "$(ENVIRONMENT=test DATABASE_URL="$QUALIFICATION_DATABASE_URL" alembic heads | wc -l | tr -d ' ')" = 1
ENVIRONMENT=test DATABASE_URL="$QUALIFICATION_DATABASE_URL" alembic check
ENVIRONMENT=test DATABASE_URL="$QUALIFICATION_DATABASE_URL" python -m pytest -q \
...
PR #215 now composes mechanically over protected #283 at tree `08034745...` and
overlaps only in `PayrollRoute.tsx`; Accounting navigation (`90aeb969...`) and
Mobile (`af7edddd...`) compose without file overlap. ECO's Git-clean effective
tree is `eb6d43a1...`, but it has two sibling Alembic heads and is forbidden.
...
`90aeb969d325b114fb32e6a5661b693f609caeec`.
...
Composition was revalidated on 2026-09-14 at tree
`af7edddda4d2946a66c08a5af30cab25d067b4b4`; lane qualification on 2026-09-12
...
| Payroll cutover review | Repaired candidate/merge/deployed SHA; sole migration head; role/actor separation matrix; every in-scope Employee/fact and bridge-coverage result; changed/exact replay; concurrency/rollback; encryption key readiness without secret content; immutable revision, business-event, and audit IDs; before/after Payroll/QBO/provider state | Shared certify/approve actor, client-selected authority, incomplete Company/Employee/fact/bridge coverage, changed replay accepted, metadata/protected-value leak, partial/racy state, schema drift/multiple heads, missing audit evidence, real cutover approval, Payroll/ACH, or provider/QBO mutation |
