...
This candidate records confirmed dispositions but deliberately does not fabricate an Inventory receipt/issue API that is absent from protected authority. Enterprise must reconcile the eventual downstream composition after the frozen Inventory wave is integrated. Until that reconciliation, `confirmed` means a human disposition is authoritative, not that stock or Job cost moved. This is the exact next integration gate.
