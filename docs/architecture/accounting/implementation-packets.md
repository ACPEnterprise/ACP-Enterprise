...
| `ACC.POST.1` | [Posting contract](domain-event-posting-contract.md) and [machine packet](acc-post-1.packet.json); accepted core, invoice, payment, and AP facts; idempotent domain fact to journal posting, tax, inventory-control, payroll-summary interfaces | packet-defined `backend/app/accounting/posting/**`, focused producer contract/receipt seams and tests only | Owns slot 5 if required; no direct cross-domain table writes; currently `NOT STARTABLE` pending named producer/mapping dependencies | Exactly-once/replay, source mapping, failure queue, balanced atomic posting and receipt retry; serialized integration |
