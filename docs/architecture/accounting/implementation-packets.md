...
| `PAY.1-3.ACCEL` | Accepted [Payment/cash contract](payment-cash-settlement-contract.md), [machine packet](pay-1-3-accel.packet.json), and Invoice seams; payment receipt, application orchestration, refunds, failures, deposits, clearing, undeposited funds, settlement reconciliation | Packet-defined new Payments backend/frontend roots and narrowly required registration, event, permission, and migration seams | Owns slot 3 after Invoice; no processor replacement, instrument storage, or secrets | Provider fakes/contracts, replay/webhook/idempotency, duplicate-charge, refund, settlement, SOD, and control tests; Finance review |
...
| `ACC.POST.1` | [Posting contract](domain-event-posting-contract.md), [readiness closure](acc-post-1-readiness.md), and [machine packet](acc-post-1.packet.json); accepted core/invoice and contract-ready payment/AP facts; idempotent domain fact to journal posting | packet-defined `backend/app/accounting/posting/**`, focused producer contract/receipt seams and tests only | Owns slot 5 if required; no direct cross-domain table writes; currently `NOT STARTABLE` pending Payment/AP authoritative runtimes and separate Owner Start | Exactly-once/replay, fail-closed activation mappings, failure queue, balanced atomic posting and receipt retry; Inventory/payroll adapters may follow without blocking generic engine Start |
