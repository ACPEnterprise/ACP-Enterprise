...
| `ACC.MIG.1` | Accepted `ACC.DATA.1`, its [manifest schema](../../project/accounting-cutover/schemas/acc-mig-1-input-manifest.schema.json), and all target schemas; deterministic opening-state loader, rejects, dispositions, replay, control report | new `backend/app/accounting_migration/**`, tests, sanitized fixtures; no customer Migration workstream modification without separate authority | Owns slot 7 if persistence required; import execution is TYPE C and separate | Fresh/synthetic loads, replay, counts, exact controls, teardown; Preview/Production import forbidden until separately approved |
