...
class PaymentService:
        ...
        request_digest = _digest(
            {
                ...
                "opaque_payment_method": spec.opaque_payment_method,
            }
        )
        async with session.begin():
            ...
            existing = await session.scalar(
                ...
                .where(
                    ...
                    PaymentIntent.idempotency_key == spec.idempotency_key,
                )
                .with_for_update()
            )
            if existing:
            ...
            if (
                ...
                or not spec.opaque_payment_method.startswith("opaque_")
            ):
                raise PaymentValidation(
                    "Positive amount, ISO currency, and provider-safe opaque identity are required."
                )
            ...
            await session.flush()
            self._event(
                session, intent, EventType.PAYMENT_INTENT_CREATED, spec.actor_user_id
            )
        ...
        try:
            result = await self.provider.collect(
                ProviderRequest(
                    ...
                    amount,
                    intent.currency,
                    spec.opaque_payment_method,
                )
            )
        except Exception:  # noqa: BLE001 -- untrusted provider boundary
            # Submission began, so response loss cannot prove money did not move.
            # Persist uncertainty and require reconciliation; never reflect the
            # provider exception or blindly resubmit the economic command.
            result = ProviderResult(
                outcome="ambiguous",
                provider_operation_id=None,
                provider_code="provider_response_unavailable",
                evidence_digest=_digest(
                    {
                        "operation": "collect",
                        "intent_id": intent.id,
                        "provider_idempotency_key": intent.provider_idempotency_key,
                        "outcome": "provider_response_unavailable",
                    }
                ),
            )
        ...
        async with session.begin():
            ...
            if result.outcome == "captured":
                ...
                self._event(
                    session,
                    receipt,
                    EventType.PAYMENT_RECEIPT_CAPTURED,
                    spec.actor_user_id,
                )
                ...
                self._event(
                    session, intent, EventType.PAYMENT_FAILED, spec.actor_user_id
                )
        if receipt:
            await invoice_service.register_payment_receipt(
                session,
                PaymentReceiptFact(
                    ...
                    verified_amount=receipt.captured_amount,
    ...
    async def apply(self, session: AsyncSession, spec: ApplyReceipt) -> PaymentReceipt:
        ...
        digest = _digest(
            {
                "invoice_id": spec.invoice_id,
                "amount": amount,
                "expected_invoice_version": spec.expected_invoice_version,
            }
        )
        async with session.begin():
            receipt = await self._receipt(
                session, spec.company_id, spec.branch_id, spec.receipt_id, True
            )
        ...
        try:
            invoice = await invoice_service.apply_payment(
                session,
                PaymentApplication(
                    company_id=spec.company_id,
                    branch_id=spec.branch_id,
                    ...
                    amount=amount,
                    expected_version=spec.expected_invoice_version,
        ...
        async with session.begin():
            ...
            session.add(
                ReceiptEvent(
                    company_id=spec.company_id,
                    receipt_id=receipt.id,
                    event_type="application_observed",
                    amount=amount,
    ...
    async def request_refund(
        self, session: AsyncSession, spec: RequestRefund
    ) -> Refund:
        amount = spec.amount.quantize(CENT)
        digest = _digest(
            {
        ...
        async with session.begin():
            ...
            prior = await session.scalar(
                select(Refund).where(
                    ...
                    Refund.idempotency_key == spec.idempotency_key,
                )
            )
            if prior:
                if prior.request_digest != digest:
                    raise PaymentConflict(
                        "Idempotency key conflicts with the original refund."
                    )
                return prior
            receipt = await self._receipt(
                session, spec.company_id, spec.branch_id, spec.receipt_id, True
            ...
            if (
                receipt.version != spec.expected_version
                or amount <= 0
                or amount > receipt.available_amount
                or not spec.reason.strip()
            ):
                raise PaymentConflict(
                    "Refund is stale or exceeds unapplied refundable balance."
                )
            refund = Refund(
                company_id=spec.company_id,
                branch_id=spec.branch_id,
                ...
                requested_by_user_id=spec.actor_user_id,
            )
            session.add(refund)
            await session.flush()
            self._event(
                session, refund, EventType.PAYMENT_REFUND_REQUESTED, spec.actor_user_id
            )
        result = await self.provider.refund(
            ProviderRequest(
                refund.id,
                f"refund_{refund.id.hex}",
                self.merchant_account,
                amount,
                refund.currency,
                "opaque_refund",
        ...
        async with session.begin():
            locked_refund = await session.scalar(
                select(Refund)
                .where(Refund.company_id == spec.company_id, Refund.id == refund.id)
                .with_for_update()
            )
            receipt = await self._receipt(
                session, spec.company_id, spec.branch_id, spec.receipt_id, True
            ...
            refund.status = (
                "reconciliation_required"
                if result.outcome == "ambiguous"
                else ("succeeded" if result.outcome == "captured" else "failed")
            )
            if refund.status == "succeeded":
                ...
                receipt.status = (
                    "refunded"
                    if receipt.refunded_amount == receipt.captured_amount
                ...
                self._event(
                    session, refund, EventType.PAYMENT_REFUND_FAILED, spec.actor_user_id
                )
            else:
                self._exception(
                    session,
                    refund,
                    "ambiguous_refund_outcome",
                    result.evidence_digest,
                    spec.actor_user_id,
                )
        return refund
        ...
        async with session.begin():
            ...
            prior = await session.scalar(
                select(WebhookReceipt).where(
                    WebhookReceipt.company_id == company_id,
                    WebhookReceipt.provider == provider,
                    WebhookReceipt.merchant_account == evidence.merchant_account,
                    WebhookReceipt.provider_event_id == evidence.provider_event_id,
                )
            )
            if prior:
                conflict = any(
                ...
                result = WebhookReceipt(
                    ...
                    provider=provider,
                    merchant_account=evidence.merchant_account,
                    provider_event_id=evidence.provider_event_id,
                    event_type=evidence.event_type,
                    evidence_digest=evidence.evidence_digest,
                    secret_version=evidence.secret_version,
                    allowed_evidence=evidence.allowed_evidence,
                )
                session.add(result)
                await session.flush()
        if conflict:
            raise PaymentConflict(
                "Provider event identity conflicts with prior evidence."
            )
        assert result is not None
        return result

    async def create_deposit(
        self, session: AsyncSession, spec: CreateDeposit
    ) -> Deposit:
        digest = _digest(
            {
                "receipt_ids": sorted(str(value) for value in spec.receipt_ids),
                "currency": spec.currency,
                "destination_reference": spec.destination_reference,
            }
        )
        async with session.begin():
            await self._lock_command(
            ...
            prior = await session.scalar(
                select(Deposit).where(
                    Deposit.company_id == spec.company_id,
                    Deposit.idempotency_key == spec.idempotency_key,
                )
            )
            if prior:
                if prior.evidence_digest != digest:
                    raise PaymentConflict(
                        "Idempotency key conflicts with the original deposit."
                    )
                return prior
            if not spec.receipt_ids or not spec.destination_reference.strip():
                raise PaymentValidation(
                    "Deposit requires receipts and a destination reference."
                )
            rows = tuple(
                (
                    await session.scalars(
                        ...
                        .where(
                            PaymentReceipt.company_id == spec.company_id,
                            PaymentReceipt.branch_id == spec.branch_id,
                            PaymentReceipt.id.in_(spec.receipt_ids),
                            PaymentReceipt.currency == spec.currency,
                        )
                        .with_for_update()
                    )
                ).all()
            ...
            deposit = Deposit(
                ...
                currency=spec.currency,
                status="submitted",
                gross_amount=sum((row.captured_amount for row in rows), Decimal(0)),
                destination_reference=spec.destination_reference.strip(),
                idempotency_key=spec.idempotency_key,
                evidence_digest=digest,
            ...
            self._event(
                session,
                deposit,
                EventType.PAYMENT_DEPOSIT_SUBMITTED,
                spec.actor_user_id,
            )
            return deposit

    async def record_settlement(
        self, session: AsyncSession, spec: RecordSettlement
    ) -> Settlement:
        expected = (
            spec.gross_amount
            - spec.refund_amount
        ...
        async with session.begin():
            ...
            prior = await session.scalar(
                select(Settlement).where(
                    Settlement.company_id == spec.company_id,
                    Settlement.provider == spec.provider,
                    Settlement.merchant_account == spec.merchant_account,
                    Settlement.provider_payout_id == spec.provider_payout_id,
                )
            )
            if prior:
                if any(
                ...
                return prior
            if (
                spec.merchant_account != self.merchant_account
                or spec.provider != self.provider.name
                or len(spec.evidence_digest) != 64
            ):
                raise PaymentValidation(
                    "Settlement provider scope or evidence is invalid."
                )
            status = (
                "received" if expected == spec.net_amount else "reconciliation_required"
            )
            row = Settlement(
                **{
                    key: value
            ...
            await session.flush()
            self._event(
                session, row, EventType.PAYMENT_SETTLEMENT_RECEIVED, spec.actor_user_id
            )
    ...
    async def record_dispute(
        self, session: AsyncSession, spec: RecordDispute
    ) -> PaymentReceipt:
        amount = spec.amount.quantize(CENT)
        ...
        async with session.begin():
            ...
            if (
                ...
                raise PaymentConflict(
                    "Dispute is stale or exceeds receipt availability."
                )
            receipt.available_amount -= amount
            ...
            self._event(
                session, receipt, EventType.PAYMENT_DISPUTE_RECORDED, spec.actor_user_id
            )
            return receipt
    ...
    async def record_posting_receipt(
        ...
        async with session.begin():
            ...
            prior = await session.scalar(
                select(PaymentPostingReceipt).where(
                    PaymentPostingReceipt.company_id == fact.company_id,
                    PaymentPostingReceipt.source_event_id == fact.source_event_id,
                )
            )
            if prior:
                if any(
                    ...
                    raise PaymentConflict(
                        "Accounting posting receipt conflicts with replay."
                    )
                return prior
    ...
    async def get_receipt(
        ...
        return await session.scalar(
            select(PaymentReceipt).where(
                PaymentReceipt.company_id == company_id, PaymentReceipt.id == receipt_id
            )
        )
    ...
    async def _receipt(
        ...
        query = select(PaymentReceipt).where(
            PaymentReceipt.company_id == company_id,
            PaymentReceipt.branch_id == branch_id,
    ...
    @staticmethod
    def _event(
        session: AsyncSession, entity: Any, event_type: EventType, actor: UUID
    ) -> None:
        BusinessEventService.stage(
            ...
            BusinessEventCreate(
                ...
                payload={
                    "schema_version": "1.0",
                    "evidence_digest": getattr(entity, "evidence_digest", None),
                    "currency": getattr(entity, "currency", None),
                    "amount": str(
                        getattr(
                            entity, "amount", getattr(entity, "captured_amount", "0.00")
                        )
                    ),
                },
            ),
    ...
    @staticmethod
    def _exception(
        session: AsyncSession, entity: Any, reason: str, digest: str, actor: UUID
    ) -> None:
        PaymentService._exception_raw(
            session,
            entity.company_id,
    ...
    def _exception_raw(
        ...
        row = ReconciliationException(
            ...
            entity_id=entity_id,
            reason_code=reason,
            idempotency_key=f"{entity_type}:{entity_id}:{reason}",
            evidence_digest=digest,
            opened_by_user_id=actor,
        )
        session.add(row)
        BusinessEventService.stage(
            session,
            BusinessEventCreate(
                event_type=EventType.PAYMENT_RECONCILIATION_EXCEPTION_OPENED,
                entity_type="payment_reconciliation_exception",
                entity_id=row.id,
                company_id=company_id,
                branch_id=branch_id,
                user_id=actor,
