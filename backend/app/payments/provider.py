...
class DeterministicFakeProvider(PaymentProvider):
    ...
    def _execute(self, request: ProviderRequest, operation: str) -> ProviderResult:
        ...
        outcome = next(
            (x for x in ("ambiguous", "declined", "failed") if x in marker), None
        )
        outcome = outcome or "captured"
        provider_id = f"fake_{operation}_{request.operation_id.hex}"
        result = ProviderResult(
            cast(ProviderOutcome, outcome),
            provider_id,
            None if outcome == "captured" else f"synthetic_{outcome}",
...
class DeterministicWebhookVerifier:
    ...
    def verify(
        self, *, raw_body: bytes, signature: str, timestamp: str, merchant_account: str
    ) -> VerifiedWebhook:
        try:
        ...
        if (
            abs((datetime.now(timezone.utc) - sent_at).total_seconds())
            > self._tolerance
        ):
        ...
        allowed = {
            key: str(data[key])
            for key in ("provider_operation_id", "amount", "currency")
            if key in data
        }
        ...
        return VerifiedWebhook(
            provider_event_id=str(data["event_id"]),
