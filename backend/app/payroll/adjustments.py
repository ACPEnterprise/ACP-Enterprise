...
class PayrollAdjustmentService:
    ...
    async def create(
        ...
        *,
        ...
        source_evidence = await self._verify_source(
            session, context, draft, posting_candidate
        )
        canonical = {
            ...
            "deltas": tuple(
                (item.component, str(item.amount))
                for item in sorted(
                    draft.delta_components, key=lambda item: item.component
                )
            ),
            "employee_id": str(draft.employee_id) if draft.employee_id else None,
            "original_pay_period_id": str(draft.original_pay_period_id)
            if draft.original_pay_period_id
            else None,
            "off_cycle_pay_period_id": str(draft.off_cycle_pay_period_id)
            if draft.off_cycle_pay_period_id
            else None,
            "supersedes_adjustment_id": str(draft.supersedes_adjustment_id)
        ...
        await session.execute(
            ...
            {
                "key": f"payroll-adjustment:{context.company.id}:{draft.source_type}:{draft.source_id}:{draft.classification.value}"
            },
        )
        existing = await session.scalar(
            select(PayrollAdjustmentAuthorityRecord).where(
                PayrollAdjustmentAuthorityRecord.company_id == context.company.id,
                PayrollAdjustmentAuthorityRecord.adjustment_digest == digest,
            )
        )
        if existing:
            return existing
        active = await session.scalar(
            select(PayrollAdjustmentAuthorityRecord)
            .where(
                PayrollAdjustmentAuthorityRecord.company_id == context.company.id,
                PayrollAdjustmentAuthorityRecord.source_type == draft.source_type,
                PayrollAdjustmentAuthorityRecord.source_id == draft.source_id,
                PayrollAdjustmentAuthorityRecord.classification
                == draft.classification.value,
                PayrollAdjustmentAuthorityRecord.lifecycle.in_(
        ...
        if draft.supersedes_adjustment_id:
            predecessor = await session.scalar(
                ...
                .where(
                    PayrollAdjustmentAuthorityRecord.company_id == context.company.id,
                    PayrollAdjustmentAuthorityRecord.id
                    == draft.supersedes_adjustment_id,
                )
                .with_for_update()
            )
            if (
                ...
                raise PayrollConflictError(
                    "Payroll adjustment supersession lineage conflicts"
                )
            predecessor.lifecycle = "superseded"
        elif active:
            raise PayrollConflictError("competing active Payroll adjustment exists")
        value = PayrollAdjustmentAuthorityRecord(
            company_id=context.company.id,
            employee_id=draft.employee_id,
            original_pay_period_id=draft.original_pay_period_id,
            off_cycle_pay_period_id=draft.off_cycle_pay_period_id,
            classification=draft.classification.value,
            reason_code=draft.reason_code,
            source_type=draft.source_type,
            ...
            delta_components=[
                {"component": item.component, "amount": str(item.amount)}
                for item in sorted(
                    draft.delta_components, key=lambda item: item.component
                )
            ],
            ...
            evidence_digest=draft.evidence_digest,
            definition_version=ADJUSTMENT_VERSION,
            adjustment_identity=f"payroll-adjustment:{digest}",
            adjustment_digest=digest,
            lifecycle="draft",
            created_by_user_id=context.user.id,
            supersedes_adjustment_id=predecessor.id if predecessor else None,
        )
        session.add(value)
        await session.flush()
        if predecessor:
            self._stage(
                ...
                context,
                predecessor,
                EventType.PAYROLL_ADJUSTMENT_SUPERSEDED,
                "payroll.adjustment.superseded",
            )
        self._stage(
            session,
            context,
            value,
            EventType.PAYROLL_ADJUSTMENT_CREATED,
    ...
    async def initiate_review(
        ...
        *,
        ...
        record = await self._review(
            session, context, value, AdjustmentReviewDecision.INITIATED, reason_code
        )
        self._stage(
            session,
            context,
            value,
            EventType.PAYROLL_ADJUSTMENT_REVIEWED,
            "payroll.adjustment.review_initiated",
        )
        await session.commit()
        return record
    ...
    async def decide_review(
        ...
        *,
        ...
        self._require(context, PayrollPermission.ADJUSTMENT_REVIEW)
        if decision not in {
            AdjustmentReviewDecision.ACCEPTED,
            AdjustmentReviewDecision.REJECTED,
        }:
        ...
        value.lifecycle = (
            "draft" if decision is AdjustmentReviewDecision.ACCEPTED else "rejected"
        )
        ...
        self._stage(
            ...
            value,
            EventType.PAYROLL_ADJUSTMENT_REVIEWED,
            f"payroll.adjustment.review_{decision.value}",
        )
        await session.commit()
        return record
    ...
    async def approve(
        ...
        *,
        ...
        accepted = await session.scalar(
            select(PayrollAdjustmentReviewRecord.id).where(
                PayrollAdjustmentReviewRecord.adjustment_id == value.id,
                PayrollAdjustmentReviewRecord.decision == "accepted",
            )
        )
        ...
        record = await self._review(
            session, context, value, AdjustmentReviewDecision.APPROVED, reason_code
        )
        self._stage(
            session,
            context,
            value,
            EventType.PAYROLL_ADJUSTMENT_APPROVED,
    ...
    async def consequence(
        ...
        *,
        ...
        return AdjustmentConsequence(
            value.id,
            value.adjustment_digest,
            classification,
            classification
            in {
                PayrollCorrectionType.PRE_PAYMENT_PAYROLL_CORRECTION,
                PayrollCorrectionType.RETROACTIVE_EARNINGS,
                PayrollCorrectionType.OFF_CYCLE_PAYROLL,
                PayrollCorrectionType.TAX_CORRECTION,
                PayrollCorrectionType.DEDUCTION_CORRECTION,
            },
            classification
            in {
            ...
            classification
            in {
                PayrollCorrectionType.ACCOUNTING_ADJUSTMENT_REQUIRED,
                PayrollCorrectionType.PAYMENT_RETURN,
                PayrollCorrectionType.PAYMENT_REVERSAL,
                PayrollCorrectionType.SETTLEMENT_CORRECTION,
            },
    ...
    async def _verify_source(
        ...
        context: AuthorizationContext,
        draft: DraftPayrollAdjustment,
        candidate: PayrollPostingFactCandidate | None,
    ) -> dict[str, object]:
        model_and_digest = {
        ...
        if draft.source_type == "payroll_posting_fact_candidate":
            if candidate is None:
                raise PayrollConflictError(
                    "verified Payroll PostingFact candidate is required"
                )
            candidate.verify()
            if (
                candidate.fact.company_id != context.company.id
                or candidate.fact.source_event_id != draft.source_id
                or candidate.candidate_identity.rsplit(":", 1)[-1]
                != draft.source_digest
            ):
                raise PayrollConflictError(
                    "Payroll PostingFact candidate source mismatch"
                )
            return {
                "posting_fact_identity": candidate.candidate_identity,
                "posting_fact_digest": candidate.fact.canonical_digest(),
                "posted": False,
            }
        if draft.source_type == "posted_accounting_journal":
            value = await session.scalar(
                select(Journal).where(
                    ...
                    Journal.id == draft.source_id,
                    Journal.status == "posted",
                    Journal.source_digest == draft.source_digest,
                )
            )
            if value is None:
                raise PayrollConflictError(
                    "posted Accounting source authority is unavailable"
                )
            return {
                "journal_id": str(value.id),
                "journal_source_digest": value.source_digest,
                "posted": True,
            }
        ...
        value = await session.scalar(
            select(model).where(
                ...
                model.id == draft.source_id,
                getattr(model, digest_field) == draft.source_digest,
            )
        )
        if value is None:
            raise PayrollConflictError(
                "authoritative Payroll adjustment source is unavailable"
            )
        return {
            "source_id": str(draft.source_id),
            "source_digest": draft.source_digest,
    ...
    def _validate_draft(draft: DraftPayrollAdjustment) -> None:
        if (
            not draft.reason_code.strip()
            or len(draft.reason_code) > 80
            or len(draft.source_digest) != 64
            or len(draft.evidence_digest) != 64
        ):
            raise PayrollConflictError("Payroll adjustment evidence is invalid")
        if not draft.delta_components or len(
            {item.component for item in draft.delta_components}
        ) != len(draft.delta_components):
            raise PayrollConflictError("Payroll adjustment delta is incomplete")
        if any(
            not item.component.strip()
            or not item.amount.is_finite()
            or item.amount == 0
            for item in draft.delta_components
        ):
            raise PayrollConflictError("Payroll adjustment delta is invalid")
        if draft.classification is PayrollCorrectionType.OFF_CYCLE_PAYROLL and (
            draft.original_pay_period_id is None
            or draft.off_cycle_pay_period_id is None
            or draft.original_pay_period_id == draft.off_cycle_pay_period_id
        ):
            raise PayrollConflictError(
                "off-cycle Payroll requires distinct period authority"
            )
    ...
    async def _review(
        ...
        sequence = (
            await session.scalar(
                select(func.count(PayrollAdjustmentReviewRecord.id)).where(
                    PayrollAdjustmentReviewRecord.adjustment_id == value.id
                )
            )
            or 0
        ...
        record = PayrollAdjustmentReviewRecord(
    ...
    @staticmethod
    async def _locked(
        session: AsyncSession, context: AuthorizationContext, adjustment_id: UUID
    ) -> PayrollAdjustmentAuthorityRecord:
        value = await session.scalar(
            select(PayrollAdjustmentAuthorityRecord)
            .where(
