...
class PayrollAdjustmentResultService:
    ...
    async def persist_candidate(
        ...
        *,
        ...
        candidate: PayrollAdjustmentCalculationCandidate,
        ...
        authority = await session.scalar(
            select(PayrollAdjustmentAuthorityRecord).where(
                ...
                PayrollAdjustmentAuthorityRecord.id == candidate.adjustment_id,
                PayrollAdjustmentAuthorityRecord.adjustment_digest
                == candidate.adjustment_digest,
        ...
        if (
            ...
            or candidate.employee_id != authority.employee_id
            or candidate.original_pay_period_id != authority.original_pay_period_id
            or candidate.correction_pay_period_id != authority.off_cycle_pay_period_id
            or candidate.source_id != authority.source_id
            or candidate.source_digest != authority.source_digest
            or candidate.source_type != authority.source_type
            or candidate.currency != authority.currency
            or candidate.classification.value != authority.classification
        ):
            raise PayrollConflictError("adjustment result admission scope mismatch")
        await session.execute(
            text("SELECT pg_advisory_xact_lock(hashtextextended(:key, 0))"),
            {
                "key": f"payroll-adjustment-result:{context.company.id}:{candidate.adjustment_id}"
            },
        )
        existing = await session.scalar(
            select(PayrollAdjustmentResultRecord).where(
                PayrollAdjustmentResultRecord.company_id == context.company.id,
                PayrollAdjustmentResultRecord.calculation_digest
        ...
        active = await session.scalar(
            select(PayrollAdjustmentResultRecord)
            .where(
                PayrollAdjustmentResultRecord.company_id == context.company.id,
                PayrollAdjustmentResultRecord.adjustment_id == candidate.adjustment_id,
                PayrollAdjustmentResultRecord.lifecycle.in_(
        ...
        if supersedes_result_id:
            predecessor = await session.scalar(
                ...
                .where(
                    PayrollAdjustmentResultRecord.company_id == context.company.id,
                    PayrollAdjustmentResultRecord.id == supersedes_result_id,
                )
                .with_for_update()
            )
            if (
                ...
                or predecessor.lifecycle == "applied_to_successor_authority"
            ):
                raise PayrollConflictError(
                    "adjustment result supersession lineage conflict"
                )
            predecessor.lifecycle = "superseded"
        elif active:
            raise PayrollConflictError("active adjustment result already exists")
        value = PayrollAdjustmentResultRecord(
            company_id=candidate.company_id,
            employee_id=candidate.employee_id,
            original_pay_period_id=candidate.original_pay_period_id,
            correction_pay_period_id=candidate.correction_pay_period_id,
            adjustment_id=candidate.adjustment_id,
            adjustment_digest=candidate.adjustment_digest,
            ...
            consequences=[item.value for item in candidate.consequences],
            ...
            supersedes_result_id=predecessor.id if predecessor else None,
        )
        session.add(value)
        await session.flush()
        ...
        if predecessor:
            self._stage(
                ...
                context,
                predecessor,
                EventType.PAYROLL_ADJUSTMENT_RESULT_SUPERSEDED,
                "payroll.adjustment_result.superseded",
            )
        self._stage(
            session,
            context,
            value,
            EventType.PAYROLL_ADJUSTMENT_RESULT_PERSISTED,
            "payroll.adjustment_result.persisted",
        )
    ...
    async def initiate_review(
        ...
        *,
        ...
        record = await self._review(
            ...
            context,
            value,
            AdjustmentResultDecision.INITIATED,
            reason_code,
            safe_note,
        )
        self._stage(
            ...
            context,
            value,
            EventType.PAYROLL_ADJUSTMENT_RESULT_REVIEWED,
            "payroll.adjustment_result.review_initiated",
        )
        await session.commit()
        return record
    ...
    async def decide_review(
        ...
        *,
        context: AuthorizationContext,
        result_id: UUID,
        decision: AdjustmentResultDecision,
        reason_code: str,
        safe_note: str | None = None,
    ) -> PayrollAdjustmentResultReviewRecord:
        self._require(context, PayrollPermission.ADJUSTMENT_RESULT_REVIEW)
        if decision not in {
            AdjustmentResultDecision.ACCEPTED,
            AdjustmentResultDecision.REJECTED,
        }:
        ...
        value.lifecycle = (
            "calculated"
            if decision is AdjustmentResultDecision.ACCEPTED
            else "rejected"
        )
        record = await self._review(
        ...
        self._stage(
            ...
            value,
            EventType.PAYROLL_ADJUSTMENT_RESULT_REVIEWED,
            f"payroll.adjustment_result.review_{decision.value}",
        )
        await session.commit()
        return record
        ...
        *,
        ...
        if value.lifecycle == "approved":
            replay = await session.scalar(
                select(PayrollAdjustmentResultReviewRecord).where(
                    PayrollAdjustmentResultReviewRecord.result_id == value.id,
                    PayrollAdjustmentResultReviewRecord.reviewer_user_id
                    == context.user.id,
                    PayrollAdjustmentResultReviewRecord.decision == "approved",
                    PayrollAdjustmentResultReviewRecord.reason_code
                    == reason_code.strip(),
                )
            )
            if replay:
        ...
        accepted = await session.scalar(
            select(PayrollAdjustmentResultReviewRecord.id).where(
                PayrollAdjustmentResultReviewRecord.result_id == value.id,
                PayrollAdjustmentResultReviewRecord.decision == "accepted",
            )
        )
        ...
        record = await self._review(
            ...
            context,
            value,
            AdjustmentResultDecision.APPROVED,
            reason_code,
            None,
        )
        self._stage(
            ...
            context,
            value,
            EventType.PAYROLL_ADJUSTMENT_RESULT_APPROVED,
            "payroll.adjustment_result.approved",
        )
        await session.commit()
        return record
        ...
        *,
        ...
        existing = await session.scalar(
            select(PayrollAdjustmentApplicationRecord).where(
                PayrollAdjustmentApplicationRecord.result_id == value.id,
                PayrollAdjustmentApplicationRecord.purpose == purpose,
            )
        )
        content = {
            ...
            "purpose": purpose,
            "successor_authority_type": successor_authority_type,
        ...
        if value.lifecycle != "approved":
            raise PayrollConflictError(
                "approved unapplied adjustment result is required"
            )
        ...
        record = PayrollAdjustmentApplicationRecord(
            ...
            successor_authority_type=successor_authority_type,
            result_digest=value.calculation_digest,
            authorized_components=value.components,
        ...
        self._stage(
            ...
            context,
            value,
            EventType.PAYROLL_ADJUSTMENT_RESULT_APPLIED,
            "payroll.adjustment_result.applied",
        )
        await session.commit()
        return record
    ...
    async def result(
        ...
        value = await session.scalar(
            select(PayrollAdjustmentResultRecord).where(
                PayrollAdjustmentResultRecord.company_id == context.company.id,
                PayrollAdjustmentResultRecord.id == result_id,
            )
        )
        if value is None:
    ...
    @staticmethod
    def application_handoff(
        value: PayrollAdjustmentApplicationRecord,
    ) -> SuccessorApplication:
        return SuccessorApplication(
            value.result_id,
            value.result_digest,
            value.purpose,
            value.successor_authority_type,
            tuple(value.authorized_components),
    ...
    async def _review(
        ...
        replay = await session.scalar(
            select(PayrollAdjustmentResultReviewRecord).where(
                PayrollAdjustmentResultReviewRecord.result_id == value.id,
                PayrollAdjustmentResultReviewRecord.reviewer_user_id == context.user.id,
                PayrollAdjustmentResultReviewRecord.decision == decision.value,
                PayrollAdjustmentResultReviewRecord.reason_code == reason,
                PayrollAdjustmentResultReviewRecord.safe_note == note,
            )
        )
        if replay:
            return replay
        sequence = (
            await session.scalar(
                select(func.count(PayrollAdjustmentResultReviewRecord.id)).where(
                    PayrollAdjustmentResultReviewRecord.result_id == value.id
                )
            )
            or 0
        ...
        record = PayrollAdjustmentResultReviewRecord(
            company_id=value.company_id,
            result_id=value.id,
            sequence=sequence,
    ...
    @staticmethod
    def _verify(
        value: PayrollAdjustmentResultRecord, predecessor_identity: str | None
    ) -> None:
        del (
            predecessor_identity
        )  # lineage is persistence authority, not economic digest input
        components = tuple(
            CalculatedAdjustmentComponent(
                component=str(item["component"]),
                delta=Decimal(str(item["delta"])),
                currency=str(item["currency"]),
                recognition_effect=RecognitionEffect(str(item["recognition_effect"])),
                provider_id=str(item["provider_id"]),
                provider_version=str(item["provider_version"]),
                rule_evidence_digest=str(item["rule_evidence_digest"]),
            )
        ...
        candidate = PayrollAdjustmentCalculationCandidate(
            ...
            definition_version=value.calculation_version,
            company_id=value.company_id,
            employee_id=value.employee_id,
            original_pay_period_id=value.original_pay_period_id,
            correction_pay_period_id=value.correction_pay_period_id,
            adjustment_id=value.adjustment_id,
            adjustment_digest=value.adjustment_digest,
            ...
            consequences=tuple(
                AdjustmentConsequenceType(item) for item in value.consequences
            ),
    ...
    def _purpose(classification: str) -> str:
        mapping = {
            "pre_payment_payroll_correction": "successor_payroll",
            "retroactive_earnings": "successor_payroll",
            "off_cycle_payroll": "off_cycle_payroll",
            "tax_correction": "successor_tax_calculation",
            "deduction_correction": "successor_deduction_calculation",
            "payment_return": "payment_recovery",
            "payment_rejection": "payment_reissue",
            "payment_reversal": "payment_recovery",
            "settlement_correction": "settlement_reconciliation",
            "accounting_adjustment_required": "accounting_adjustment",
        }
    ...
    async def _predecessor_identity(
        self, session: AsyncSession, value: PayrollAdjustmentResultRecord
    ) -> str | None:
        if value.supersedes_result_id is None:
            return None
        return await session.scalar(
            select(PayrollAdjustmentResultRecord.result_identity).where(
                PayrollAdjustmentResultRecord.id == value.supersedes_result_id
            )
        )
    ...
    def _stage(
        ...
        BusinessEventService.stage(
            ...
            BusinessEventCreate(
                event_type=event,
                entity_type="payroll_adjustment_result",
                entity_id=value.id,
                company_id=value.company_id,
        ...
        self._audit.stage(
            ...
            AuditEntry(
                action=action,
                resource_type="payroll_adjustment_result",
