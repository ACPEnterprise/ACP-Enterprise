    ...
    def verify(self) -> None:
        ...
        if expected != self.evidence_digest:
            raise PayrollConflictError(
                "payment destination admission digest is invalid"
            )
...
class PaymentReleaseCandidate:
    ...
    def canonical_content(self) -> dict[str, object]:
        return {
            ...
            "currency": self.currency,
            "instructions": tuple(
                item.canonical_content() for item in self.instructions
            ),
    ...
    def verify(self) -> None:
        ...
        if (
            ...
            or self.package_identity != f"payroll-payment-release:{digest}"
        ):
            raise PayrollConflictError("payment release identity or digest is invalid")
        if len({item.employee_id for item in self.instructions}) != len(
            self.instructions
        ):
        ...
        for item in self.instructions:
            ...
            if (
                item.instruction_digest != item_digest
                or item.instruction_identity
                != f"payroll-payment-instruction:{item_digest}"
            ):
        ...
        total = sum(
            (
                item.amount
                for item in self.instructions
                if item.disposition is PaymentInstructionDisposition.READY
            ),
            Decimal(0),
        )
...
class PayrollPaymentReleaseService:
    ...
    async def create_destination(
        ...
        *,
        ...
        if (
            not draft.destination_reference.strip()
            or not draft.masked_display.strip()
        ...
        if draft.protected_payload is not None:
            if self._cipher is None:
                raise PayrollConflictError(
                    "protected payment destination key is unavailable"
                )
            key_id, nonce, ciphertext, protected_digest = self._cipher.encrypt(
                company_id=context.company.id, payload=draft.protected_payload
            )
        ...
        canonical = {
            ...
            "destination_version": draft.destination_version,
            "definition_version": PAYMENT_DESTINATION_VERSION,
            "method": draft.method.value,
            "destination_reference": draft.destination_reference,
            "protected_digest": protected_digest,
            "masked_display": draft.masked_display,
            "verification_evidence_digest": draft.verification_evidence_digest,
            "effective_start": draft.effective_start.isoformat(),
            "effective_end": draft.effective_end.isoformat()
            if draft.effective_end
            else None,
        }
        value = PayrollPaymentDestinationVersion(
            company_id=context.company.id,
            employee_id=draft.employee_id,
            destination_version=draft.destination_version,
            definition_version=PAYMENT_DESTINATION_VERSION,
            method_type=draft.method.value,
            destination_reference=draft.destination_reference,
            ...
            lifecycle="draft",
            authority_digest=canonical_digest(canonical),
            supersedes_destination_id=None,
            created_by_user_id=context.user.id,
        ...
        session.add(value)
        await session.flush()
        self._stage_destination(
            session,
            context,
            value,
            EventType.PAYROLL_PAYMENT_DESTINATION_CREATED,
            "payroll.payment_destination.created",
        )
    ...
    async def approve_destination(
        ...
        *,
        ...
        value = await session.scalar(
            select(PayrollPaymentDestinationVersion)
            .where(
                PayrollPaymentDestinationVersion.company_id == context.company.id,
                PayrollPaymentDestinationVersion.id == destination_id,
            )
            .with_for_update()
        )
        ...
        self._stage_destination(
            ...
            context,
            value,
            EventType.PAYROLL_PAYMENT_DESTINATION_APPROVED,
        ...
        values = tuple(
            (
                await session.scalars(
                    select(PayrollPaymentDestinationVersion).where(
                        PayrollPaymentDestinationVersion.company_id == company_id,
                        PayrollPaymentDestinationVersion.employee_id == employee_id,
                        PayrollPaymentDestinationVersion.effective_start <= as_of_date,
                        (
                            PayrollPaymentDestinationVersion.effective_end.is_(None)
                            | (
                                PayrollPaymentDestinationVersion.effective_end
                                > as_of_date
                            )
                        ),
                    )
    ...
    async def assemble_candidate(
        ...
        *,
        context: AuthorizationContext,
        payroll_run_id: UUID,
        destinations: dict[UUID, DestinationResolution],
        assembled_at: datetime,
        supersedes_package_identity: str | None = None,
    ) -> PaymentReleaseCandidate:
        ...
        if set(destinations) != {item.employee_id for item in members}:
            raise PayrollConflictError(
                "payment release payable population is incomplete"
            )
        ...
        for member in members:
            ...
            if resolution.state is DestinationAdmissionState.READY:
                disposition, amount, blocker = (
                    PaymentInstructionDisposition.READY,
                    tax.net_pay_candidate,
                    None,
                )
            else:
                disposition, amount, blocker = (
                    PaymentInstructionDisposition.BLOCKED,
                    Decimal("0.00"),
                    resolution.evidence_digest,
                )
            provisional = PaymentInstruction(
                member.employee_id,
                disposition,
                ...
                resolution.destination_digest,
                resolution.method,
                resolution.protected_reference,
                amount,
                run.currency,
                blocker,
                "",
            ...
            instructions.append(
                replace(
                    provisional,
                    instruction_identity=f"payroll-payment-instruction:{digest}",
                    instruction_digest=digest,
                )
            )
        ordered = tuple(instructions)
        provisional_package = PaymentReleaseCandidate(
            run.company_id,
            run.id,
            run.run_digest,
            run.pay_period_id,
            PAYMENT_RELEASE_VERSION,
            run.currency,
            ordered,
            sum(
                (
            ...
            "",
            assembled_at,
            supersedes_package_identity,
        )
        digest = canonical_digest(provisional_package.canonical_content())
        result = replace(
            provisional_package,
            package_identity=f"payroll-payment-release:{digest}",
    ...
    async def persist_candidate(
        ...
        *,
        context: AuthorizationContext,
        candidate: PaymentReleaseCandidate,
    ) -> PayrollPaymentReleaseRecord:
        self._require(context, PayrollPermission.PAYMENT_RELEASE_ASSEMBLE)
        candidate.verify()
        ...
        await session.execute(
            ...
            {
                "key": f"payroll-payment-release:{candidate.company_id}:{candidate.payroll_run_id}"
            },
        )
        existing = await session.scalar(
            select(PayrollPaymentReleaseRecord).where(
                PayrollPaymentReleaseRecord.company_id == context.company.id,
                PayrollPaymentReleaseRecord.package_digest == candidate.package_digest,
            )
        )
        if existing:
            return existing
        active = await session.scalar(
            select(PayrollPaymentReleaseRecord)
            .where(
                PayrollPaymentReleaseRecord.company_id == context.company.id,
                PayrollPaymentReleaseRecord.payroll_run_id == candidate.payroll_run_id,
                PayrollPaymentReleaseRecord.lifecycle.in_(
                    ("draft", "under_review", "approved_for_release")
                ),
            )
            .with_for_update()
        )
        ...
        if candidate.supersedes_package_identity:
            prior = await session.scalar(
                ...
                .where(
                    PayrollPaymentReleaseRecord.company_id == context.company.id,
                    PayrollPaymentReleaseRecord.package_identity
                    == candidate.supersedes_package_identity,
                )
                .with_for_update()
            )
            if (
                ...
                or prior.execution_started_at is not None
            ):
                raise PayrollConflictError(
                    "payment release supersession lineage conflict"
                )
            prior.lifecycle = "superseded"
            await session.flush()
        elif active:
            raise PayrollConflictError("active payment release already exists")
        value = PayrollPaymentReleaseRecord(
            company_id=candidate.company_id,
            payroll_run_id=candidate.payroll_run_id,
            payroll_run_digest=candidate.payroll_run_digest,
            pay_period_id=candidate.pay_period_id,
            definition_version=candidate.definition_version,
            currency=candidate.currency,
            package_identity=candidate.package_identity,
            package_digest=candidate.package_digest,
            aggregate_release_amount=candidate.aggregate_release_amount,
            assembled_by_user_id=context.user.id,
            assembled_at=candidate.assembled_at,
            lifecycle="draft",
            review_state="not_started",
            supersedes_release_id=prior.id if prior else None,
        )
        session.add(value)
        await session.flush()
        for item in candidate.instructions:
            session.add(
                PayrollPaymentInstructionRecord(
                    ...
                    run_member_digest=item.run_member_digest,
                    tax_result_id=item.tax_result_id,
                    tax_result_digest=item.tax_result_digest,
                    destination_id=item.destination_id,
                    destination_digest=item.destination_digest,
                    method_type=item.method.value if item.method else None,
                    protected_destination_reference=item.protected_destination_reference,
                    amount=item.amount,
                    currency=item.currency,
                    blocker_evidence_digest=item.blocker_evidence_digest,
                    instruction_identity=item.instruction_identity,
        ...
        if prior:
            self._stage_release(
                ...
                context,
                prior,
                EventType.PAYROLL_PAYMENT_RELEASE_SUPERSEDED,
                "payroll.payment_release.superseded",
            )
        self._stage_release(
            session,
            context,
            value,
            EventType.PAYROLL_PAYMENT_RELEASE_ASSEMBLED,
            "payroll.payment_release.assembled",
        )
        await session.commit()
        return value
    ...
    async def initiate_review(
        ...
        *,
        ...
        record = await self._review(
            session, context, value, PaymentReleaseReviewDecision.INITIATED, reason_code
        )
        self._stage_release(
            session,
            context,
            value,
            EventType.PAYROLL_PAYMENT_RELEASE_REVIEWED,
            "payroll.payment_release.review_initiated",
        )
        await session.commit()
        return record
    ...
    async def decide_review(
        ...
        *,
        context: AuthorizationContext,
        release_id: UUID,
        decision: PaymentReleaseReviewDecision,
        reason_code: str,
    ) -> PayrollPaymentReleaseReviewRecord:
        self._require(context, PayrollPermission.PAYMENT_RELEASE_REVIEW)
        if decision not in {
            PaymentReleaseReviewDecision.ACCEPTED,
            PaymentReleaseReviewDecision.REJECTED,
        }:
        ...
        value.lifecycle, value.review_state = (
            ("draft", "accepted")
            if decision is PaymentReleaseReviewDecision.ACCEPTED
        ...
        self._stage_release(
            ...
            value,
            EventType.PAYROLL_PAYMENT_RELEASE_REVIEWED,
            f"payroll.payment_release.review_{decision.value}",
        )
        await session.commit()
        return record
    ...
    async def approve_release(
        ...
        *,
        ...
        record = await self._review(
            session, context, value, PaymentReleaseReviewDecision.APPROVED, reason_code
        )
        self._stage_release(
            session,
            context,
            value,
            EventType.PAYROLL_PAYMENT_RELEASE_APPROVED,
            "payroll.payment_release.approved",
        )
        await session.commit()
        return record

    async def execution_handoff(
        self, session: AsyncSession, *, context: AuthorizationContext, release_id: UUID
    ) -> PaymentExecutionHandoff:
        self._require(context, PayrollPermission.PAYMENT_RELEASE_READ)
        value = await session.scalar(
            select(PayrollPaymentReleaseRecord).where(
                PayrollPaymentReleaseRecord.company_id == context.company.id,
                PayrollPaymentReleaseRecord.id == release_id,
                PayrollPaymentReleaseRecord.lifecycle == "approved_for_release",
            )
        )
        if value is None:
        ...
        instructions = tuple(
            (
                await session.scalars(
                    ...
                    .where(
                        PayrollPaymentInstructionRecord.release_id == value.id,
                        PayrollPaymentInstructionRecord.disposition == "ready",
                    )
                    .order_by(PayrollPaymentInstructionRecord.employee_id)
                )
            ).all()
        )
        approval = await session.scalar(
            select(PayrollPaymentReleaseReviewRecord.review_digest).where(
                PayrollPaymentReleaseReviewRecord.release_id == value.id,
                PayrollPaymentReleaseReviewRecord.decision == "approved",
            )
        )
        if approval is None:
            raise PayrollConflictError(
                "payment release approval evidence is unavailable"
            )
        identity = canonical_digest(
            {
                ...
                "instruction_identities": tuple(
                    item.instruction_identity for item in instructions
                ),
            }
        )
        return PaymentExecutionHandoff(
            PAYMENT_EXECUTION_HANDOFF_VERSION,
            value.company_id,
            value.pay_period_id,
            value.id,
            ...
            value.currency,
            tuple(item.instruction_identity for item in instructions),
            value.aggregate_release_amount,
            approval,
            f"payment-execution:{identity}",
        )
    ...
    def _resolution(
        employee_id: UUID,
        state: DestinationAdmissionState,
        value: PayrollPaymentDestinationVersion | None = None,
    ) -> DestinationResolution:
        content = {
            "employee_id": str(employee_id),
            "state": state.value,
            "destination_id": str(value.id) if value else None,
            "destination_digest": value.authority_digest if value else None,
            "method": value.method_type if value else None,
            "protected_reference": value.destination_reference if value else None,
        }
        return DestinationResolution(
            employee_id,
            state,
            value.id if value else None,
            value.authority_digest if value else None,
            PaymentMethod(value.method_type) if value else None,
            value.destination_reference if value else None,
    ...
    async def _review(
        ...
        sequence = (
            await session.scalar(
                select(func.count(PayrollPaymentReleaseReviewRecord.id)).where(
                    PayrollPaymentReleaseReviewRecord.release_id == value.id
                )
            )
            or 0
        ...
        record = PayrollPaymentReleaseReviewRecord(
            company_id=value.company_id,
            release_id=value.id,
    ...
    async def _locked_release(
        session: AsyncSession, context: AuthorizationContext, release_id: UUID
    ) -> PayrollPaymentReleaseRecord:
        value = await session.scalar(
            select(PayrollPaymentReleaseRecord)
            .where(
    ...
    def _stage_destination(
        ...
        BusinessEventService.stage(
            ...
            BusinessEventCreate(
                event_type=event,
                entity_type="payroll_payment_destination",
                entity_id=value.id,
                company_id=value.company_id,
        ...
        self._audit.stage(
            ...
            AuditEntry(
                action=action,
                resource_type="payroll_payment_destination",
                actor_user_id=context.user.id,
                company_id=value.company_id,
    ...
    def _stage_release(
        ...
        BusinessEventService.stage(
            ...
            BusinessEventCreate(
                event_type=event,
                entity_type="payroll_payment_release",
                entity_id=value.id,
                company_id=value.company_id,
                user_id=context.user.id,
                payload=details,
        ...
        self._audit.stage(
            ...
            AuditEntry(
                action=action,
                resource_type="payroll_payment_release",
                actor_user_id=context.user.id,
                company_id=value.company_id,
                resource_id=value.id,
                details=details,
