...
class PayrollAccountingFinalizationService:
        ...
        *,
        ...
        existing = await session.scalar(
            select(PayrollAccountingConsumptionRecord).where(
                PayrollAccountingConsumptionRecord.company_id == fact.company_id,
                PayrollAccountingConsumptionRecord.recognition_event
                == fact.event_type.removeprefix("payroll."),
                PayrollAccountingConsumptionRecord.source_event_id
        ...
        if existing:
            if (
                existing.fact_digest != fact_digest
                or existing.candidate_identity != candidate.candidate_identity
            ):
                raise PayrollConflictError(
                    "Payroll Accounting source consumption conflicts"
                )
            return existing
        debits = sum(
            (
                fact.components[leg.component]
                for leg in candidate.posting_rule.legs
    ...
    async def post_candidate(
        ...
        *,
        ...
        consumption = await self.persist_candidate(
            session, context=preparer, candidate=candidate
        )
        if consumption.lifecycle == "posted":
            if consumption.journal_id is None or consumption.posted_at is None:
                raise PayrollConflictError(
                    "posted Payroll Accounting consumption is incomplete"
                )
            return PostingReceipt(
                candidate.fact.company_id,
                candidate.fact.branch_id,
                candidate.fact.source_event_id,
                candidate.fact.source_type,
                candidate.fact.source_id,
                consumption.journal_id,
                consumption.journal_version,
                candidate.posting_rule.version,
                PostingOutcome.POSTED,
                candidate.fact.effective_date,
                consumption.posted_at,
            )
        posting = AutomatedPostingService(
            rules=PostingRuleRegistry((candidate.posting_rule,))
        )
        receipt = await posting.post(
            session,
            fact=candidate.fact,
            period_id=period_id,
            preparer=preparer,
        ...
        value = await session.scalar(
            select(PayrollAccountingConsumptionRecord)
            .where(PayrollAccountingConsumptionRecord.id == consumption.id)
            .with_for_update()
        )
        if value is None:
        ...
        value.lifecycle, value.journal_id, value.journal_version, value.posted_at = (
            "posted",
            receipt.journal_id,
        ...
        details: dict[str, object] = {
            "consumption_id": str(value.id),
            "fact_digest": value.fact_digest,
            "journal_id": str(receipt.journal_id),
            "recognition_event": value.recognition_event,
            "lifecycle": value.lifecycle,
        }
        BusinessEventService.stage(
            session,
            BusinessEventCreate(
                event_type=EventType.PAYROLL_ACCOUNTING_FACT_POSTED,
                entity_type="payroll_accounting_consumption",
                entity_id=value.id,
        ...
        self._audit.stage(
            session,
            AuditEntry(
                action="payroll.accounting_fact.posted",
                resource_type="payroll_accounting_consumption",
                actor_user_id=poster.user.id,
                company_id=value.company_id,
    ...
    async def reconciliation(
        ...
        *,
        ...
        if run is None:
            raise PayrollConflictError(
                "approved Payroll run is required for reconciliation"
            )
        rows = tuple(
            (
                await session.scalars(
                    select(PayrollAccountingConsumptionRecord).where(
                        PayrollAccountingConsumptionRecord.company_id
                        == context.company.id,
        ...
        accrued = sum(
            (row.amount for row in rows if row.recognition_event == "payroll_accrual"),
            Decimal(0),
        )
        wage = sum(
            (row.amount for row in rows if row.recognition_event == "wage_settlement"),
            Decimal(0),
        )
        remittance = sum(
            (
                row.amount
                for row in rows
                if row.recognition_event in {"tax_remittance", "deduction_remittance"}
            ),
            Decimal(0),
        )
        ...
        if outstanding < 0:
            raise PayrollConflictError(
                "Payroll Accounting settlement exceeds approved net pay"
            )
        ...
        digest = canonical_digest(
            {
                ...
                "accrued": str(accrued),
                "wage_settled": str(wage),
                "remittance_settled": str(remittance),
                "outstanding_net_pay": str(outstanding),
                "currency": run.currency,
                "disposition": disposition,
            }
        )
        return PayrollAccountingReconciliation(
            run.id,
            run.currency,
            run.aggregate_net_pay,
            accrued,
            wage,
            remittance,
            outstanding,
