...
class PayrollTaxDeductionResultService:
    ...
    async def persist_candidate(
        ...
        *,
        ...
        if (
            admission.state is TaxDeductionAdmissionState.NOT_APPLICABLE
            and candidate.components
        ):
            raise PayrollConflictError(
                "not-applicable admission cannot persist components"
            )
        if (
        ...
        if candidate.supersedes_result_id:
            prior = await session.scalar(
                select(PayrollTaxDeductionResultRecord)
                .where(
                    PayrollTaxDeductionResultRecord.company_id == context.company.id,
                    PayrollTaxDeductionResultRecord.result_identity
        ...
        value = PayrollTaxDeductionResultRecord(
            ...
            authority_evidence=[
                item.canonical_content() for item in admission.resolutions
            ],
        ...
        if prior:
            self._stage(
                ...
                context,
                prior,
                EventType.PAYROLL_TAX_RESULT_SUPERSEDED,
                "payroll.tax_result.superseded",
            )
        self._stage(
            session,
            context,
    ...
    async def initiate_review(
        ...
        *,
        ...
        if value.lifecycle not in {
            TaxResultLifecycle.CALCULATED.value,
            TaxResultLifecycle.REJECTED.value,
        }:
        ...
        record = await self._review(
            session, context, value, TaxReviewDecision.INITIATED, reason_code, safe_note
        )
        self._stage(
            session,
            context,
            value,
            EventType.PAYROLL_TAX_RESULT_REVIEW_INITIATED,
            "payroll.tax_result.review_initiated",
        )
        await session.commit()
        return record
    ...
    async def decide_review(
        ...
        *,
        ...
        value.lifecycle = (
            TaxResultLifecycle.APPROVED.value
            if accepted
            else TaxResultLifecycle.REJECTED.value
        )
        value.review_state = (
            TaxResultReviewState.ACCEPTED.value
            if accepted
            else TaxResultReviewState.REJECTED.value
        )
        record = await self._review(
            session, context, value, decision, reason_code, safe_note
        )
        event = (
            EventType.PAYROLL_TAX_RESULT_REVIEW_ACCEPTED
            if accepted
            else EventType.PAYROLL_TAX_RESULT_REVIEW_REJECTED
        )
        self._stage(
            session,
            context,
            value,
            event,
            f"payroll.tax_result.review_{decision.value}",
        )
        await session.commit()
        return record
    ...
    async def result(
        ...
        value = await session.scalar(
            select(PayrollTaxDeductionResultRecord).where(
                PayrollTaxDeductionResultRecord.company_id == context.company.id,
                PayrollTaxDeductionResultRecord.id == result_id,
            )
        )
        if value is None:
    ...
    async def period_results(
        ...
        *,
        context: AuthorizationContext,
        pay_period_id: UUID,
        blocked_admissions: tuple[TaxDeductionAdmissionResult, ...] = (),
    ) -> tuple[TaxPeriodResultStatus, ...]:
        self._require(context, PayrollPermission.TAX_RESULT_READ)
        values = (
        ...
        statuses = [
            TaxPeriodResultStatus(
                v.employee_id, v.id, v.lifecycle, v.calculation_digest
            )
            for v in values
        ]
        for admission in blocked_admissions:
            admission.verify()
            if admission.company_id != context.company.id or admission.state not in {
    ...
    async def history(
        ...
        *,
        ...
        employee_id: UUID,
        pay_period_id: UUID,
    ) -> tuple[PayrollTaxDeductionResultRecord, ...]:
        self._require(context, PayrollPermission.TAX_RESULT_READ)
        values = await session.scalars(
            ...
            .order_by(PayrollTaxDeductionResultRecord.created_at)
        )
    ...
    async def _review(
        ...
        sequence = (
            await session.scalar(
                select(func.count(PayrollTaxDeductionReviewRecord.id)).where(
                    PayrollTaxDeductionReviewRecord.result_id == value.id
                )
            )
            or 0
        ) + 1
        ...
        record = PayrollTaxDeductionReviewRecord(
    ...
    @staticmethod
    def _verify_record(
        value: PayrollTaxDeductionResultRecord, supersedes_identity: str | None
    ) -> None:
        components = tuple(
            TaxDeductionComponent(
                component_key=str(c["component_key"]),
                kind=ComponentKind(str(c["kind"])),
                responsibility=str(c["responsibility"]),
                authority_id=UUID(str(c["authority_id"])),
                authority_digest=str(c["authority_digest"]),
                provider_id=str(c["provider_id"]) if c.get("provider_id") else None,
                provider_version=str(c["provider_version"])
                if c.get("provider_version")
                else None,
                jurisdiction_reference=str(c["jurisdiction_reference"])
                if c.get("jurisdiction_reference")
                else None,
                calculation_basis=str(c["calculation_basis"]),
                basis_amount=Decimal(str(c["basis_amount"])),
                amount=Decimal(str(c["amount"])),
                currency=str(c["currency"]),
                priority=int(cast(int | str, c["priority"])),
                rounding_rule=str(c["rounding_rule"]),
                evidence_digest=str(c["evidence_digest"]),
            )
            for c in value.components
        )
        TaxDeductionCalculationResult(
            result_id=value.result_identity,
            definition_version=value.calculation_version,
            company_id=value.company_id,
            employee_id=value.employee_id,
            pay_period_id=value.pay_period_id,
            gross_result_id=value.gross_result_id,
            gross_calculation_digest=value.gross_calculation_digest,
            currency=value.currency,
            admission_digest=value.admission_digest,
            components=components,
            gross_pay=value.gross_pay,
            total_employee_taxes=value.employee_tax_total,
            total_employee_deductions=value.employee_deduction_total,
            total_employer_contributions=value.employer_contribution_total,
            net_pay_candidate=value.net_pay_candidate,
            money_version=value.money_version,
            calculation_digest=value.calculation_digest,
            calculated_at=value.calculated_at,
            supersedes_result_id=supersedes_identity,
        ).verify()

    async def _predecessor_identity(
        self, session: AsyncSession, value: PayrollTaxDeductionResultRecord
    ) -> str | None:
        if value.supersedes_result_id is None:
            return None
        return await session.scalar(
            select(PayrollTaxDeductionResultRecord.result_identity).where(
                PayrollTaxDeductionResultRecord.id == value.supersedes_result_id
            )
        )
    ...
    async def _subject_lock(
        session: AsyncSession, company_id: UUID, employee_id: UUID, pay_period_id: UUID
    ) -> None:
        await session.execute(
            text("SELECT pg_advisory_xact_lock(hashtextextended(:key, 0))"),
            {"key": f"payroll-tax:{company_id}:{employee_id}:{pay_period_id}"},
        )

    @staticmethod
    def _active_query(
        candidate: TaxDeductionCalculationResult,
    ) -> Select[tuple[PayrollTaxDeductionResultRecord]]:
        return select(PayrollTaxDeductionResultRecord).where(
            PayrollTaxDeductionResultRecord.company_id == candidate.company_id,
            PayrollTaxDeductionResultRecord.employee_id == candidate.employee_id,
            PayrollTaxDeductionResultRecord.pay_period_id == candidate.pay_period_id,
            PayrollTaxDeductionResultRecord.lifecycle.in_(
    ...
    def _stage(
        ...
        BusinessEventService.stage(
            ...
            BusinessEventCreate(
                ...
                entity_type="payroll_tax_deduction_result",
                entity_id=value.id,
                company_id=context.company.id,
        ...
        self._audit.stage(
            ...
            AuditEntry(
                action=action,
                resource_type="payroll_tax_deduction_result",
