...
class PayrollCompensationProrationPolicyVersion(Base):
    __tablename__ = "payroll_compensation_proration_policy_versions"
    __table_args__ = (
        CheckConstraint("policy_version >= 1", name="ck_payroll_proration_version"),
        CheckConstraint(
        ...
        UniqueConstraint("company_id", "id", name="uq_payroll_proration_company_id"),
        UniqueConstraint("supersedes_policy_id", name="uq_payroll_proration_successor"),
        UniqueConstraint("policy_digest", name="uq_payroll_proration_digest"),
    )
    ...
    supersedes_policy_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey(
            "payroll_compensation_proration_policy_versions.id", ondelete="RESTRICT"
        ),
    )
...
class PayrollTaxDeductionResultRecord(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "company_id", "result_identity", name="uq_payroll_tax_result_identity"
        ),
        UniqueConstraint(
            "company_id", "calculation_digest", name="uq_payroll_tax_result_digest"
        ),
        UniqueConstraint(
            "supersedes_result_id", name="uq_payroll_tax_result_successor"
        ),
        Index(
        ...
        Index(
            "ix_payroll_tax_result_period", "company_id", "pay_period_id", "employee_id"
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    ...
    currency: Mapped[str] = mapped_column(String(3), nullable=False)
    admission_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    authority_evidence: Mapped[list[dict[str, object]]] = mapped_column(
        JSONB, nullable=False
    )
    ...
    employee_deduction_total: Mapped[Decimal] = mapped_column(
        Numeric(18, 2), nullable=False
    )
    employer_contribution_total: Mapped[Decimal] = mapped_column(
        Numeric(18, 2), nullable=False
    )
    ...
    supersedes_result_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_tax_deduction_results.id", ondelete="RESTRICT"),
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=utc_now
    )
...
class PayrollTaxDeductionReviewRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "decision IN ('initiated','accepted','rejected')",
            name="ck_payroll_tax_review_decision",
        ),
        ForeignKeyConstraint(
            ["company_id", "result_id"],
            [
                "payroll_tax_deduction_results.company_id",
                "payroll_tax_deduction_results.id",
            ],
            ondelete="RESTRICT",
        ),
        UniqueConstraint(
            "result_id", "review_sequence", name="uq_payroll_tax_review_sequence"
        ),
...
class PayrollRunRecord(Base):
    ...
    pay_period_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("timekeeping_pay_periods.id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    aggregate_employee_taxes: Mapped[Decimal] = mapped_column(
        Numeric(18, 2), nullable=False
    )
    aggregate_employee_deductions: Mapped[Decimal] = mapped_column(
        Numeric(18, 2), nullable=False
    )
    aggregate_net_pay: Mapped[Decimal] = mapped_column(Numeric(18, 2), nullable=False)
    aggregate_employer_contributions: Mapped[Decimal] = mapped_column(
        Numeric(18, 2), nullable=False
    )
    ...
    consumed_by_payment_at: Mapped[datetime | None] = mapped_column(
...
class PayrollRunMemberRecord(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "run_id", "employee_id", name="uq_payroll_run_member_employee"
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class PayrollRunReviewRecord(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "run_id", "review_sequence", name="uq_payroll_run_review_sequence"
        ),
...
class PayrollPaymentDestinationVersion(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "company_id", "id", name="uq_payroll_payment_destination_company_id"
        ),
        UniqueConstraint(
            "company_id",
            "employee_id",
            "destination_version",
            name="uq_payroll_payment_destination_version",
        ),
        Index(
            "ix_payroll_payment_destination_resolution",
            "company_id",
            "employee_id",
            "lifecycle",
            "effective_start",
    ...
    verification_evidence_digest: Mapped[str] = mapped_column(
    ...
    lifecycle: Mapped[str] = mapped_column(String(20), nullable=False)
    authority_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    supersedes_destination_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_payment_destination_versions.id", ondelete="RESTRICT"),
    )
    created_by_user_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("users.id", ondelete="RESTRICT"),
...
class PayrollPaymentReleaseRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('draft','under_review','approved_for_release','rejected','superseded','voided')",
            name="ck_payroll_payment_release_lifecycle",
        ),
        CheckConstraint(
            "review_state IN ('not_started','under_review','accepted','rejected')",
            name="ck_payroll_payment_release_review_state",
        ),
        ForeignKeyConstraint(
        ...
        UniqueConstraint(
            "company_id", "id", name="uq_payroll_payment_release_company_id"
        ),
        UniqueConstraint(
            "company_id", "package_identity", name="uq_payroll_payment_release_identity"
        ),
        UniqueConstraint(
            "company_id", "package_digest", name="uq_payroll_payment_release_digest"
        ),
        UniqueConstraint(
            "supersedes_release_id", name="uq_payroll_payment_release_successor"
        ),
        Index(
            "uq_payroll_payment_release_active_run",
            "company_id",
            "payroll_run_id",
            unique=True,
            postgresql_where=text(
    ...
    aggregate_release_amount: Mapped[Decimal] = mapped_column(
        Numeric(18, 2), nullable=False
    )
    assembled_by_user_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
    ...
    supersedes_release_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_payment_releases.id", ondelete="RESTRICT"),
    )
...
class PayrollPaymentInstructionRecord(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["payroll_payment_releases.company_id", "payroll_payment_releases.id"],
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["company_id", "employee_id"],
        ...
        UniqueConstraint(
            "release_id", "employee_id", name="uq_payroll_payment_instruction_employee"
        ),
        UniqueConstraint(
            "company_id",
            "instruction_identity",
            name="uq_payroll_payment_instruction_identity",
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class PayrollPaymentReleaseReviewRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "decision IN ('initiated','accepted','rejected','approved')",
            name="ck_payroll_payment_release_review_decision",
        ),
        ForeignKeyConstraint(
            ["company_id", "release_id"],
            ["payroll_payment_releases.company_id", "payroll_payment_releases.id"],
        ...
        UniqueConstraint(
            "release_id",
            "review_sequence",
            name="uq_payroll_payment_release_review_sequence",
        ),
        UniqueConstraint(
            "review_digest", name="uq_payroll_payment_release_review_digest"
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class PayrollPaymentExecutionRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('authorized','submission_pending','submitted','provider_acknowledged','settlement_pending','partially_settled','settled','rejected','failed','canceled','uncertain')",
            name="ck_payroll_payment_execution_lifecycle",
        ),
        ForeignKeyConstraint(
            ["company_id", "release_id"],
            ["payroll_payment_releases.company_id", "payroll_payment_releases.id"],
        ...
        UniqueConstraint(
            "company_id", "id", name="uq_payroll_payment_execution_company_id"
        ),
        UniqueConstraint(
            "company_id",
            "execution_identity",
            name="uq_payroll_payment_execution_identity",
        ),
        UniqueConstraint(
            "company_id", "execution_digest", name="uq_payroll_payment_execution_digest"
        ),
        Index(
            "uq_payroll_payment_execution_active_release",
            "company_id",
            "release_id",
            unique=True,
            postgresql_where=text(
                "lifecycle IN ('authorized','submission_pending','submitted','provider_acknowledged','settlement_pending','partially_settled','uncertain')"
            ),
        ),
    )
    id: Mapped[UUID] = mapped_column(
...
class PayrollPaymentExecutionItemRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('authorized','submitted','acknowledged','settlement_pending','settled','rejected','failed','unresolved')",
            name="ck_payroll_payment_execution_item_lifecycle",
        ),
        ForeignKeyConstraint(
            ["company_id", "execution_id"],
            ["payroll_payment_executions.company_id", "payroll_payment_executions.id"],
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["instruction_id"], ["payroll_payment_instructions.id"], ondelete="RESTRICT"
        ),
        UniqueConstraint(
            "execution_id",
            "instruction_id",
            name="uq_payroll_payment_execution_item_instruction",
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class PayrollPaymentExecutionEvidenceRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "evidence_type IN ('submission','acknowledgement','settlement','failure','uncertain')",
            name="ck_payroll_payment_execution_evidence_type",
        ),
        ForeignKeyConstraint(
            ["company_id", "execution_id"],
            ["payroll_payment_executions.company_id", "payroll_payment_executions.id"],
        ...
        UniqueConstraint(
            "execution_id",
            "evidence_digest",
            name="uq_payroll_payment_execution_evidence_digest",
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class PayrollAccountingPolicyVersion(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_payroll_accounting_policy_event",
        ),
        CheckConstraint(
            "lifecycle IN ('draft','approved','superseded','retired')",
            name="ck_payroll_accounting_policy_lifecycle",
        ),
        CheckConstraint(
            "effective_end IS NULL OR effective_end > effective_start",
            name="ck_payroll_accounting_policy_interval",
        ),
        UniqueConstraint(
            "company_id",
            "recognition_event",
            "policy_version",
            name="uq_payroll_accounting_policy_version",
        ),
        Index(
            "ix_payroll_accounting_policy_resolution",
            "company_id",
            "recognition_event",
            "lifecycle",
            "effective_start",
    ...
    supersedes_policy_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_accounting_policy_versions.id", ondelete="RESTRICT"),
        unique=True,
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=utc_now
...
class PayrollAccountingMappingVersion(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "recognition_event IN ('payroll_accrual','payment_release','wage_settlement','tax_remittance','deduction_remittance','return_adjustment','adjustment_applied')",
            name="ck_payroll_accounting_mapping_event",
        ),
        CheckConstraint(
            "posting_side IN ('debit','credit')",
            name="ck_payroll_accounting_mapping_side",
        ),
        CheckConstraint(
            "lifecycle IN ('draft','approved','superseded','retired')",
            name="ck_payroll_accounting_mapping_lifecycle",
        ),
        CheckConstraint(
            "effective_end IS NULL OR effective_end > effective_start",
            name="ck_payroll_accounting_mapping_interval",
        ),
        ForeignKeyConstraint(
        ...
        UniqueConstraint(
            ...
            "component",
            "mapping_version",
            name="uq_payroll_accounting_mapping_version",
        ),
        Index(
            "ix_payroll_accounting_mapping_resolution",
            "company_id",
            "recognition_event",
            "component",
            "lifecycle",
    ...
    supersedes_mapping_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_accounting_mapping_versions.id", ondelete="RESTRICT"),
        unique=True,
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=utc_now
...
class PayrollPayStatementRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('created','issued','superseded','voided')",
            name="ck_payroll_pay_statement_lifecycle",
        ),
        CheckConstraint(
            "payment_status IN ('not_available','pending','acknowledged','partially_settled','settled','failed','unresolved')",
            name="ck_payroll_pay_statement_payment_status",
        ),
        ForeignKeyConstraint(
            ["company_id", "run_id"],
            ["payroll_runs.company_id", "payroll_runs.id"],
        ...
        UniqueConstraint(
            "company_id", "statement_identity", name="uq_payroll_pay_statement_identity"
        ),
        UniqueConstraint(
            "company_id", "id", name="uq_payroll_pay_statement_company_id"
        ),
        UniqueConstraint(
            "company_id", "statement_digest", name="uq_payroll_pay_statement_digest"
        ),
        UniqueConstraint(
            "supersedes_statement_id", name="uq_payroll_pay_statement_successor"
        ),
        Index(
            "ix_payroll_pay_statement_employee_period",
            "company_id",
            "employee_id",
            "pay_period_id",
            "created_at",
    ...
    adjustment_result_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_adjustment_results.id", ondelete="RESTRICT"),
    )
    adjustment_digest: Mapped[str | None] = mapped_column(String(64))
    reporting_snapshot_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_reporting_snapshots.id", ondelete="RESTRICT"),
    )
    ...
    lifecycle: Mapped[str] = mapped_column(String(16), nullable=False)
    supersedes_statement_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_pay_statements.id", ondelete="RESTRICT"),
    )
...
class PayrollPayStatementArtifactRecord(Base):
    ...
    retention_state: Mapped[str] = mapped_column(
        String(24), nullable=False, default="preserve"
    )
    created_by_user_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("users.id", ondelete="RESTRICT"),
...
class PayrollHistoryCoverageRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('draft','approved','superseded','retired')",
            name="ck_payroll_history_coverage_lifecycle",
        ),
        CheckConstraint(
            "coverage_end >= coverage_start",
            name="ck_payroll_history_coverage_interval",
        ),
        UniqueConstraint(
            "company_id",
            "coverage_identity",
            name="uq_payroll_history_coverage_identity",
        ),
        Index(
            "ix_payroll_history_coverage_resolution",
            "company_id",
            "lifecycle",
            "coverage_start",
            "coverage_end",
...
class PayrollReportingSnapshotRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "period_kind IN ('pay_period','quarter','year')",
            name="ck_payroll_reporting_period_kind",
        ),
        CheckConstraint(
            "state IN ('authoritative','partial','unavailable','conflicting')",
            name="ck_payroll_reporting_state",
        ),
        UniqueConstraint(
            "company_id", "report_identity", name="uq_payroll_reporting_identity"
        ),
        Index(
            "ix_payroll_reporting_period",
            "company_id",
            "period_start",
            "period_end",
            "employee_id",
...
class PayrollFilingPackageRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "state IN ('prepared_not_submitted','superseded','voided')",
            name="ck_payroll_filing_package_state",
        ),
        UniqueConstraint(
            "company_id", "package_identity", name="uq_payroll_filing_package_identity"
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    ...
    reporting_snapshot_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_reporting_snapshots.id", ondelete="RESTRICT"),
        nullable=False,
    )
    compliance_schema_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_compliance_schemas.id", ondelete="RESTRICT"),
    )
    ...
    supersedes_package_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_filing_packages.id", ondelete="RESTRICT"),
    )
...
class PayrollAdjustmentAuthorityRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "classification IN ('pre_payment_payroll_correction','retroactive_earnings','off_cycle_payroll','tax_correction','deduction_correction','payment_return','payment_rejection','payment_reversal','settlement_correction','accounting_adjustment_required')",
            name="ck_payroll_adjustment_classification",
        ),
        CheckConstraint(
            "lifecycle IN ('draft','under_review','approved','applied_to_successor_authority','rejected','superseded','voided')",
            name="ck_payroll_adjustment_lifecycle",
        ),
        UniqueConstraint("company_id", "id", name="uq_payroll_adjustment_company_id"),
        UniqueConstraint(
            "company_id", "adjustment_identity", name="uq_payroll_adjustment_identity"
        ),
        UniqueConstraint(
            "company_id", "adjustment_digest", name="uq_payroll_adjustment_digest"
        ),
        UniqueConstraint(
            "supersedes_adjustment_id", name="uq_payroll_adjustment_successor"
        ),
        Index(
            "uq_payroll_adjustment_active_subject",
            "company_id",
            "source_type",
            "source_id",
            "classification",
            unique=True,
            postgresql_where=text("lifecycle IN ('draft','under_review','approved')"),
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    ...
    source_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    source_evidence: Mapped[dict[str, object]] = mapped_column(JSONB, nullable=False)
    delta_components: Mapped[list[dict[str, object]]] = mapped_column(
        JSONB, nullable=False
    )
    currency: Mapped[str] = mapped_column(String(3), nullable=False)
    ...
    approved_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    supersedes_adjustment_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_adjustment_authorities.id", ondelete="RESTRICT"),
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=utc_now
    )
...
class PayrollAdjustmentReviewRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "decision IN ('initiated','accepted','rejected','approved')",
            name="ck_payroll_adjustment_review_decision",
        ),
        ForeignKeyConstraint(
        ...
        UniqueConstraint(
            "adjustment_id", "sequence", name="uq_payroll_adjustment_review_sequence"
        ),
...
class PayrollAdjustmentApplicationRecord(Base):
    ...
    result_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    authorized_components: Mapped[list[dict[str, object]]] = mapped_column(
        JSONB, nullable=False
    )
...
class PayrollRemittancePolicyRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('draft','approved','superseded','retired')",
            name="ck_payroll_remittance_policy_lifecycle",
        ),
        UniqueConstraint(
            "company_id",
            "classification",
            "version",
            name="uq_payroll_remittance_policy_version",
        ),
        UniqueConstraint(
            "company_id", "policy_digest", name="uq_payroll_remittance_policy_digest"
        ),
        Index(
            "ix_payroll_remittance_policy_resolve",
            "company_id",
            "classification",
            "lifecycle",
            "effective_start",
...
class PayrollRemittanceDestinationRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('draft','approved','superseded','revoked','expired')",
            name="ck_payroll_remittance_destination_lifecycle",
        ),
        UniqueConstraint(
            "company_id",
            "destination_identity",
            name="uq_payroll_remittance_destination_identity",
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class PayrollRemittanceObligationRecord(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "lifecycle IN ('identified','ready_for_review','under_review','approved_for_remittance','instruction_prepared','submission_pending','submitted','provider_acknowledged','settlement_pending','partially_settled','settled','blocked','rejected','failed','uncertain','superseded','voided','returned','reversed')",
            name="ck_payroll_remittance_obligation_lifecycle",
        ),
        ForeignKeyConstraint(
            ["company_id", "payroll_run_id"],
            ["payroll_runs.company_id", "payroll_runs.id"],
        ...
        UniqueConstraint(
            "company_id", "id", name="uq_payroll_remittance_obligation_company_id"
        ),
        UniqueConstraint(
            "company_id",
            "obligation_identity",
            name="uq_payroll_remittance_obligation_identity",
        ),
        UniqueConstraint(
            "company_id",
            "obligation_digest",
            name="uq_payroll_remittance_obligation_digest",
        ),
        UniqueConstraint(
            "supersedes_obligation_id",
            name="uq_payroll_remittance_obligation_successor",
        ),
        Index(
            "uq_payroll_remittance_active_source",
            "company_id",
            "payroll_run_id",
            "classification",
            unique=True,
            postgresql_where=text(
                "lifecycle NOT IN ('superseded','voided','returned','reversed')"
            ),
        ),
    )
    id: Mapped[UUID] = mapped_column(
    ...
    classification: Mapped[str] = mapped_column(String(48), nullable=False)
    source_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    contribution_evidence: Mapped[list[dict[str, object]]] = mapped_column(
        JSONB, nullable=False
    )
    policy_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_remittance_policies.id", ondelete="RESTRICT"),
        nullable=False,
    )
    policy_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    destination_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_remittance_destinations.id", ondelete="RESTRICT"),
    )
    destination_digest: Mapped[str | None] = mapped_column(String(64))
    ...
    supersedes_obligation_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("payroll_remittance_obligations.id", ondelete="RESTRICT"),
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, default=utc_now
    )
...
class PayrollRemittanceReviewRecord(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            [
                "payroll_remittance_obligations.company_id",
                "payroll_remittance_obligations.id",
            ],
            ondelete="RESTRICT",
        ),
        UniqueConstraint(
            "obligation_id", "sequence", name="uq_payroll_remittance_review_sequence"
        ),
        UniqueConstraint("review_digest", name="uq_payroll_remittance_review_digest"),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    )
...
class PayrollRemittanceInstructionRecord(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "company_id", "id", name="uq_payroll_remittance_instruction_company_id"
        ),
        UniqueConstraint(
            "obligation_id", name="uq_payroll_remittance_instruction_obligation"
        ),
        UniqueConstraint(
            "instruction_digest", name="uq_payroll_remittance_instruction_digest"
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class PayrollRemittanceEvidenceRecord(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            [
                "payroll_remittance_instructions.company_id",
                "payroll_remittance_instructions.id",
            ],
            ondelete="RESTRICT",
        ),
        UniqueConstraint(
            "instruction_id",
            "evidence_digest",
            name="uq_payroll_remittance_evidence_digest",
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
