...
class SyntheticPaymentExecutionProvider:
    ...
    def __init__(
        ...
        *,
        environment: str,
        result: ProviderResultState = ProviderResultState.ACCEPTED,
    ) -> None:
        if environment != "test":
...
class ExecutionCandidate:
    ...
    def canonical_content(self) -> dict[str, object]:
        return {
            ...
            "instruction_ids": tuple(str(value) for value in self.instruction_ids),
            "instruction_digests": self.instruction_digests,
    ...
    def verify(self) -> None:
        ...
        if (
            ...
            or self.execution_identity != f"payroll-payment-execution:{digest}"
        ):
...
class PayrollPaymentExecutionService:
        ...
        *,
        ...
        if release is None:
            raise PayrollConflictError(
                "approved-for-release payment package is required"
            )
        instructions = tuple(
            (
                await session.scalars(
                    ...
                    .where(
                        PayrollPaymentInstructionRecord.company_id
                        == context.company.id,
                        PayrollPaymentInstructionRecord.release_id == release.id,
                        PayrollPaymentInstructionRecord.disposition == "ready",
                    )
                    .order_by(PayrollPaymentInstructionRecord.id)
                )
            ).all()
        )
        if not instructions or any(
            not item.protected_destination_reference for item in instructions
        ):
            raise PayrollConflictError(
                "release-ready payment instructions are required"
            )
        total = sum((item.amount for item in instructions), Decimal(0))
        if total != release.aggregate_release_amount:
            raise PayrollConflictError(
                "payment instruction total does not match approved release"
            )
        provisional = ExecutionCandidate(
            release.company_id,
            release.id,
            release.payroll_run_id,
            release.package_digest,
            provider_identity,
            provider_version,
            idempotency_identity,
            release.currency,
            tuple(item.id for item in instructions),
            tuple(item.instruction_digest for item in instructions),
            total,
            "",
            "",
        )
        ...
        return ExecutionCandidate(
            **{
                **provisional.__dict__,
                "execution_identity": f"payroll-payment-execution:{digest}",
    ...
    async def authorize(
        ...
        *,
        ...
        await session.execute(
            ...
            {
                "key": f"payroll-payment-execution:{candidate.company_id}:{candidate.release_id}"
            },
        )
        existing = await session.scalar(
            select(PayrollPaymentExecutionRecord).where(
                PayrollPaymentExecutionRecord.company_id == context.company.id,
                PayrollPaymentExecutionRecord.execution_digest
                == candidate.execution_digest,
            )
        )
        if existing:
            return existing
        active = await session.scalar(
            select(PayrollPaymentExecutionRecord).where(
                PayrollPaymentExecutionRecord.company_id == context.company.id,
                PayrollPaymentExecutionRecord.release_id == candidate.release_id,
        ...
        release = await session.scalar(
            ...
            .where(
                PayrollPaymentReleaseRecord.company_id == context.company.id,
                PayrollPaymentReleaseRecord.id == candidate.release_id,
        ...
        instructions = tuple(
            (
                await session.scalars(
                    ...
                    .where(
                        PayrollPaymentInstructionRecord.release_id == release.id,
                        PayrollPaymentInstructionRecord.id.in_(
                            candidate.instruction_ids
                        ),
                    )
                    .order_by(PayrollPaymentInstructionRecord.id)
                )
        ...
        if (
            tuple(item.instruction_digest for item in instructions)
            != candidate.instruction_digests
            or sum((item.amount for item in instructions), Decimal(0))
            != candidate.authorized_total
        ):
        ...
        value = PayrollPaymentExecutionRecord(
            ...
            execution_idempotency_key=candidate.idempotency_identity,
            execution_identity=candidate.execution_identity,
            execution_digest=candidate.execution_digest,
            currency=candidate.currency,
            authorized_total=candidate.authorized_total,
        ...
        self._stage(
            ...
            context,
            value,
            EventType.PAYROLL_PAYMENT_EXECUTION_AUTHORIZED,
            "payroll.payment_execution.authorized",
        )
    ...
    async def submit(
        ...
        *,
        ...
        items = tuple(
            (
                await session.scalars(
                    ...
                    .where(PayrollPaymentExecutionItemRecord.execution_id == value.id)
                    .order_by(PayrollPaymentExecutionItemRecord.instruction_id)
                )
            ).all()
        )
        source = tuple(
            (
                await session.scalars(
                    ...
                    .where(
                        PayrollPaymentInstructionRecord.id.in_(
                            tuple(item.instruction_id for item in items)
                        )
                    )
                    .order_by(PayrollPaymentInstructionRecord.id)
                )
        ...
        request_items = tuple(
            ProviderInstruction(
                item.instruction_identity,
                item.instruction_digest,
                item.protected_destination_reference or "",
                item.amount,
                item.currency,
            )
            for item in source
        ...
        request_digest = canonical_digest(
            {
                ...
                "instructions": tuple(
                    (
                        ...
                        str(item.amount),
                        item.currency,
                        item.protected_destination_reference,
                    )
        ...
        request = ProviderExecutionRequest(
            ...
            value.execution_idempotency_key,
    ...
    async def record_acknowledgement(
        ...
        *,
        ...
        value = await self._locked(session, context, execution_id)
        existing = await session.scalar(
            select(PayrollPaymentExecutionEvidenceRecord).where(
                PayrollPaymentExecutionEvidenceRecord.execution_id == value.id,
                PayrollPaymentExecutionEvidenceRecord.response_digest
        ...
        if (
            ...
            and value.request_digest != acknowledgement.request_digest
        ):
            raise PayrollConflictError("contradictory provider acknowledgement")
        if (
            value.response_digest
            and value.response_digest != acknowledgement.response_digest
        ):
            raise PayrollConflictError("contradictory provider acknowledgement")
        lifecycle = {
            ProviderResultState.ACCEPTED: "provider_acknowledged",
            ProviderResultState.PENDING: "submitted",
            ProviderResultState.REJECTED: "rejected",
            ProviderResultState.UNCERTAIN: "uncertain",
        ...
        evidence = self._evidence(
            ...
            value,
            "uncertain"
            if acknowledgement.state is ProviderResultState.UNCERTAIN
            ...
            (
                await session.scalars(
                    select(PayrollPaymentExecutionItemRecord).where(
                        PayrollPaymentExecutionItemRecord.execution_id == value.id
                    )
                )
            ).all()
        )
        item_state = (
            ...
            if acknowledgement.state is ProviderResultState.REJECTED
            else "acknowledged"
        )
        for item in items:
            item.lifecycle, item.provider_safe_reference, item.evidence_digest = (
                item_state,
                acknowledgement.provider_safe_reference,
                evidence.evidence_digest,
            )
        event = (
            EventType.PAYROLL_PAYMENT_EXECUTION_FAILED
            if acknowledgement.state
            in {ProviderResultState.REJECTED, ProviderResultState.UNCERTAIN}
            else EventType.PAYROLL_PAYMENT_EXECUTION_ACKNOWLEDGED
        )
        self._stage(
            session, context, value, event, f"payroll.payment_execution.{lifecycle}"
        )
        await session.commit()
        return value
    ...
    async def record_settlement(
        ...
        *,
        context: AuthorizationContext,
        execution_id: UUID,
        outcomes: tuple[SettlementItemEvidence, ...],
        ...
        if value.lifecycle not in {
            ...
            raise PayrollConflictError(
                "acknowledged execution is required for settlement"
            )
        items = tuple(
            (
                await session.scalars(
        ...
        if {item.instruction_id for item in items} != {
            item.instruction_id for item in outcomes
        }:
        ...
        allowed = {
            InstructionExecutionState.SETTLED,
            InstructionExecutionState.FAILED,
            InstructionExecutionState.REJECTED,
            InstructionExecutionState.UNRESOLVED,
            InstructionExecutionState.SETTLEMENT_PENDING,
        }
        ...
        for item in items:
            ...
            expected = canonical_digest(
                {
                    "instruction_id": str(outcome.instruction_id),
                    "state": outcome.state.value,
                    "provider_safe_reference": outcome.provider_safe_reference,
                }
            )
            ...
            item.lifecycle, item.provider_safe_reference, item.evidence_digest = (
                outcome.state.value,
                outcome.provider_safe_reference,
        ...
        response_digest = canonical_digest(
            tuple(sorted(item.evidence_digest for item in outcomes))
        )
        self._evidence(
            session,
            context,
            value,
            "settlement"
            if value.lifecycle in {"settled", "partially_settled", "settlement_pending"}
            else "failure",
            value.provider_reference,
            value.request_digest or canonical_digest({}),
            response_digest,
            occurred_at,
        )
        self._stage(
            ...
            context,
            value,
            EventType.PAYROLL_PAYMENT_SETTLEMENT_RECORDED
            if value.lifecycle != "failed"
            else EventType.PAYROLL_PAYMENT_EXECUTION_FAILED,
            f"payroll.payment_execution.{value.lifecycle}",
        )
        await session.commit()
        return value
        ...
        evidence = tuple(
            (
                await session.scalars(
                    ...
                    .where(
                        PayrollPaymentExecutionEvidenceRecord.execution_id == value.id,
                        PayrollPaymentExecutionEvidenceRecord.evidence_type
                        == "settlement",
                    )
                    .order_by(PayrollPaymentExecutionEvidenceRecord.occurred_at)
                )
            ).all()
        )
        return AccountingSettlementHandoff(
            SETTLEMENT_HANDOFF_VERSION,
            value.company_id,
    ...
    def _evidence(
        ...
        record = PayrollPaymentExecutionEvidenceRecord(
            company_id=value.company_id,
            execution_id=value.id,
            evidence_type=evidence_type,
            provider_identity=value.provider_identity,
            provider_version=value.provider_version,
            provider_safe_reference=provider_reference,
            request_digest=request_digest,
            response_digest=response_digest,
    ...
    @staticmethod
    async def _locked(
        session: AsyncSession, context: AuthorizationContext, execution_id: UUID
    ) -> PayrollPaymentExecutionRecord:
        value = await session.scalar(
            select(PayrollPaymentExecutionRecord)
            .where(
                PayrollPaymentExecutionRecord.company_id == context.company.id,
                PayrollPaymentExecutionRecord.id == execution_id,
            )
    ...
    def _require(context: AuthorizationContext, permission: str) -> None:
        if not context.has_permission(permission):
            raise PayrollAuthorizationError(
                "Payroll payment execution permission denied"
            )
    ...
    def _stage(
        ...
        BusinessEventService.stage(
            ...
            BusinessEventCreate(
                event_type=event,
                entity_type="payroll_payment_execution",
                entity_id=value.id,
                company_id=value.company_id,
        ...
        self._audit.stage(
            ...
            AuditEntry(
                action=action,
                resource_type="payroll_payment_execution",
