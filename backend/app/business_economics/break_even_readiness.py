...
def build_break_even_input_readiness(
    ...
    *,
    ...
    reconciliation_keys = {item.reconciliation_key for item in economic_evidence}
    if len(reconciliation_keys) > 1:
        raise ValueError("mixed reconciliation periods are not aggregable")
    semantic_values = {
        (item.component, item.value_digest) for item in economic_evidence
    }
    if len(semantic_values) != len(economic_evidence):
        raise ValueError("duplicate break-even economic fact")
