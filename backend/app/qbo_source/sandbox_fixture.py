...
class SandboxFixtureService:
    ...
    async def qualify(self) -> SandboxFixtureResult:
        ...
        if not existing.exists():
            return SandboxFixtureResult(
                ...
                expected_ledger_digest=str(
                    expected_economic_manifest()["manifest_sha256"]
                ),
                counts={},
                created=0,
                reused=0,
    ...
    async def create(self) -> SandboxFixtureResult:
        ...
        account_specs: tuple[tuple[str, dict[str, object]], ...] = (
            (
                "account-bank",
                _account("ACP Qualification Operating Checking", "Bank", "Checking"),
            ),
            (
                "account-bank-secondary",
                _account("ACP Qualification Reserve Savings", "Bank", "Savings"),
            ),
            (
                "account-ar",
                _account(
                    "ACP Qualification Accounts Receivable",
                    "Accounts Receivable",
                    "AccountsReceivable",
                ),
            ),
            (
                "account-ap",
                _account(
                    "ACP Qualification Accounts Payable",
                    "Accounts Payable",
                    "AccountsPayable",
                ),
            ),
            (
                ...
                _account(
                    "ACP Qualification Service Revenue", "Income", "ServiceFeeIncome"
                ),
            ),
            (
                "account-product-income",
                _account(
                    "ACP Qualification Material Revenue",
                    "Income",
            ...
            (
                "account-cogs",
                _account(
                    "ACP Qualification Materials COGS",
                    "Cost of Goods Sold",
            ...
            (
                "account-expense",
                _account(
                    "ACP Qualification Operating Expense",
                    "Expense",
            ...
            (
                ...
                _account(
                    "ACP Qualification Payroll Expense", "Expense", "PayrollExpenses"
                ),
            ),
            (
                "account-payroll-liability",
                _account(
                    "ACP Qualification Payroll Liability",
            ...
            (
                ...
                _account(
                    "ACP Qualification Accumulated Depreciation",
                    "Fixed Asset",
                    "OtherFixedAssets",
                ),
            ),
            (
                "account-equity",
                _account("ACP Qualification Owner Equity", "Equity", "OwnersEquity"),
            ),
            (
                "account-credit-card",
                _account("ACP Qualification Credit Card", "Credit Card", "CreditCard"),
            ),
        )
        ...
        refs = {
            ...
            "bank_secondary": str(fixture_accounts["account-bank-secondary"]["Id"]),
        ...
        customer_specs: tuple[tuple[str, dict[str, object]], ...] = (
            (
                "customer-active",
                {
                    "DisplayName": "ACP QBO Qualification Customer 001",
                    "Notes": FIXTURE_TAG,
                },
            ),
            (
                "customer-history",
                {
                    "DisplayName": "ACP QBO Qualification Customer 002",
                    "Notes": FIXTURE_TAG,
                },
            ),
        )
        ...
        vendor, was_created = await self._ensure_named(
            ...
            "vendor-primary",
            {
                "DisplayName": "ACP QBO Qualification Vendor 001",
                "CompanyName": "ACP Synthetic Qualification Vendor",
                "Notes": FIXTURE_TAG,
            },
        )
        ...
        for key, payload in item_specs:
            value, was_created = await self._ensure_named(
                "Item", key, payload, name_field="Name"
            )
            items[key] = value
        ...
        payment_specs = (
            ...
            (
                ...
                "ACP-QUAL-PAY-003",
                "customer-active",
                (("invoice-unpaid", 30), ("invoice-multiple", 30)),
            ),
        )
                ...
                "Line": [
                    {
                        ...
                        "LinkedTxn": [
                            {
                                "TxnId": str(invoices[invoice_key]["Id"]),
                                "TxnType": "Invoice",
                            }
                        ],
                    }
        ...
        credit, was_created = await self._ensure_transaction(
            "CreditMemo", "credit-memo", "ACP-QUAL-CM-001", credit_payload
        )
        ...
        bill_specs = (
            ("bill-unpaid", "ACP-QUAL-BILL-001", 150),
            ("bill-partial", "ACP-QUAL-BILL-002", 200),
        )
            ...
            "Line": [
                {
                    "Amount": 75,
                    "LinkedTxn": [
                        {"TxnId": str(bills["bill-partial"]["Id"]), "TxnType": "Bill"}
                    ],
                }
            ],
        }
        bill_payment, was_created = await self._ensure_transaction(
            "BillPayment", "bill-payment", "ACP-QUAL-BP-001", bill_payment_payload
        )
        ...
        purchase, was_created = await self._ensure_private_note(
            "Purchase", "purchase", "ACP-QUAL-PUR-001", purchase_payload
        )
        ...
        vendor_credit, was_created = await self._ensure_transaction(
            "VendorCredit", "vendor-credit", "ACP-QUAL-VC-001", vendor_credit_payload
        )
        ...
        for key, doc, amount, debit, credit_ref, txn_date in journal_specs:
            journal_payload: dict[str, object] = {
                ...
                "PrivateNote": FIXTURE_TAG,
                "Line": [
                    _journal_line(amount, "Debit", debit),
                    _journal_line(amount, "Credit", credit_ref),
                ],
            }
            value, was_created = await self._ensure_transaction(
        ...
        transfer, was_created = await self._ensure_private_note(
            "Transfer", "transfer", "ACP-QUAL-XFER-001", transfer_payload
        )
    ...
    async def update_controlled_change(self) -> dict[str, str]:
        ...
        record = next(
            (
                ...
                and item.get("fixture_key") == "customer-active"
            ),
            None,
        )
        ...
        return {
            "state": "CHANGED"
            if before != change["after_sha256"]
            else "ALREADY_CURRENT",
            "change_digest": str(change["change_digest"]),
        }

    def _seal(
        self, objects: list[FixtureObject], *, created: int, reused: int
        ...
        document: dict[str, object] = {
            ...
            "environment": "sandbox",
            "realm_id_hash": hashlib.sha256(
                self.authority.realm_id.encode()
            ).hexdigest(),
            "fixture_tag": FIXTURE_TAG,
    ...
    async def _ensure_private_note(
        self,
        native_type: str,
        fixture_key: str,
        identity: str,
    ...
    async def _query(
        self, native_type: str, field: str, value: str
    ) -> list[dict[str, object]]:
        escaped = value.replace("'", "\\'")
        query = f"select * from {native_type} where {field} = '{escaped}' maxresults 10"
        url = f"{self.api_base}/{self.authority.realm_id}/query?" + urlencode(
            {"query": query, "minorversion": self.minor_version}
        )
    ...
    async def _post(
        self, native_type: str, payload: dict[str, object]
    ) -> dict[str, object]:
...
class SandboxFixtureHttpTransport:
    ...
    async def request(
        ...
        if (
            parsed.scheme != "https"
            or parsed.hostname != "sandbox-quickbooks.api.intuit.com"
        ):
            raise SandboxFixtureError("fixture_destination_rejected")
...
def build_fixture_service(
    authority: SandboxFixtureAuthority, configuration: Settings = settings
    ...
    if (
        marker.get("realm_id") != authority.realm_id
        or marker.get("environment") != "sandbox"
    ):
        raise SandboxFixtureError("fixture_realm_mismatch")
    ...
    return SandboxFixtureService(
        authority=authority,
        token_manager=SerializedTokenManager(
            oauth=oauth, secrets=provider, binding=binding
        ),
        transport=SandboxFixtureHttpTransport(),
        runtime_root=root,
...
def _fixture_object(
    family: str, fixture_key: str, value: Mapping[str, object]
) -> FixtureObject:
    return FixtureObject(
        family=family,
        fixture_key=fixture_key,
        native_id=str(value["Id"]),
        sync_token=str(value.get("SyncToken", "0")),
...
def _read_json(path: Path) -> dict[str, object]:
    try:
        ...
        raise SandboxFixtureError("fixture_authority_missing") from error
    if not isinstance(value, dict):
        raise SandboxFixtureError("fixture_document_invalid")
    return value
...
def _write_protected(path: Path, document: Mapping[str, object]) -> None:
    ...
    if path.exists():
        if path.read_bytes() != rendered:
            raise SandboxFixtureError("fixture_immutable_conflict")
        return
...
def _safe_result(result: SandboxFixtureResult) -> dict[str, object]:
    return {
        ...
        "expected_ledger_digest": result.expected_ledger_digest,
...
async def _main_async(arguments: argparse.Namespace) -> None:
    if (
        arguments.command in {"create", "controlled-change"}
        and not arguments.authorize_fixture_mutation
    ):
        raise SandboxFixtureError("fixture_mutation_not_authorized")
    root = _runtime_root(settings)
    marker = _read_json(root / "connections" / "verified.json")
    authority = SandboxFixtureAuthority(
    ...
    service = build_fixture_service(authority)
    if arguments.command == "qualify":
        print(json.dumps(_safe_result(await service.qualify()), sort_keys=True))
    elif arguments.command == "create":
        print(json.dumps(_safe_result(await service.create()), sort_keys=True))
    else:
        print(json.dumps(await service.update_controlled_change(), sort_keys=True))


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Manage authorized synthetic QBO sandbox qualification fixtures"
    )
    parser.add_argument("command", choices=("qualify", "create", "controlled-change"))
    parser.add_argument("--repository-sha", required=True)
    parser.add_argument("--actor", required=True)
    parser.add_argument(
        "--authorization-id", default="MIGRATION.CONTINUOUS.PRODUCTION.12H.1"
    )
    parser.add_argument("--authorize-fixture-mutation", action="store_true")
    arguments = parser.parse_args()
    try:
        asyncio.run(_main_async(arguments))
    except (SandboxFixtureError, SandboxRuntimeError) as error:
        print(
