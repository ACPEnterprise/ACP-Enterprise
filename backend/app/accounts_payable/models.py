...
class AccountingVendor(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "length(btrim(code)) > 0 AND length(btrim(legal_name)) > 0 AND length(btrim(display_name)) > 0",
            name="ck_ap_vendors_names",
        ),
...
class VendorSourceMapping(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["ap_vendors.company_id", "ap_vendors.id"],
            ondelete="RESTRICT",
        ),
        UniqueConstraint(
            "company_id",
            ...
            "source_company_id",
            "source_vendor_id",
            name="uq_ap_vendor_source_identity",
        ),
        UniqueConstraint(
            "company_id",
            "vendor_id",
            "source_system",
            name="uq_ap_vendor_source_vendor",
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    ...
    mapped_by_user_id: Mapped[UUID] = mapped_column(
...
class APAccountMapping(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "classification IN ('expense','prepaid','fixed_asset','inventory_asset','tax','freight','discount','cash','clearing','other')",
            name="ck_ap_account_mappings_classification",
        ),
        UniqueConstraint(
            "company_id",
            "mapping_key",
            "effective_from",
            name="uq_ap_account_mapping_version",
        ),
...
class VendorBill(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('draft','submitted','approved','posted','rejected','cancelled','partially_paid','paid','credited','reversed')",
            name="ck_ap_bills_status",
        ),
        CheckConstraint(
            "accounting_status IN ('pending','posted','reversed','reconciliation_required')",
            name="ck_ap_bills_accounting_status",
        ),
        CheckConstraint(
            "currency ~ '^[A-Z]{3}$' AND total_amount >= 0 AND open_amount >= 0 AND open_amount <= total_amount",
            name="ck_ap_bills_amounts",
        ),
        CheckConstraint("due_date >= bill_date", name="ck_ap_bills_dates"),
        UniqueConstraint(
            "company_id", "bill_number", name="uq_ap_bills_company_number"
        ),
        UniqueConstraint(
            "company_id",
            "vendor_id",
            "normalized_document_number",
            name="uq_ap_bills_vendor_document",
        ),
        UniqueConstraint(
            "company_id",
            "source_system",
            "source_identity",
            name="uq_ap_bills_source_identity",
        ),
...
class BillRevision(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "company_id", "bill_id", "revision", name="uq_ap_bill_revision"
        ),
        UniqueConstraint("company_id", "id", name="uq_ap_bill_revisions_company_id"),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    )
...
class BillLine(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["ap_bill_revisions.company_id", "ap_bill_revisions.id"],
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["company_id", "mapping_id"],
            ["ap_account_mappings.company_id", "ap_account_mappings.id"],
            ondelete="RESTRICT",
        ),
        CheckConstraint(
            "position >= 1 AND quantity > 0 AND net_amount >= 0 AND tax_amount >= 0",
            name="ck_ap_bill_lines_amounts",
        ),
        UniqueConstraint(
            "company_id", "revision_id", "position", name="uq_ap_bill_line_position"
        ),
...
class DuplicateOverride(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "requester_user_id <> reviewer_user_id", name="ck_ap_duplicate_override_sod"
        ),
        UniqueConstraint("company_id", "bill_id", name="uq_ap_duplicate_override_bill"),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    )
    company_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), nullable=False)
    bill_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), nullable=False)
    duplicate_bill_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), nullable=False
    )
...
class VendorCredit(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "amount > 0 AND available_amount >= 0 AND available_amount <= amount",
            name="ck_ap_vendor_credits_amounts",
        ),
        UniqueConstraint(
            "company_id", "credit_number", name="uq_ap_vendor_credit_number"
        ),
        UniqueConstraint(
            "company_id",
            "source_system",
            "source_identity",
            name="uq_ap_vendor_credit_source",
        ),
        UniqueConstraint("company_id", "id", name="uq_ap_vendor_credits_company_id"),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    )
...
class CreditApplication(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["ap_vendor_credits.company_id", "ap_vendor_credits.id"],
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["company_id", "bill_id"],
        ...
        CheckConstraint("amount > 0", name="ck_ap_credit_applications_amount"),
        UniqueConstraint(
            "company_id", "idempotency_key", name="uq_ap_credit_application_idempotency"
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
...
class APSubledgerEntry(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "entry_type IN ('bill','bill_reversal','credit','credit_application','credit_unapplication','disbursement','disbursement_reversal','correction')",
            name="ck_ap_subledger_entry_type",
        ),
        UniqueConstraint(
            "company_id", "idempotency_key", name="uq_ap_subledger_idempotency"
        ),
        Index("ix_ap_subledger_as_of", "company_id", "vendor_id", "effective_date"),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    )
...
class Disbursement(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "amount > 0 AND available_amount >= 0 AND available_amount <= amount",
            name="ck_ap_disbursements_amounts",
        ),
        CheckConstraint(
            "recorder_user_id <> approver_user_id", name="ck_ap_disbursements_sod"
        ),
        UniqueConstraint(
            "company_id",
            "source_system",
            "source_identity",
            name="uq_ap_disbursement_source",
        ),
        UniqueConstraint("company_id", "id", name="uq_ap_disbursements_company_id"),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
    )
...
class DisbursementApplication(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["company_id", "disbursement_id"],
            ["ap_disbursements.company_id", "ap_disbursements.id"],
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["company_id", "bill_id"],
        ...
        CheckConstraint("amount > 0", name="ck_ap_disbursement_applications_amount"),
        UniqueConstraint(
            "company_id",
            "idempotency_key",
...
class APPostingReceipt(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "company_id", "source_event_id", name="uq_ap_posting_receipt_event"
        ),
    )
    id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), primary_key=True, default=uuid4
