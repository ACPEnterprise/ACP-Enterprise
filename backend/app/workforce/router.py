...
ReadContext = Annotated[AuthorizationContext, Depends(require_permission(WorkforcePermission.READ))]
...
async def detail(employee_id: UUID, context: ReadContext, session: Session) -> WorkforceEmployeeDetail:
    result = await workforce_operations_service.detail(session, context=context, employee_id=employee_id)
    if result is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Workforce profile was not found.")
    return result


@router.post("/eligibility", response_model=WorkforceEligibilityResponse)
async def eligibility(payload: WorkforceEligibilityRequest, context: ReadContext, session: Session) -> WorkforceEligibilityResponse:
    return await workforce_operations_service.eligibility(session, context=context, request=payload)
