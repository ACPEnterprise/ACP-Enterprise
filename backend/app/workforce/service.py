...
class WorkforceOperationsService:
    ...
    def _summary(
        ...
        languages = (
            tuple(item.code for item in profile.languages if item.status == "active")
            if profile
            else ()
        )
        return WorkforceEmployeeSummary(
            ...
            technician="technician" in capabilities,
    ...
    async def directory(
        ...
        for employee, profile_model in rows:
            profile = (
                ...
                else await workforce_capability_repository.get_complete_profile(
    ...
    async def detail(
        ...
        complete = await workforce_capability_repository.get_complete_profile(
            session, company_id=context.company.id, profile_id=profile_id
        )
        if complete is None:
            return None
        ...
        availability = tuple(
            ...
            for item in (
                await session.scalars(
                    ...
                    .order_by(
                        WorkforceWorkingAvailability.start_at,
                        WorkforceWorkingAvailability.id,
                    )
                )
            ).all()
        ...
        return WorkforceEmployeeDetail(
            **summary.model_dump(),
            capabilities=tuple(
                ...
                for i in complete.capabilities
            ),
            certifications=tuple(
                WorkforceCertificationItem(
                    code=i.code,
                    display_name=i.display_name,
                    credential_reference=i.credential_reference,
                    status=i.status,
                    issued_on=i.issued_on,
                    expires_on=i.expires_on,
                )
                for i in complete.certifications
            ),
            languages=tuple(
                WorkforceLanguageItem(
                    code=i.code,
                    english_name=i.english_name,
                    native_name=i.native_name,
                    spoken_proficiency=i.spoken_proficiency,
                    customer_facing_eligible=i.customer_facing_eligible,
                    interpreter_verified=i.interpreter_verified,
                    status=i.status,
                )
                for i in complete.languages
            ),
            branches=tuple(
                WorkforceBranchItem(
                ...
                for i in complete.branch_eligibilities
                if context.can_access_branch(i.branch_id)
            ),
            work_restrictions=tuple(
                i.code for i in complete.work_restrictions if i.status == "active"
            ),
            equipment_capabilities=tuple(
                ...
                for i in complete.equipment_capabilities
            ),
    ...
    async def eligibility(
        ...
        *,
        ...
        request: WorkforceEligibilityRequest,
    ) -> WorkforceEligibilityResponse:
        rows = await workforce_eligibility_service.eligible_technicians(
            ...
            query=WorkforceEligibilityQuery(
                ...
                required_capability_codes=request.required_capability_codes,
                required_language_codes=request.required_language_codes,
            ),
        )
        return WorkforceEligibilityResponse(
            items=tuple(WorkforceEligibilityItem(**item.__dict__) for item in rows)
        )
