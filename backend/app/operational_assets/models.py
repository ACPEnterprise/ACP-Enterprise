...
class AssetEvidence(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "evidence_type IN ('manufacturer','model','serial_reference','vin','license_plate','provider_identity','installation','removal','replacement','job_service','warranty','inspection','maintenance','readiness','document','custody')",
            name="ck_asset_evidence_type",
        ),
        CheckConstraint(
            "state IN ('recorded','verified','unverified','eligible','not_eligible','expired','conflicting_evidence','insufficient_evidence','completed','canceled','deferred')",
