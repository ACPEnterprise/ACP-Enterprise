"""Restricted Preview command for Operational Migration Phase 2."""
...
from app.operational_migration.financial import FinancialMigrationService
...
class FinancialExecutionApproval(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    approval_version: Literal["operational-migration-phase2-execution/v1"]
    ...
    def verify(self) -> None:
        ...
        if expected != self.approval_sha256:
            raise ValueError("financial execution approval digest mismatch")
...
def prepare(args: argparse.Namespace) -> int:
    for path in (args.source, args.phase1_reviewed):
        _restricted(path)
    review = transform_phase2(
        source_bytes=args.source.read_bytes(),
        phase1_review_bytes=args.phase1_reviewed.read_bytes(),
    )
...
def manifest(args: argparse.Namespace) -> int:
    reviewed = ReviewedFinancialOutput.model_validate(_json(args.reviewed_output))
    prior = (
        OperationalPhase2Manifest.model_validate(_json(args.prior_manifest))
...
def approval(args: argparse.Namespace) -> int:
    selected = OperationalPhase2Manifest.model_validate(_json(args.manifest))
    payload: dict[str, object] = {
        "approval_version": "operational-migration-phase2-execution/v1",
        "target": "preview",
        "mode": args.mode,
        "manifest_sha256": selected.manifest_sha256,
        "replay_digest": selected.replay_digest,
        "ordered_invoice_identity_sha256": selected.ordered_invoice_identity_sha256,
...
async def _counts(company_id: UUID) -> dict[str, int]:
    ...
    async with AsyncSessionFactory() as session:
        for label, model in (
            ...
            ("payments", Payment),
            ("estimate_source_identities", EstimateSourceIdentity),
            ("estimate_line_item_source_identities", EstimateLineItemSourceIdentity),
            ("invoice_source_identities", InvoiceSourceIdentity),
            ("invoice_line_item_source_identities", InvoiceLineItemSourceIdentity),
            ("payment_source_identities", PaymentSourceIdentity),
        ):
            values[label] = int(
            ...
            await session.scalar(
                ...
                .where(
                    ...
                    BusinessEvent.event_type.in_(
                        ("estimate.migrated", "invoice.migrated", "payment.migrated")
                    ),
                )
            )
            or 0
...
async def execute(args: argparse.Namespace) -> int:
    if settings.environment != "preview" or args.target != "preview":
        raise ValueError("financial migration can execute only in Preview")
    reviewed = ReviewedFinancialOutput.model_validate(_json(args.reviewed_output))
    selected = OperationalPhase2Manifest.model_validate(_json(args.manifest))
    authorized = FinancialExecutionApproval.model_validate(_json(args.approval))
    authorized.verify()
    ...
    if (
        ...
        or authorized.ordered_invoice_identity_sha256
        != selected.ordered_invoice_identity_sha256
        or authorized.expected_deployed_git_sha != deployed_sha
    ):
        raise ValueError("financial approval does not match execution inputs")
    backup = None
    if args.mode == "import":
    ...
    invoices, payments = stage_records(reviewed, selected)
...
def parser() -> argparse.ArgumentParser:
    ...
    prep.add_argument("--phase1-reviewed", type=Path, required=True)
