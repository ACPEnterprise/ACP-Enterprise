...
def _validate_assertion_shape(value: ParticipationAssertion) -> None:
    ...
    if (value.kind is ParticipationKind.JOB) != (value.job_id is not None):
        raise JobParticipationError("Job identity is required only for Job participation")


def _minutes(start: datetime, end: datetime) -> int:
    seconds = int((end - start).total_seconds())
    if seconds % 60:
        raise JobParticipationError("participation evidence must resolve to whole minutes")
    return seconds // 60
