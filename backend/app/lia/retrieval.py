...
ADAPTERS = (
    AdapterSpec("customers", "Customer records", CustomerPermission.READ, Customer, Customer.status),
    AdapterSpec("jobs", "Jobs", JobPermission.READ, Job, Job.status),
    AdapterSpec("scheduling", "Appointments", SchedulingPermission.READ, Appointment, Appointment.status),
    AdapterSpec("estimates", "Estimates", EstimatePermission.READ, Estimate, Estimate.status),
    AdapterSpec("invoicing", "Invoices", InvoicePermission.READ, Invoice, Invoice.status),
    AdapterSpec("payments", "Payment receipts", PaymentPermission.READ, PaymentReceipt, PaymentReceipt.status),
    AdapterSpec("purchasing", "Purchase orders", PurchasingPermission.READ, PurchaseOrder, PurchaseOrder.status),
    AdapterSpec("inventory", "Inventory items", InventoryPermission.READ, InventoryItem, InventoryItem.status),
)
    ...
    async def retrieve(
        ...
        *,
        ...
        for adapter in permitted:
            ...
            if context.active_branch is not None and hasattr(adapter.model, "branch_id"):
            ...
            state = ", ".join(f"{key}={value}" for key, value in counts.items()) or "no records"
            evidence.append(
                EvidenceReference(
                    domain=adapter.domain,
                    label=adapter.label,
