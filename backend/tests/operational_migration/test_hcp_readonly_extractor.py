...
def snap() -> SnapshotIdentity:
    ...
    return SnapshotIdentity(
        "fixture",
        now,
        AcquisitionMechanism.PUBLIC_API,
        "fixture",
        "page=1",
        now,
        now,
...
def test_collection_and_envelopes_preserve_missing_relationships() -> None:
    body = json.dumps(
        {
            "jobs": [{"id": "j1", "work_status": "open", "customer_id": "c1"}],
            "page": 1,
            "page_size": 1,
            "total_items": 1,
            "total_pages": 1,
    ...
    envelopes = qualify_records(
        native_entity="job",
        records=records,
        snapshot=snap(),
        status_field="work_status",
    ...
    assert [
        (r.relationship, r.parent_native_id) for r in envelopes[0].relationships
    ] == [("customer", "c1")]
    assert envelopes[0].source_created_at is None


def test_conflicting_duplicate_native_id_is_rejected() -> None:
    first = qualify_records(
        native_entity="job",
        records=[{"id": "j1", "work_status": "open"}],
        snapshot=snap(),
        status_field="work_status",
        created_field=None,
        updated_field=None,
    ...
    second = qualify_records(
        native_entity="job",
        records=[{"id": "j1", "work_status": "completed"}],
        snapshot=snap(),
        status_field="work_status",
        created_field=None,
        updated_field=None,
