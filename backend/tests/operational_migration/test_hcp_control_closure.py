...
def entry(tmp_path: Path, kind: ControlExportType, company: str = SHA):
    ...
    return intake_csv_control(
        ...
        artifact_name=f"{kind.value}.csv",
        source=b"Native ID,Status\n1,source-value\n",
        control_type=kind,
        source_report_identity=f"housecall_pro/{kind.value}/v1",
        extraction_timestamp=datetime(
            2026, 8, 26, 12, tzinfo=ZoneInfo("America/New_York")
        ),
        timezone="America/New_York",
        filters={"scope": "unfiltered"},
        exporting_admin_evidence_sha256=SHA,
        company_identity_sha256=company,
    )
...
def test_intake_and_manifest_are_immutable_and_company_scoped(tmp_path: Path) -> None:
    ...
    assert all(
        (tmp_path / item.control_type.value / item.protected_artifact_name)
        .stat()
        .st_mode
        & 0o777
        == 0o600
...
def test_payment_partitions_are_contiguous_without_duplicate_dates() -> None:
    ranges = validate_payment_ranges(
        (
            PaymentDateRange(date(2026, 2, 1), date(2026, 2, 28)),
            PaymentDateRange(date(2026, 1, 1), date(2026, 1, 31)),
        )
    )
    ...
    with pytest.raises(ValueError, match="contiguous"):
        validate_payment_ranges(
            (
                PaymentDateRange(date(2026, 1, 1), date(2026, 1, 31)),
                PaymentDateRange(date(2026, 1, 31), date(2026, 2, 28)),
            )
        )


def test_reconciliation_preserves_missing_conflicting_and_unsupported() -> None:
    results = reconcile_native_evidence(
        entity="job",
        api={"1": SHA, "2": SHA, "3": SHA},
        control={"1": SHA, "2": "b" * 64, "4": SHA},
    )
    assert [item.classification for item in results] == [
        ReconciliationClassification.MATCHED,
        ReconciliationClassification.CONFLICTING,
        ReconciliationClassification.CONTROL_EXPORT_MISSING,
        ReconciliationClassification.SOURCE_API_MISSING,
    ]
    assert (
        unsupported_relationship("payment", "application").classification
        is ReconciliationClassification.UNSUPPORTED_RELATIONSHIP
    )


def test_financial_conflict_keeps_both_source_assertions() -> None:
    evidence = financial_conflict(
        SourceAssertion("housecall_pro", "invoice", "h1", "status", "paid", SHA),
        SourceAssertion(
            "quickbooks_online", "invoice", "q1", "status", "open", "b" * 64
        ),
    )
...
def test_owner_cutoff_changes_reconciliation_not_source_truth() -> None:
    ...
    assert (
        missing_evidence_impact(legacy) is ReadinessImpact.NON_BLOCKING_MISSING_EVIDENCE
    )
