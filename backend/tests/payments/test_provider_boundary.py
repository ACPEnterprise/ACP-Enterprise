...
async def test_fake_models_decline_and_ambiguity_without_receipt_claim() -> None:
    ...
    for marker, outcome in (
        ("opaque_declined", "declined"),
        ("opaque_ambiguous", "ambiguous"),
    ):
        result = await provider.collect(
            ProviderRequest(
                uuid4(), marker, "synthetic", Decimal("1.00"), "USD", marker
            )
        )
...
async def test_existing_processor_is_fail_closed() -> None:
    with pytest.raises(PaymentSecurityError):
        await ExistingProcessorAdapter().collect(
            ProviderRequest(
                uuid4(), "key", "merchant", Decimal("1.00"), "USD", "opaque_ref"
            )
        )
...
def test_webhook_verifies_raw_body_merchant_rotation_and_tamper() -> None:
    ...
    signature = (
        "v2="
        + hmac.new(secret, timestamp.encode() + b"." + body, hashlib.sha256).hexdigest()
    )
