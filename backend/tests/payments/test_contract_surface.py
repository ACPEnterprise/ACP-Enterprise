...
def test_slot_three_migration_has_invoice_parent_and_sensitive_columns_are_absent() -> (
    None
):
    root = Path(__file__).resolve().parents[2]
    migration = (
        root / "alembic/versions/x9n1j3l5p720_create_day_one_payments_runtime.py"
    ).read_text()
