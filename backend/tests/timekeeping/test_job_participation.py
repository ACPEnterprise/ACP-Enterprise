...
def test_incorrect_job_creates_reviewed_successor_and_preserves_predecessor() -> None:
    original = assertion(1, ParticipationKind.JOB, at(8), at(12), job_id=JOB)
    replacement_job = UUID("66666666-6666-6666-6666-666666666666")
    reviewer = UUID("77777777-7777-7777-7777-777777777777")
    correction = correct_participation(
        predecessor=original,
        successor_id=UUID(int=8),
        replacement_job_id=replacement_job,
        reason="Dispatch history proves the work belonged to the replacement Job",
        reviewed_by_user_id=reviewer,
    )
    assert original.job_id == JOB
    assert correction.predecessor_assertion_id == original.assertion_id
    assert correction.successor.job_id == replacement_job
    assert correction.successor.approved is True
    assert correction.reviewed_by_user_id == reviewer
    assert correction.evidence_digest == correction.successor.evidence_digest
