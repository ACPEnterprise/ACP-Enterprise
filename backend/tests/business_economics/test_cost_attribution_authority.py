...
def policy(*, company_id, family_key, lifecycle):
    ...
    approved_at = (
        datetime(2026, 9, 1, tzinfo=timezone.utc) if approver else None
    )
    return seal_policy(
        policy_id=uuid4(),
...
def test_component_specific_burden_requires_certified_reference_list() -> None:
    ...
    candidate = seal_policy(
            policy_id=uuid4(), company_id=company, branch_id=None,
            family_key="employer_burden_allocation", policy_version=1,
            disposition=PolicyDisposition.SELECTED,
            strategy_key="component_specific_certified_drivers", parameters={},
            evidence_acceptance_rule_refs=(), effective_start=date(2026, 9, 1),
            effective_end=None, lifecycle=PolicyLifecycle.APPROVED,
            definition_version=POLICY_DEFINITION_VERSION,
            approved_by_user_id=uuid4(),
            approved_at=datetime(2026, 9, 1, tzinfo=timezone.utc),
            decision_evidence_digest="b" * 64, supersedes_policy_id=None,
        )
    candidate.validate()
