...
async def test_receipt_replay_exact_mapping_and_split_disposition(
    ...
    async with factory() as session:
        ...
        assert {row.state for row in submitted.dispositions} == {"composed"}
    async with factory() as session:
        replay = await service.dispose(
            session,
            context=context,
            job_id=job.id,
            purchase_id=purchase.id,
        ...
        assert quantity.on_hand == Decimal(1)
        assert quantity.reserved == Decimal(0)
    conflicting = disposition_payload.model_copy(
        update={
            "dispositions": (
                disposition_payload.dispositions[0].model_copy(
                    update={"quantity": Decimal(2)}
                ),
                disposition_payload.dispositions[1],
            )
        }
    )
    async with factory() as session:
        with pytest.raises(FieldServiceConflict, match="different evidence"):
