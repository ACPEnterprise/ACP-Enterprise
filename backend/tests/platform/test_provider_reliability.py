...
def _evidence(
    knowledge: ProviderKnowledge, attempt: int = 1
) -> ProviderAttemptEvidence:
...
def test_provider_uncertainty_never_becomes_blind_retry() -> None:
    ...
    assert (
        retry_disposition(_evidence(ProviderKnowledge.UNCERTAIN), policy)
        is RetryDisposition.RECONCILE_BEFORE_RETRY
    )
    assert (
        retry_disposition(_evidence(ProviderKnowledge.ACCEPTED), policy)
        is RetryDisposition.NO_RETRY
    )
    assert (
        retry_disposition(_evidence(ProviderKnowledge.ACKNOWLEDGED), policy)
        is RetryDisposition.NO_RETRY
    )
...
def test_pre_acceptance_retry_requires_explicit_versioned_policy() -> None:
    allowed = RetryPolicy(
        "provider-neutral", "v1", retry_before_acceptance=True, maximum_attempts=2
    )
    ...
    assert (
        retry_disposition(_evidence(evidence.knowledge, 2), allowed)
        is RetryDisposition.NO_RETRY
    )
...
def test_failure_injection_is_mechanically_test_only() -> None:
    with pytest.raises(
        InjectedReliabilityFailure, match=FailurePoint.REDIS_UNAVAILABLE.value
    ):
