...
async def test_statement_is_deterministic_immutable_and_self_scoped(
    ...
    async with factory() as session:
        ...
        assert [
            item.id
            for item in await service.list_own(session, context=own, limit=1, offset=0)
        ] == [value.id]
        ...
        unauthorized = FakeContext(
            values["company_id"],
            values["reviewer_id"],
            {PayrollPermission.STATEMENT_READ},
        )
        ...
        with pytest.raises(PayrollConflictError, match="not found"):
            await service.administrative(
                session, context=other_company, statement_id=value.id
            )
...
async def test_unapproved_run_cannot_issue_statement(
    ...
    async with factory() as session:
        ...
        with pytest.raises(PayrollConflictError, match="approved complete"):
            await PayrollPayStatementService().create(
                session,
                context=manage,
                run_id=run.id,
