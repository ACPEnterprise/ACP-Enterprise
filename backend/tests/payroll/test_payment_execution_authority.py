...
async def approved_release(session: AsyncSession, values: dict[str, object]):  # type: ignore[no-untyped-def]
    ...
    resolution = await release_service.resolve_destination(
        session,
    ...
    candidate = await release_service.assemble_candidate(
        session,
        context=assemble,
        payroll_run_id=run.id,
        destinations={values["employee_id"]: resolution},
        assembled_at=NOW,
    )
    release = await release_service.persist_candidate(
        session, context=assemble, candidate=candidate
    )
    await release_service.initiate_review(
        session, context=review, release_id=release.id, reason_code="synthetic"
    )
    await release_service.decide_review(
        session,
        context=review,
    ...
    await release_service.approve_release(
        session, context=approve_read, release_id=release.id, reason_code="synthetic"
    )
...
def execution_contexts(values: dict[str, object]) -> tuple[Any, Any, Any]:
    authorize = FakeContext(
        values["company_id"],
        values["actor_id"],
        {PayrollPermission.PAYMENT_EXECUTION_AUTHORIZE},
    )
    reconcile = FakeContext(
        values["company_id"],
        values["reviewer_id"],
        {PayrollPermission.PAYMENT_SETTLEMENT_RECONCILE},
    )
    read = FakeContext(
        values["company_id"],
        values["reviewer_id"],
        {PayrollPermission.PAYMENT_EXECUTION_READ},
    )
...
async def authorized_execution(session: AsyncSession, values: dict[str, object]):  # type: ignore[no-untyped-def]
    ...
    candidate = await service.create_candidate(
        ...
        context=authorize,
        release_id=release.id,
        provider_identity="synthetic.payment-provider",
        provider_version="test.v1",
        idempotency_identity="synthetic-idempotency-1",
    )
...
@pytest.mark.asyncio
async def test_execution_acknowledgement_settlement_handoff_and_safe_events(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        service, release, candidate, execution = await authorized_execution(
            session, values
        )
        ...
        with pytest.raises(PayrollConflictError, match="candidate"):
            await service.authorize(
                ...
                candidate=replace(
                    candidate,
                    authorized_total=candidate.authorized_total + Decimal("1.00"),
                ),
            )
        replay = await service.authorize(
            session, context=authorize, candidate=candidate
        )
        assert replay.id == execution.id
        provider = SyntheticPaymentExecutionProvider(environment="test")
        acknowledged = await service.submit(
            session, context=authorize, execution_id=execution.id, provider=provider
        )
        ...
        with pytest.raises(PayrollConflictError, match="settlement evidence"):
            await service.accounting_handoff(
                session, context=read, execution_id=execution.id
            )
        ...
        digest = canonical_digest(
            {
                ...
                "state": "settled",
                "provider_safe_reference": reference,
            }
        )
        settled = await service.record_settlement(
            session,
            context=reconcile,
            ...
            (
                await session.scalars(
                    select(BusinessEvent).where(
                        BusinessEvent.entity_type == "payroll_payment_execution"
                    )
                )
            ).all()
        )
        assert events and all(
            "amount" not in str(event.payload)
            and "destination" not in str(event.payload)
            for event in events
        )


@pytest.mark.asyncio
async def test_rejection_uncertain_contradiction_permissions_and_company_scope(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        service, release, _candidate, execution = await authorized_execution(
            session, values
        )
        ...
        with pytest.raises(PayrollAuthorizationError):
            await service.create_candidate(
                ...
                provider_identity="synthetic.payment-provider",
                provider_version="test.v1",
                idempotency_identity="denied",
            )
        other_company = FakeContext(
        ...
        with pytest.raises(PayrollConflictError, match="approved-for-release"):
            await service.create_candidate(
                session,
                context=other_company,
                release_id=release.id,
                provider_identity="synthetic.payment-provider",
                provider_version="test.v1",
                idempotency_identity="cross-company",
            )
        uncertain = SyntheticPaymentExecutionProvider(
            environment="test", result=ProviderResultState.UNCERTAIN
        )
        result = await service.submit(
            session, context=authorize, execution_id=execution.id, provider=uncertain
        )
        ...
        contradictory = ProviderAcknowledgement(
            ProviderResultState.ACCEPTED,
            "different-safe-reference",
            NOW,
            result.request_digest or "",
...
@pytest.mark.asyncio
async def test_failed_settlement_and_test_only_provider(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    ...
    async with factory() as session:
        ...
        failed = await service.record_settlement(
            session,
            context=reconcile,
            execution_id=execution.id,
            outcomes=(
