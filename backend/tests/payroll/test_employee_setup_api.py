...
def test_employee_setup_routes_are_bounded_and_separate_approval() -> None:
    ...
    assert set(paths["/api/v1/payroll/setup/employees/{employee_id}/readiness"]) == {
        "get"
    }
    assert set(
        paths["/api/v1/payroll/setup/employees/{employee_id}/compensations"]
    ) == {"post"}
    assert set(paths["/api/v1/payroll/setup/employees/{employee_id}/inputs"]) == {
        "post"
    }
    assert set(paths["/api/v1/payroll/setup/compensations/{authority_id}/approve"]) == {
        "post"
    }
...
def test_protected_values_are_write_only() -> None:
    operation = app.openapi()["paths"]["/api/v1/payroll/setup/employees/{employee_id}"][
        "get"
    ]
