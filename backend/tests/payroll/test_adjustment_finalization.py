...
async def candidate_and_authority(session: AsyncSession, values: dict[str, object]):  # type: ignore[no-untyped-def]
    ...
    candidate = await PayrollAdjustmentCalculationService(
        runtime_environment=RuleEnvironment.TEST
    ).calculate(
        session,
        context=context(values, PayrollPermission.ADJUSTMENT_CALCULATE),
...
@pytest.mark.asyncio
async def test_persist_review_approve_apply_replay_and_safe_immutability(
    ...
    async with factory() as session:
        ...
        first = await service.persist_candidate(
            session, context=execute, candidate=candidate
        )
        ...
        initiated = await service.initiate_review(
            session, context=review, result_id=first.id, reason_code="synthetic"
        )
        await service.decide_review(
            session,
            context=review,
            result_id=first.id,
            decision=AdjustmentResultDecision.ACCEPTED,
            reason_code="synthetic",
        )
        approved = await service.approve(
            session, context=approve, result_id=first.id, reason_code="synthetic"
        )
        ...
        application = await service.apply(
            ...
            context=apply,
            result_id=first.id,
            purpose="successor_payroll",
            successor_authority_type="payroll.gross-pay-successor.v1",
        )
        application_replay = await service.apply(
            session,
            context=apply,
            result_id=first.id,
            purpose="successor_payroll",
        ...
        assert (
            await session.scalar(
                select(func.count(PayrollAdjustmentApplicationRecord.id)).where(
                    PayrollAdjustmentApplicationRecord.result_id == first.id
                )
            )
            == 1
        )
...
@pytest.mark.asyncio
async def test_admission_tamper_unapproved_scope_and_permissions_fail_closed(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        ...
        with pytest.raises(PayrollConflictError):
            await service.persist_candidate(
                session, context=execute, candidate=replace(candidate, currency="EUR")
            )
        ...
        with pytest.raises(PayrollConflictError):
            await service.persist_candidate(
                session,
                context=context(
                    values, PayrollPermission.ADJUSTMENT_CALCULATE, other=True
                ),
                candidate=candidate,
            )
...
@pytest.mark.asyncio
async def test_rejection_history_and_unauthorized_approval(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        ...
        service = PayrollAdjustmentResultService()
        value = await service.persist_candidate(
            session,
            context=context(values, PayrollPermission.ADJUSTMENT_CALCULATE),
            candidate=candidate,
        )
        ...
        await service.initiate_review(
            session, context=review, result_id=value.id, reason_code="synthetic_reject"
        )
        await service.decide_review(
            session,
            context=review,
            result_id=value.id,
            decision=AdjustmentResultDecision.REJECTED,
            reason_code="synthetic_reject",
        )
        with pytest.raises(PayrollAuthorizationError):
            await service.approve(
                session, context=review, result_id=value.id, reason_code="no"
            )
        history = tuple(
            (
...
@pytest.mark.asyncio
async def test_supersession_is_append_only_and_cannot_follow_application(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        ...
        changed = replace(
            candidate,
            definition_version="payroll.adjustment-calculation.v2",
            calculation_digest="",
            result_identity="",
        )
        ...
        second = await service.persist_candidate(
            session, context=execute, candidate=changed, supersedes_result_id=first.id
        )
        await session.refresh(first)
        assert (
            first.lifecycle == "superseded" and second.supersedes_result_id == first.id
        )
        forked = replace(
            changed,
            definition_version="payroll.adjustment-calculation.v3",
            calculation_digest="",
            result_identity="",
        )
