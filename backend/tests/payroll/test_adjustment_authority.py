...
def contexts(values: dict[str, object]) -> tuple[Any, Any, Any, Any]:
    ...
    read = FakeContext(
        values["company_id"], values["actor_id"], {PayrollPermission.ADJUSTMENT_READ}
    )
...
def draft(
    run,
    classification: PayrollCorrectionType = PayrollCorrectionType.RETROACTIVE_EARNINGS,
    *,
    amount: Decimal = Decimal("25.00"),
    supersedes=None,
):  # type: ignore[no-untyped-def]
    return DraftPayrollAdjustment(
        ...
        evidence_digest=canonical_digest({"synthetic-evidence": str(amount)}),
        delta_components=(EconomicDelta("gross_wages", amount),),
        employee_id=None,
        original_pay_period_id=run.pay_period_id,
        off_cycle_pay_period_id=uuid4()
        if classification is PayrollCorrectionType.OFF_CYCLE_PAYROLL
        else None,
...
@pytest.mark.asyncio
async def test_retroactive_negative_delta_review_approval_and_original_immutable(
    ...
    async with factory() as session:
        ...
        await service.initiate_review(
            session, context=review, adjustment_id=first.id, reason_code="synthetic"
        )
        ...
        await service.approve(
            session, context=approve, adjustment_id=first.id, reason_code="synthetic"
        )
        consequence = await service.consequence(
            session, context=read, adjustment_id=first.id
        )
...
@pytest.mark.asyncio
async def test_off_cycle_competing_cross_company_and_permissions_fail_closed(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        ...
        with pytest.raises(PayrollConflictError, match="competing"):
            await service.create(
                ...
                draft=DraftPayrollAdjustment(
                    **{**command.__dict__, "evidence_digest": "b" * 64}
                ),
            )
        other = FakeContext(
            values["other_company_id"],
            values["actor_id"],
            {PayrollPermission.ADJUSTMENT_MANAGE},
        )
...
@pytest.mark.asyncio
async def test_payment_return_binds_execution_without_erasing_payment_evidence(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        _, _, _, execution = await authorized_execution(session, values)
        original_digest, original_state = (
        ...
        command = DraftPayrollAdjustment(
            classification=PayrollCorrectionType.PAYMENT_RETURN,
            reason_code="synthetic_return",
            source_type="payment_execution",
            source_id=execution.id,
            source_digest=execution.execution_digest,
            currency=execution.currency,
            effective_date=date(2026, 9, 16),
            evidence_digest=canonical_digest({"synthetic-return": True}),
        ...
        assert (
            execution.execution_digest == original_digest
            and execution.lifecycle == original_state
        )
