...
def authority_contexts(values: dict[str, object]) -> tuple[Any, Any, Any]:
    manage = FakeContext(
        ...
        values["actor_id"],
        {
            PayrollPermission.ACCOUNTING_POLICY_MANAGE,
            PayrollPermission.ACCOUNTING_MAPPING_MANAGE,
        },
    )
    approve = FakeContext(
        values["company_id"],
        values["reviewer_id"],
        {
            PayrollPermission.ACCOUNTING_POLICY_APPROVE,
            PayrollPermission.ACCOUNTING_MAPPING_APPROVE,
        },
    )
    prepare = FakeContext(
        values["company_id"], values["actor_id"], {PayrollPermission.ACCOUNTING_PREPARE}
    )
    return manage, approve, prepare


async def accounting_foundation(
    session: AsyncSession,
    values: dict[str, object],
    components: tuple[PayrollAccountingComponent, ...],
    event: PayrollRecognitionEvent,
):
    chart = ChartVersion(
        company_id=values["company_id"],
        version=1,
        name="Synthetic Payroll Chart",
        currency="USD",
    ...
    for ordinal, component in enumerate(components, 1):
        account = Account(
            ...
            code=f"SYN-{ordinal:03}",
            name=f"Synthetic {component.value}",
            ...
            in {
                PayrollAccountingComponent.GROSS_WAGES,
                PayrollAccountingComponent.EMPLOYER_CONTRIBUTION_EXPENSE,
                PayrollAccountingComponent.NET_PAY_LIABILITY_SETTLEMENT,
            }
            else "credit",
            status="active",
    ...
    policy = await service.create_policy(
        ...
        draft=DraftPayrollAccountingPolicy(
            ...
            date(2026, 1, 1),
            None,
            canonical_digest({"synthetic-policy": event.value}),
        ),
    )
    ...
    for component in components:
        mapping = await service.create_mapping(
            ...
            draft=DraftPayrollAccountingMapping(
                ...
                component,
                PostingSide.DEBIT
                if component in debit_components
                else PostingSide.CREDIT,
                accounts[component].id,
                "USD",
                date(2026, 1, 1),
                None,
                canonical_digest({"synthetic-mapping": component.value}),
            ),
        )
...
@pytest.mark.asyncio
async def test_approved_run_projects_balanced_native_posting_fact(
    ...
    async with factory() as session:
        ...
        service, prepare, period = await accounting_foundation(
            session, values, components, PayrollRecognitionEvent.PAYROLL_ACCRUAL
        )
        candidate = await service.prepare_accrual(
            session,
            context=prepare,
        ...
        data = AutomatedPostingService(rules=registry)._journal_create(
            candidate.fact, registry.resolve(candidate.fact), period.id
        )
        assert sum(line.debit for line in data.lines) == sum(
            line.credit for line in data.lines
        )
        replay = await service.prepare_accrual(
            session,
            context=prepare,
            payroll_run_id=run.id,
            effective_date=date(2026, 9, 4),
...
@pytest.mark.asyncio
async def test_policy_mapping_company_currency_and_period_fail_closed(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
...
@pytest.mark.asyncio
async def test_only_explicit_settled_instructions_project_wage_settlement(
    finalization_database: tuple[async_sessionmaker[AsyncSession], dict[str, object]],
) -> None:
    factory, values = finalization_database
    async with factory() as session:
        ...
        await execution_service.record_settlement(
            session,
            context=reconcile,
            execution_id=execution.id,
            outcomes=(
        ...
        components = (
            PayrollAccountingComponent.NET_PAY_LIABILITY_SETTLEMENT,
            PayrollAccountingComponent.WAGE_SETTLEMENT,
        )
        service, prepare, period = await accounting_foundation(
            session, values, components, PayrollRecognitionEvent.WAGE_SETTLEMENT
        )
        candidate = await service.prepare_wage_settlement(
            session,
            context=prepare,
            execution_id=execution.id,
        ...
        assert (
            PayrollAccountingComponent.GROSS_WAGES.value
            not in candidate.fact.components
        )
