...
async def approved_gross(
    ...
    persisted = await service.persist_candidate(
        session, context=execute_context, candidate=gross_candidate
    )
    await service.initiate_review(
        session, context=review_context, result_id=persisted.id, reason_code="synthetic"
    )
...
def contexts(values: dict[str, object]) -> tuple[Any, Any, Any]:
    execute_context = FakeContext(
        values["company_id"],
        values["actor_id"],
        {PayrollPermission.TAX_CALCULATION_EXECUTE},
    )
    read_context = FakeContext(
        values["company_id"], values["actor_id"], {PayrollPermission.TAX_RESULT_READ}
    )
    review_context = FakeContext(
        values["company_id"],
        values["reviewer_id"],
...
async def test_persist_replay_review_verification_and_safe_events(
    ...
    async with factory() as session:
        ...
        first = await service.persist_candidate(
            session, context=execute_context, candidate=calculated, admission=accepted
        )
            ...
            (
                await session.scalars(
                    select(BusinessEvent).where(
                        BusinessEvent.entity_type == "payroll_tax_deduction_result"
                    )
                )
            ).all()
        )
            ...
            (
                await session.scalars(
                    select(AuditRecord).where(
                        AuditRecord.resource_type == "payroll_tax_deduction_result"
                    )
                )
            ).all()
        )
    ...
    async with factory() as session:
        ...
        with pytest.raises(PayrollConflictError, match="blocked"):
            await service.persist_candidate(
                session, context=execute_context, candidate=empty, admission=blocked
            )
        ...
        with pytest.raises(PayrollConflictError, match="scope"):
            await service.persist_candidate(
                session, context=other, candidate=empty, admission=not_applicable
            )
        persisted = await service.persist_candidate(
            session, context=execute_context, candidate=empty, admission=not_applicable
        )
        ...
        with pytest.raises(PayrollConflictError, match="blocked"):
            await service.period_results(
                session,
                context=read_context,
                pay_period_id=persisted.pay_period_id,
                blocked_admissions=(not_applicable,),
            )
...
async def test_supersession_is_append_only_and_cannot_fork(
    ...
    async with factory() as session:
        ...
        second_candidate = replace(
            second_candidate,
            money_version="money.currency-minor-unit-provider-rounding.v2",
            calculation_digest="",
        )
        ...
        second_candidate = replace(
            second_candidate,
            calculation_digest=canonical_digest(
                second_candidate.canonical_economic_content()
            ),
        )
        second_candidate = replace(
            second_candidate,
            result_id=f"tax-deduction-calculation:{second_candidate.calculation_digest}",
        )
        ...
        fork = replace(
            second_candidate,
            supersedes_result_id=first_candidate.result_id,
            money_version="money.v3",
            calculation_digest="",
        )
        fork = replace(
            fork, calculation_digest=canonical_digest(fork.canonical_economic_content())
        )
        fork = replace(
            fork, result_id=f"tax-deduction-calculation:{fork.calculation_digest}"
        )
        with pytest.raises(PayrollConflictError, match="lineage"):
            await service.persist_candidate(
                session, context=execute_context, candidate=fork, admission=accepted
            )
...
async def test_execute_permission_does_not_grant_review(
    ...
    async with factory() as session:
        ...
        value = await service.persist_candidate(
            session,
            context=execute_context,
            candidate=execute(gross=gross, admission=accepted),
            admission=accepted,
        )
        with pytest.raises(PayrollAuthorizationError):
            await service.initiate_review(
