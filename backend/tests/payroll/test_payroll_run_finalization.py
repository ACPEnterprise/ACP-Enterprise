...
def population(
    company_id: UUID, pay_period_id: UUID, employees: tuple[UUID, ...]
) -> PayrollPopulationEvidence:
    provisional = PayrollPopulationEvidence(
        company_id,
        pay_period_id,
        "synthetic-active-population",
        "payroll.population.v1",
        employees,
        "",
    )
    return replace(
        provisional, evidence_digest=canonical_digest(provisional.canonical_content())
    )


def contexts(values: dict[str, object]) -> tuple[Any, Any, Any, Any]:
    assemble = FakeContext(
        values["company_id"], values["actor_id"], {PayrollPermission.RUN_ASSEMBLE}
    )
    read = FakeContext(
        values["company_id"], values["actor_id"], {PayrollPermission.RUN_READ}
    )
    review = FakeContext(
        values["company_id"], values["reviewer_id"], {PayrollPermission.RUN_REVIEW}
    )
    approve = FakeContext(
        values["company_id"], values["reviewer_id"], {PayrollPermission.RUN_APPROVE}
    )
    return assemble, read, review, approve


async def approved_tax_result(
    session: AsyncSession, values: dict[str, object], pay_period_id: UUID | None = None
    ...
    existing_snapshot = await session.scalar(
        select(PayrollTimeInputRecord).where(
            PayrollTimeInputRecord.snapshot_identity == gross_candidate.time_snapshot_id
        )
    )
    ...
    gross_execute = FakeContext(
        values["company_id"],
        values["actor_id"],
        {PayrollPermission.CALCULATION_EXECUTE},
    )
    gross_review = FakeContext(
        values["company_id"],
        values["reviewer_id"],
        {PayrollPermission.CALCULATION_REVIEW},
    )
    gross = await gross_service.persist_candidate(
        session, context=gross_execute, candidate=gross_candidate
    )
    await gross_service.initiate_review(
        session, context=gross_review, result_id=gross.id, reason_code="synthetic"
    )
    await gross_service.decide_review(
        session,
        context=gross_review,
        result_id=gross.id,
        decision=GrossReviewDecision.ACCEPTED,
        reason_code="synthetic",
    )
    evidence = ApprovedGrossPayEvidence(
        gross.id,
        gross.lifecycle,
        gross.company_id,
        gross.employee_id,
    ...
    tax_execute = FakeContext(
        values["company_id"],
        values["actor_id"],
        {PayrollPermission.TAX_CALCULATION_EXECUTE},
    )
    tax_review = FakeContext(
        values["company_id"],
        values["reviewer_id"],
        {PayrollPermission.TAX_RESULT_REVIEW},
    )
    tax = await tax_service.persist_candidate(
        session, context=tax_execute, candidate=tax_candidate, admission=admitted
    )
    await tax_service.initiate_review(
        session, context=tax_review, result_id=tax.id, reason_code="synthetic"
    )
    await tax_service.decide_review(
        session,
        context=tax_review,
        result_id=tax.id,
        decision=TaxReviewDecision.ACCEPTED,
        reason_code="synthetic",
    )
...
async def test_assembly_totals_replay_review_approval_and_safe_handoffs(
    ...
    async with factory() as session:
        ...
        second_tax, _ = await approved_tax_result(
            session, second_values, tax.pay_period_id
        )
        evidence = population(
            values["company_id"],
            tax.pay_period_id,
            (values["employee_id"], second_values["employee_id"]),
        )  # type: ignore[arg-type]
        member = PayrollRunMemberInput(
            values["employee_id"], PayrollRunDisposition.READY, tax_result_id=tax.id
        )  # type: ignore[arg-type]
        second_member = PayrollRunMemberInput(
            second_values["employee_id"],
            PayrollRunDisposition.READY,
            tax_result_id=second_tax.id,
        )  # type: ignore[arg-type]
        candidate_value = await service.assemble_candidate(
            session,
            context=assemble,
            population=evidence,
            member_inputs=(member, second_member),
            currency="USD",
            assembled_at=NOW,
        )
        assert candidate_value.aggregate_gross == tax.gross_pay + second_tax.gross_pay
        assert (
            candidate_value.aggregate_net_pay
            == tax.net_pay_candidate + second_tax.net_pay_candidate
        )
        first = await service.persist_candidate(
            session, context=assemble, candidate=candidate_value
        )
        ...
        await service.initiate_review(
            session, context=review, run_id=first.id, reason_code="synthetic"
        )
        ...
        await service.approve(
            session, context=approve, run_id=first.id, reason_code="synthetic"
        )
        payment = await service.approved_handoff(
            session, context=read, run_id=first.id, purpose="future_payment_release"
        )
        accounting = await service.approved_handoff(
            session, context=read, run_id=first.id, purpose="future_accounting_posting"
        )
            ...
            (
                await session.scalars(
                    select(BusinessEvent).where(
                        BusinessEvent.entity_type == "payroll_run"
                    )
                )
            ).all()
        )
            ...
            (
                await session.scalars(
                    select(AuditRecord).where(
                        AuditRecord.resource_type == "payroll_run"
                    )
                )
            ).all()
        )
        ...
        assert all(
            "net_pay" not in str(item.payload) and "aggregate" not in str(item.payload)
            for item in events
        )
        assert all(
            "net_pay" not in str(item.details) and "aggregate" not in str(item.details)
            for item in audits
        )
        reporting = FakeContext(
            values["company_id"],
            values["reviewer_id"],
            {PayrollPermission.REPORTING_READ},
        )
        registers = await PayrollOperationsService().registers(
    ...
    async with factory() as session:
        ...
        session.add(
            Employee(
                ...
                company_id=values["company_id"],
                home_branch_id=None,
                employee_number=f"BLOCK-{uuid4().hex[:6]}",
                first_name="Blocked",
                last_name="Synthetic",
                display_name="Blocked Synthetic",
                employee_type="employee",
                status="active",
            )
        )
        ...
        blocked = replace(
            admitted,
            employee_id=blocked_id,
            state=TaxDeductionAdmissionState.MISSING,
            blockers=("synthetic:missing",),
            admission_digest="",
        )
        blocked = replace(
            blocked, admission_digest=canonical_digest(blocked.canonical_content())
        )
        evidence = population(
            values["company_id"], tax.pay_period_id, (values["employee_id"], blocked_id)
        )  # type: ignore[arg-type]
        ready = PayrollRunMemberInput(
            values["employee_id"], PayrollRunDisposition.READY, tax_result_id=tax.id
        )  # type: ignore[arg-type]
        blocked_member = PayrollRunMemberInput(
            blocked_id, PayrollRunDisposition.BLOCKED, blocked_admission=blocked
        )
        ...
        candidate_value = await service.assemble_candidate(
            ...
            context=assemble,
            population=evidence,
            member_inputs=(ready, blocked_member),
            currency="USD",
            assembled_at=NOW,
        )
        ...
        reporting = FakeContext(
            values["company_id"], values["actor_id"], {PayrollPermission.REPORTING_READ}
        )
        register = (
            await PayrollOperationsService().registers(session, context=reporting)
        )[0]
        blocked_row = next(
            item
            for item in register["members"]
            if item["employee_id"] == str(blocked_id)
        )
        assert blocked_row["status"] == "BLOCKED_FOR_PAYROLL"
        assert blocked_row["blockers"] == ["synthetic:missing"]
        assert (
            len(candidate_value.members) == 2
            and candidate_value.aggregate_net_pay == tax.net_pay_candidate
        )
        other = FakeContext(
            values["other_company_id"], values["actor_id"], {PayrollPermission.RUN_READ}
        )
        ...
        with pytest.raises(PayrollConflictError, match="blocked"):
            bad = PayrollRunMemberInput(
                blocked_id, PayrollRunDisposition.BLOCKED, blocked_admission=admitted
            )
            await service.assemble_candidate(
                session,
                context=assemble,
                population=evidence,
                member_inputs=(ready, bad),
                currency="USD",
                assembled_at=NOW,
            )
...
async def test_permissions_competing_run_and_append_only_supersession(
    ...
    async with factory() as session:
        tax, _ = await approved_tax_result(session, values)
        evidence = population(
            values["company_id"], tax.pay_period_id, (values["employee_id"],)
        )  # type: ignore[arg-type]
        ready = PayrollRunMemberInput(
            values["employee_id"], PayrollRunDisposition.READY, tax_result_id=tax.id
        )  # type: ignore[arg-type]
        ...
        changed_population = replace(
            evidence, population_identity="synthetic-corrected", evidence_digest=""
        )
        changed_population = replace(
            changed_population,
            evidence_digest=canonical_digest(changed_population.canonical_content()),
        )
        competing = await service.assemble_candidate(
            session,
            context=assemble,
        ...
        with pytest.raises(PayrollConflictError, match="active"):
            await service.persist_candidate(
                session, context=assemble, candidate=competing
            )
        successor = await service.assemble_candidate(
            session,
            context=assemble,
            population=changed_population,
            member_inputs=(ready,),
            currency="USD",
            assembled_at=NOW,
            supersedes_run_identity=first.run_identity,
        )
        second = await service.persist_candidate(
            session, context=assemble, candidate=successor
        )
        await session.refresh(first)
        assert first.lifecycle == "superseded" and second.supersedes_run_id == first.id
        no_permission = FakeContext(
            values["company_id"], values["actor_id"], {PayrollPermission.RUN_ASSEMBLE}
        )
        with pytest.raises(PayrollAuthorizationError):
            await service.initiate_review(
                session, context=no_permission, run_id=second.id, reason_code="denied"
            )
        await service.initiate_review(
            session, context=review, run_id=second.id, reason_code="synthetic"
        )
        with pytest.raises(PayrollAuthorizationError):
            await service.approve(
                session, context=review, run_id=second.id, reason_code="denied"
            )
