...
async def test_synthetic_governed_posting_is_single_consumption(
    ...
    async with factory() as session:
        ...
        poster = FakeContext(
            values["company_id"], poster_id, {AccountingPermission.JOURNAL_POST}
        )
        ...
        assert (
            await session.scalar(
                select(func.count(Journal.id)).where(
                    Journal.company_id == values["company_id"]
                )
            )
            == 1
        )
...
async def test_candidate_persistence_is_company_isolated(
    ...
    async with factory() as session:
        ...
        with pytest.raises(PayrollConflictError, match="cross-Company"):
            await PayrollAccountingFinalizationService().persist_candidate(
                session, context=other, candidate=candidate
            )
