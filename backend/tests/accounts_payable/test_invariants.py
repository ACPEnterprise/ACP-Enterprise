...
def test_bill_contract_keeps_receipt_references_as_evidence_only() -> None:
    line = BillLineSpec(
        ...
        mapping_id=uuid4(),
        branch_id=uuid4(),
        purchasing_reference="PO-evidence",
        receipt_reference="receipt-evidence",
    )
    spec = BillSpec(
        company_id=uuid4(),
        ...
        source_identity="evidence-1",
        source_digest="a" * 64,
        evidence_reference="restricted://bill/1",
...
def test_disbursement_contract_contains_no_money_movement_credentials() -> None:
    ...
    assert not (
        {"bank_account", "routing_number", "credential", "payment_token"} & fields
    )
