...
def test_database_constraints_enforce_identity_duplicate_and_sod_boundaries() -> None:
    vendor_constraints = {
        constraint.name for constraint in AccountingVendor.__table__.constraints
    }
    bill_constraints = {
        constraint.name for constraint in VendorBill.__table__.constraints
    }
    override_constraints = {
        constraint.name for constraint in DuplicateOverride.__table__.constraints
    }
    disbursement_constraints = {
        constraint.name for constraint in Disbursement.__table__.constraints
    }
...
def test_slot_four_parent_and_domain_seams() -> None:
    ...
    migration = (
        root
        / "alembic/versions/y0p2k4m6q831_create_day_one_accounts_payable_runtime.py"
    ).read_text()
