...
class _Repository:
    ...
    async def period(
        self, session: object, company_id: UUID, period_id: UUID
    ) -> object:
    ...
    async def account(
        self, session: object, company_id: UUID, account_id: UUID
    ) -> object | None:
...
def _inputs(
    tmp_path: Path,
) -> tuple[Any, Any, tuple[AccountTargetBinding, ...], tuple[BranchTargetBinding, ...]]:
    ...
    bindings = (
        AccountTargetBinding(
            "synthetic-cash",
            cash_id,
            OpeningComponent.OTHER_BALANCE_SHEET,
            "finance:map:cash",
        ),
        AccountTargetBinding(
            "synthetic-equity",
            equity_id,
            OpeningComponent.OPENING_EQUITY,
            "finance:map:equity",
        ),
    )
    return (
        ...
        accounts,
        bindings,
        (BranchTargetBinding(BRANCH_ID, UUID(BRANCH_ID)),),
    )


@pytest.mark.asyncio
async def test_reconciles_to_native_accounts_with_complete_provenance(
    tmp_path: Path,
) -> None:
    ...
    service = NativeOpeningStateService(
        accounting=accounting, repository=cast(Any, _Repository(accounts=accounts))
    )
    preparer = _context(
        uuid4(), {AccountingPermission.JOURNAL_PREPARE, AccountingPermission.RECONCILE}
    )
    ...
    assert (
        len(result.canonical_package_digest) == len(result.reconciliation_digest) == 64
    )
...
async def test_missing_evidence_is_not_coerced_to_zero(tmp_path: Path) -> None:
    ...
    result = await service.reconcile(
        cast(
            Any,
            SimpleNamespace(
                scalar=AsyncMock(return_value="opening_balance"), rollback=AsyncMock()
            ),
        ),
        package=package,
        plan=_plan(package),
        policy=_policy(),
        account_bindings=bindings[:1],
        branch_bindings=branches,
        preparer=preparer,
        finance_approver=approver,
    )
...
@pytest.mark.asyncio
async def test_conflicting_mapping_and_unresolved_policy_fail_closed(
    tmp_path: Path,
) -> None:
    package, accounts, bindings, branches = _inputs(tmp_path)
    accounting = cast(Any, AsyncMock(spec=AccountingService))
    ...
    conflict = await service.reconcile(
        ...
        plan=_plan(package),
        policy=_policy(),
        account_bindings=bindings + (bindings[0],),
        branch_bindings=branches,
        preparer=preparer,
        finance_approver=approver,
    )
...
@pytest.mark.asyncio
async def test_posts_only_through_native_lifecycle_and_replay_is_idempotent(
    tmp_path: Path,
) -> None:
    package, accounts, bindings, branches = _inputs(tmp_path)
    accounting = cast(Any, AsyncMock(spec=AccountingService))
    ...
    posted = SimpleNamespace(
        id=uuid4(), status="posted", version=4, posted_at=datetime.now(timezone.utc)
    )
    ...
    with pytest.raises(AccountingConflict, match="three distinct"):
        await service.post(
            cast(Any, SimpleNamespace(rollback=AsyncMock())),
            reconciliation=same_actor,
            preparer=preparer,
            finance_approver=preparer,
            poster=poster,
        )


@pytest.mark.asyncio
async def test_posted_opening_fact_is_consumed_by_acc_rpt_deterministically(
    tmp_path: Path,
) -> None:
    package, accounts, bindings, branches = _inputs(tmp_path)
    accounting = cast(Any, AsyncMock(spec=AccountingService))
    ...
    facts = (
        replace(
            reporting_line(
                ...
                ordinal=1,
                account_id=cast(UUID, debit.target_account_id),
                code="1000",
                classification="asset",
                normal_balance="debit",
                effective_date=policy.cutover_date,
                debit=cast(Any, debit.actual_prepared_debit),
            ...
            source_identity=f"package:{package.package_id}",
            source_digest=receipt.reconciliation_digest,
            account_effective_from=date(2030, 1, 1),
            period_id=policy.period_id,
            period_name="2030-01",
            period_start_date=date(2030, 1, 1),
            period_end_date=date(2030, 1, 31),
        ),
        replace(
            reporting_line(
                company_id=company_id,
                ...
                journal_id=journal_id,
                ordinal=2,
                account_id=cast(UUID, credit.target_account_id),
                code="3000",
                classification="equity",
                normal_balance="credit",
                effective_date=policy.cutover_date,
                credit=cast(Any, credit.actual_prepared_credit),
