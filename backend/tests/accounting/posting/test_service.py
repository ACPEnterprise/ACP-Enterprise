...
def _runtime():
    ...
    prepared = SimpleNamespace(
        **{**created.__dict__, "status": "prepared", "version": 2}
    )
    approved = SimpleNamespace(
        **{**created.__dict__, "status": "approved", "version": 3}
    )
    posted_at = datetime.now(timezone.utc)
    posted = SimpleNamespace(
        **{**created.__dict__, "status": "posted", "version": 4, "posted_at": posted_at}
    )
    ...
    service = AutomatedPostingService(
        rules=PostingRuleRegistry((rule,)), accounting=accounting
    )
    contexts = (
...
async def test_posts_balanced_journal_through_governed_core_lifecycle() -> None:
    ...
    assert (
        sum(line.debit for line in data.lines)
        == sum(line.credit for line in data.lines)
        == Decimal("40.00")
    )
...
def test_unknown_or_extra_components_never_become_zero() -> None:
    ...
    changed = replace(fact, components={"cash": Decimal(40), "fee": Decimal(1)})
