...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("supersedes_policy_id", postgresql.UUID(as_uuid=True)),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.CheckConstraint(
            f"recognition_event IN ({events})",
            name="ck_payroll_accounting_policy_event",
        ),
        sa.CheckConstraint(
            f"lifecycle IN ({lifecycle})", name="ck_payroll_accounting_policy_lifecycle"
        ),
        sa.CheckConstraint(
        ...
        sa.ForeignKeyConstraint(
            ...
            ["payroll_accounting_policy_versions.id"],
            ondelete="RESTRICT",
        ),
        sa.UniqueConstraint(
            "company_id",
        ...
        sa.UniqueConstraint("supersedes_policy_id"),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("posting_side", sa.String(8), nullable=False),
        ...
        sa.Column("approved_by_user_id", postgresql.UUID(as_uuid=True)),
        sa.Column("approved_at", sa.DateTime(timezone=True)),
        sa.Column("supersedes_mapping_id", postgresql.UUID(as_uuid=True)),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.CheckConstraint(
            f"recognition_event IN ({events})",
        ...
        sa.ForeignKeyConstraint(
            ...
            ["payroll_accounting_mapping_versions.id"],
            ondelete="RESTRICT",
        ),
        sa.UniqueConstraint(
            "company_id",
        ...
        sa.UniqueConstraint("mapping_digest"),
        sa.UniqueConstraint("supersedes_mapping_id"),
    )
    op.create_index(
...
def downgrade() -> None:
    op.drop_index(
        "ix_payroll_accounting_mapping_resolution",
        table_name="payroll_accounting_mapping_versions",
    )
    op.drop_table("payroll_accounting_mapping_versions")
    op.drop_index(
        "ix_payroll_accounting_policy_resolution",
        table_name="payroll_accounting_policy_versions",
    )
