...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "aggregate_employer_contributions", sa.Numeric(18, 2), nullable=False
        ),
        sa.Column(
        ...
        sa.ForeignKeyConstraint(
            ["pay_period_id"], ["timekeeping_pay_periods.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["assembled_by_user_id"], ["users.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["supersedes_run_id"], ["payroll_runs.id"], ondelete="RESTRICT"
        ),
        sa.UniqueConstraint("company_id", "id", name="uq_payroll_run_company_id"),
        sa.UniqueConstraint(
            "company_id", "run_identity", name="uq_payroll_run_identity"
        ),
